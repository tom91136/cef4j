 ...
 */
public final class RunQuickMenuCallback implements AutoCloseable {
    ...
    public synchronized CompletableFuture<Void> closeAsync() {
        if (closeFuture == null) {
            closeFuture = CefFutures.map(
                session.request(
                    new ReleaseHandleRequest(handle, "cef_run_quick_menu_callback_t"), ReleaseHandleResponse.DECODER),
                r -> null);
        }
        return closeFuture;
    }
    ...
    public CompletableFuture<Void> cont(int commandId, int eventFlags) {
        return CefFutures.map(
            session.request(new RunQuickMenuCallbackContRequest(handle, commandId, eventFlags), RunQuickMenuCallbackContResponse.DECODER),
            r -> null);
    }
    ...
    public CompletableFuture<Void> cancel() {
        return CefFutures.map(
            session.request(new RunQuickMenuCallbackCancelRequest(handle), RunQuickMenuCallbackCancelResponse.DECODER),
            r -> null);
    }
}
