...
public final class V8ValueGetKeysRequest implements CefMessageView, CefMessageEncoder {
    ...
    public static final int MESSAGE_ID = 341739378;

    private final RemoteHandle frame;
    private final RemoteHandle self;
    private final String[] keys;

    public V8ValueGetKeysRequest(@Nonnull RemoteHandle frame, @Nonnull RemoteHandle self, @Nonnull String[] keys) {
        this.frame = frame;
        this.self = self;
        this.keys = keys;
    }
    ...
    public int encodedSize() {
        return 12 + java.util.Arrays.stream(keys).mapToInt(__s -> 4 + __s.getBytes(StandardCharsets.UTF_8).length).sum();
    }
