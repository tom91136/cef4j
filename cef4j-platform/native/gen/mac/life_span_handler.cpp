...
struct JniCefLifeSpanHandler : public cef_life_span_handler_t {
    ...
    static int CEF_CALLBACK _on_before_popup(cef_life_span_handler_t* self, struct _cef_browser_t* browser, struct _cef_frame_t* frame, int popup_id, const cef_string_t* target_url, const cef_string_t* target_frame_name, cef_window_open_disposition_t target_disposition, int user_gesture, const cef_popup_features_t* popupFeatures, struct _cef_window_info_t* windowInfo, struct _cef_client_t** client, struct _cef_browser_settings_t* settings, struct _cef_dictionary_value_t** extra_info, int* no_javascript_access) {
        ...
        auto j_windowInfo_cls = FindClassCached(env, "net/kurobako/cef4j/gen/mac/CefWindowInfo$Mutable");
        auto j_windowInfo_ctor = env->GetMethodID(j_windowInfo_cls, "<init>", "(Ljava/lang/String;Lnet/kurobako/cef4j/gen/CefRect;IJIIIJLnet/kurobako/cef4j/gen/CefRuntimeStyle;)V");
    ...
    ? env->NewObject(j_windowInfo_cls, j_windowInfo_ctor,
        ...
        static_cast<jint>((windowInfo)->hidden),
        to_jlong((windowInfo)->parent_view),
    ...
    : nullptr;
        ...
        if (windowInfo && j_windowInfo) {
            ...
            (windowInfo)->hidden = static_cast<decltype((windowInfo)->hidden)>(env->GetIntField(j_windowInfo, env->GetFieldID(j_windowInfo_cls, "hidden", "I")));
            (windowInfo)->parent_view = from_jlong<decltype((windowInfo)->parent_view)>(env->GetLongField(j_windowInfo, env->GetFieldID(j_windowInfo_cls, "parentView", "J")));
            ...
            (windowInfo)->view = from_jlong<decltype((windowInfo)->view)>(env->GetLongField(j_windowInfo, env->GetFieldID(j_windowInfo_cls, "view", "J")));
