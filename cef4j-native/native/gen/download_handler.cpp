...
// JNI wrapper struct for cef_download_handler_t
struct JniCefDownloadHandler: public cef_download_handler_t {
    ...
    JniCefDownloadHandler(JavaVM *vm, jobject handler) : cef_download_handler_t { }, jvm(vm) {
        javaHandler = handler;
        InitRefCount<JniCefDownloadHandler, cef_download_handler_t> (&base);
...
extern "C" JNIEXPORT jlong JNICALL Java_net_kurobako_cef4j_gen_CefDownloadHandler_1N_N_1Create(JNIEnv* env, jobject obj) {
    return reinterpret_cast<jlong>(Create_JniCefDownloadHandler(env, obj));
}
