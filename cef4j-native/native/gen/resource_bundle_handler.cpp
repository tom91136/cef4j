...
// JNI wrapper struct for cef_resource_bundle_handler_t
struct JniCefResourceBundleHandler: public cef_resource_bundle_handler_t {
    ...
    JniCefResourceBundleHandler(JavaVM *vm, jobject handler) : cef_resource_bundle_handler_t { }, jvm(vm) {
        javaHandler = handler;
        InitRefCount<JniCefResourceBundleHandler, cef_resource_bundle_handler_t> (&base);
    ...
    static int CEF_CALLBACK _get_localized_string(cef_resource_bundle_handler_t* self, int string_id, cef_string_t* string) {
        ...
        auto mid = env->GetMethodID(cls, "getLocalizedString", "(ILjava/lang/String;)I");
        if (!mid) {env->PopLocalFrame(nullptr); return 0;}
        auto jResult = env->CallIntMethod(h->javaHandler, mid, static_cast<jint>(string_id), j_string);
    ...
    static int CEF_CALLBACK _get_data_resource(cef_resource_bundle_handler_t* self, int resource_id, void** data, size_t* data_size) {
        auto* h = reinterpret_cast<JniCefResourceBundleHandler*>(self);
        ScopedJNIEnv env(h->jvm);
        if (env->PushLocalFrame(9) < 0) {return 0;}
        ...
        auto cls = env->GetObjectClass(h->javaHandler);
        auto mid = env->GetMethodID(cls, "getDataResource", "(ILnet/kurobako/cef4j/gen/NativePointer;[J)I");
        if (!mid) {env->PopLocalFrame(nullptr); return 0;}
        auto jResult = env->CallIntMethod(h->javaHandler, mid, static_cast<jint>(resource_id), j_data, j_data_size);
        if (CheckJNIException(env)) {env->PopLocalFrame(nullptr); return 0;}
    ...
    static int CEF_CALLBACK _get_data_resource_for_scale(cef_resource_bundle_handler_t* self, int resource_id, cef_scale_factor_t scale_factor, void** data, size_t* data_size) {
        ...
        if (data_size) {jlong _v = static_cast<jlong>(*data_size); env->SetLongArrayRegion(j_data_size, 0, 1, &_v);}
        auto cls = env->GetObjectClass(h->javaHandler);
        auto mid = env->GetMethodID(cls, "getDataResourceForScale", "(ILnet/kurobako/cef4j/gen/CefScaleFactor;Lnet/kurobako/cef4j/gen/NativePointer;[J)I");
        if (!mid) {env->PopLocalFrame(nullptr); return 0;}
        auto jResult = env->CallIntMethod(h->javaHandler, mid, static_cast<jint>(resource_id), j_scale_factor, j_data, j_data_size);
        if (CheckJNIException(env)) {env->PopLocalFrame(nullptr); return 0;}
        if (data_size) {jlong _v; env->GetLongArrayRegion(j_data_size, 0, 1, &_v); *data_size = static_cast<size_t>(_v);}
        int nativeResult = static_cast<int>(jResult);
        env->PopLocalFrame(nullptr);
...
extern "C" JNIEXPORT jlong JNICALL Java_net_kurobako_cef4j_gen_CefResourceBundleHandler_1N_N_1Create(JNIEnv* env, jobject obj) {
    return reinterpret_cast<jlong>(Create_JniCefResourceBundleHandler(env, obj));
}
