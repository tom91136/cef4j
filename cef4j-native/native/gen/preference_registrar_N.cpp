...
extern "C" JNIEXPORT jboolean JNICALL Java_net_kurobako_cef4j_gen_CefPreferenceRegistrar_00024NativePeer_N_1AddPreference(JNIEnv* env, jobject obj, jlong self, jstring name, jobject default_value) {
    ...
    if (!default_value) {env->ThrowNew(env->FindClass("java/lang/NullPointerException"), "defaultValue must not be null"); return JNI_FALSE;}
    auto _name_str = JStringToCefString(env, name);
    cef_value_t* _default_value_ptr = reinterpret_cast<cef_value_t*>(env->GetLongField(default_value, env->GetFieldID(env->GetObjectClass(default_value), "nativePtr", "J")));
