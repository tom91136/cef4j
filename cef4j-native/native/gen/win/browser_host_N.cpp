...
CEF4J_JNI_EXPORT(void, CEF4J_PEER(CefBrowserHost), showDevTools0)(JNIEnv* env, jobject obj, jlong self, jobject windowInfo, jobject client, jobject settings, jobject inspect_element_at) {
    ...
    if (windowInfo) {
        ...
        _windowInfo_val.style = static_cast<decltype(_windowInfo_val.style)>(env->GetIntField(windowInfo, env->GetFieldID(_windowInfo_c, "style", "I")));
        ...
        _windowInfo_val.window = from_jlong<decltype(_windowInfo_val.window)>(env->GetLongField(windowInfo, env->GetFieldID(_windowInfo_c, "window", "J")));
