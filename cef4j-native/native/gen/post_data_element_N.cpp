...
CEF4J_JNI_EXPORT(void, CEF4J_PEER(CefPostDataElement), setToBytes0)(JNIEnv* env, jobject obj, jlong self, jobject bytes) {
    auto* s = reinterpret_cast<cef_post_data_element_t*>(self);
    if (!s) return;
    if (!bytes) { env->ThrowNew(FindClassCached(env, "java/lang/NullPointerException"), "bytes must not be null"); return; }
    const void* _bytes_addr = bytes ? env->GetDirectBufferAddress(bytes) : nullptr;
    if (bytes && !_bytes_addr) { env->ThrowNew(FindClassCached(env, "java/lang/IllegalArgumentException"), "bytes must be a direct ByteBuffer; use ByteBuffer.allocateDirect(...)"); return; }
...
CEF4J_JNI_EXPORT(jlong, CEF4J_PEER(CefPostDataElement), getBytes0)(JNIEnv* env, jobject obj, jlong self, jobject bytes) {
    auto* s = reinterpret_cast<cef_post_data_element_t*>(self);
    if (!s) return 0;
    if (!bytes) { env->ThrowNew(FindClassCached(env, "java/lang/NullPointerException"), "bytes must not be null"); return 0; }
    void* _bytes_addr = bytes ? env->GetDirectBufferAddress(bytes) : nullptr;
    if (bytes && !_bytes_addr) { env->ThrowNew(FindClassCached(env, "java/lang/IllegalArgumentException"), "bytes must be a direct ByteBuffer; use ByteBuffer.allocateDirect(...)"); return 0; }
...
CEF4J_JNI_EXPORT(jobject, CEF4J_PEER(CefPostDataElement), create0)(JNIEnv* env, jclass clz) {
    ...
    auto _rCls = FindClassCached(env, "net/kurobako/cef4j/gen/CefPostDataElement$NativePeer");
