...
struct JniCefV8Accessor : public cef_v8_accessor_t {
    JavaVM* jvm;
    jobject javaHandler;  // global ref
    std::atomic<int> refCount{1};

    JniCefV8Accessor(JavaVM* vm, jobject handler) : cef_v8_accessor_t{}, jvm(vm) {
        javaHandler = handler;
        InitRefCount<JniCefV8Accessor, cef_v8_accessor_t>(reinterpret_cast<cef_base_ref_counted_t*>(static_cast<cef_v8_accessor_t*>(this)));
        get = &_get;
        set = &_set;
    }

    static int CEF_CALLBACK _get(cef_v8_accessor_t* self, const cef_string_t* name, struct _cef_v8_value_t* object, struct _cef_v8_value_t** retval, cef_string_t* exception) {
        auto* h = reinterpret_cast<JniCefV8Accessor*>(self);
        ...
        auto mid = env->GetMethodID(cls, "get", "(Ljava/lang/String;Lnet/kurobako/cef4j/gen/CefV8Value;Ljava/util/concurrent/atomic/AtomicReference;Ljava/lang/String;)Z");
        if (!mid) { env->PopLocalFrame(nullptr); return false; }
        auto jResult = env->CallBooleanMethod(h->javaHandler, mid, j_name, j_object, j_retval, j_exception);
    ...
    static int CEF_CALLBACK _set(cef_v8_accessor_t* self, const cef_string_t* name, struct _cef_v8_value_t* object, struct _cef_v8_value_t* value, cef_string_t* exception) {
        auto* h = reinterpret_cast<JniCefV8Accessor*>(self);
        ScopedJNIEnv env(h->jvm);
        ...
        auto j_exception = CefStringToJString(env, exception);
        auto cls = env->GetObjectClass(h->javaHandler);
        auto mid = env->GetMethodID(cls, "set", "(Ljava/lang/String;Lnet/kurobako/cef4j/gen/CefV8Value;Lnet/kurobako/cef4j/gen/CefV8Value;Ljava/lang/String;)Z");
        if (!mid) { env->PopLocalFrame(nullptr); return false; }
        auto jResult = env->CallBooleanMethod(h->javaHandler, mid, j_name, j_object, j_value, j_exception);
        if (CheckJNIException(env)) { env->PopLocalFrame(nullptr); return false; }
        env->PopLocalFrame(nullptr);
        return jResult;
    }
...
extern "C" cef_v8_accessor_t* Create_JniCefV8Accessor(JNIEnv* env, jobject handler) {
    ...
    return reinterpret_cast<cef_v8_accessor_t*>(new JniCefV8Accessor(jvm, globalRef));
}
