...
struct JniCefRequestHandler : public cef_request_handler_t {
    ...
    static cef_resource_request_handler_t* CEF_CALLBACK _get_resource_request_handler(cef_request_handler_t* self, struct _cef_browser_t* browser, struct _cef_frame_t* frame, struct _cef_request_t* request, int is_navigation, int is_download, const cef_string_t* request_initiator, int* disable_default_handling) {
        auto* h = reinterpret_cast<JniCefRequestHandler*>(self);
        ScopedJNIEnv env(h->jvm);
