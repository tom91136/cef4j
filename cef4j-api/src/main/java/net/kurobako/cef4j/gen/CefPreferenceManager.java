...
/**
 * Manage access to preferences. Many built-in preferences are registered by Chromium. Custom preferences can be registered in {@link net.kurobako.cef4j.gen.CefBrowserProcessHandler#onRegisterCustomPreferences(CefPreferencesType, CefPreferenceRegistrar)}.
...
public interface CefPreferenceManager extends CefLibraryObject {
     ...
     * <p>Definition generated from cef_preference_capi.h
     * <pre>int (CEF_CALLBACK* set_preference)(struct _cef_preference_manager_t* self, const cef_string_t* name, struct _cef_value_t* value, cef_string_t* error);</pre>
     *
     ...
     * <p>Definition generated from cef_preference_capi.h
     * <pre>cef_registration_t* (CEF_CALLBACK* add_preference_observer)(struct _cef_preference_manager_t* self, const cef_string_t* name, struct _cef_preference_observer_t* observer);</pre>
     *
    ...
    /**
     * Returns the current Chrome Variations configuration (combination of field trials and chrome://flags) as equivalent command-line switches (`--[enable|disable]-features=XXXX`, etc). These switches can be used to apply the same configuration when launching a CEF-based application. See <a href="https://developer.chrome.com/docs/web-platform/chrome-variations">https://developer.chrome.com/docs/web-platform/chrome-variations</a> for background and details. Note that field trial tests are disabled by default in Official CEF builds (via the `disable_fieldtrial_testing_config={@code true}` GN flag). This method must be called on the browser process UI thread.
     * <p>Definition generated from cef_preference_capi.h
    ...
    /**
     * Returns the current Chrome Variations configuration (combination of field trials and chrome://flags) as human-readable strings. This is the human-readable equivalent of the "Active Variations" section of chrome://version. See <a href="https://developer.chrome.com/docs/web-platform/chrome-variations">https://developer.chrome.com/docs/web-platform/chrome-variations</a> for background and details. Note that field trial tests are disabled by default in Official CEF builds (via the `disable_fieldtrial_testing_config={@code true}` GN flag). This method must be called on the browser process UI thread.
     * <p>Definition generated from cef_preference_capi.h
