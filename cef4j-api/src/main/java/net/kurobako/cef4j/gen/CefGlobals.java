...
public final class CefGlobals {
     ...
     * executable specified by the cef_settings_t.browser_subprocess_path value. If called for the browser process
     * (identified by no "type" command-line value) it will return immediately with a value of -1. If called for a
     * recognized secondary process it will block until the process should exit and then return the process exit code.
     * The {@code application} parameter may be empty. The {@code windows_sandbox_info} parameter is only used on
     * Windows and may be {@code null} (see cef_sandbox_win.h for details).
     *
    ...
    public static int executeProcess(
            @Nonnull CefMainArgs args, @Nullable CefApp application, @Nullable NativePointer windowsSandboxInfo) {
        return executeProcess0(args, application, windowsSandboxInfo);
    }

    /**
     * This function should be called on the main application thread to initialize the CEF browser process. The
     * {@code application} parameter may be empty. Returns {@code true} if initialization succeeds. Returns
     * {@code false} if initialization fails or if early exit is desired (for example, due to process singleton relaunch
     * behavior). If this function returns {@code false} then the application should exit immediately without calling
     * any other CEF functions except, optionally, CefGetExitCode. The {@code windows_sandbox_info} parameter is only
     * used on Windows and may be {@code null} (see cef_sandbox_win.h for details).
     *
     * <p>Definition generated from cef_app_capi.h
     *
     * <pre>
    ...
    /**
     * This function should be called on the main application thread to shut down the CEF browser process before the
     * application exits. Do not call any other CEF functions after calling this function.
     *
     * <p>Definition generated from cef_app_capi.h
     *
    ...
    /**
     * Perform a single iteration of CEF message loop processing. This function is provided for cases where the CEF
     * message loop must be integrated into an existing application message loop. Use of this function is not
    ...
    /**
     * Run the CEF message loop. Use this function instead of an application-provided message loop to get the best
     * balance between performance and CPU usage. This function should only be called on the main application thread and
     * only if CefInitialize() is called with a cef_settings_t.multi_threaded_message_loop value of {@code false}. This
    ...
    /**
     * Quit the CEF message loop that was started by calling CefRunMessageLoop(). This function should only be called on
     * the main application thread and only if CefRunMessageLoop() was used.
     *
     * <p>Definition generated from cef_app_capi.h
     *
    ...
    /**
     * Set to {@code true} before calling OS APIs on the CEF UI thread that will enter a native message loop (see usage
     * restrictions below). Set to {@code false} after exiting the native message loop. On Windows, use the
     * CefSetOSModalLoop function instead in cases like native top menus where resize of the browser content is not
     * required, or in cases like printer APIs where reentrancy safety cannot be guaranteed.
     *
     * <p>Nested processing of Chromium tasks is disabled by default because common controls and/or printer functions
     * may use nested native message loops that lead to unplanned reentrancy. This function re-enables nested processing
     * in the scope of an upcoming native message loop. It must only be used in cases where the stack is reentrancy safe
     * and processing nestable tasks is explicitly safe. Do not use in cases (like the printer example) where an OS API
     * may experience unplanned reentrancy as a result of a new task executing immediately.
     *
     * <p>For instance,
     *
     * <ul>
     *   <li>The UI thread is running a message loop.
     *   <li>It receives a task #1 and executes it.
     *   <li>The task #1 implicitly starts a nested message loop. For example, via
     * </ul>
     *
     * Windows APIs such as MessageBox or GetSaveFileName, or default handling of a user-initiated drag/resize operation
     * (e.g. DefWindowProc handling of WM_SYSCOMMAND for SC_MOVE/SC_SIZE).
     *
     * <ul>
     *   <li>The UI thread receives a task #2 before or while in this second message
     * </ul>
     *
     * loop.
     *
     * <ul>
     *   <li>With NestableTasksAllowed set to {@code true}, the task #2 will run right away.
     * </ul>
     *
     ...
     *
     * <p><b>WARNING:</b> In general, you should use the temp directory variants below instead of this function. Those
     * variants will ensure that the proper permissions are set so that other users on the system can't edit them while
    ...
    /**
     * Creates a new directory. On Windows if {@code prefix} is provided the new directory name is in the format of
     * "prefixyyyy". Returns {@code true} on success and sets {@code new_temp_path} to the full path of the directory
     * that was created. The directory is only readable by the current user. Calling this function on the browser
    ...
    /**
     * Creates a directory within another directory. Extra characters will be appended to {@code prefix} to ensure that
     * the new directory does not have the same name as an existing directory. Returns {@code true} on success and sets
     * {@code new_dir} to the full path of the directory that was created. The directory is only readable by the current
     * user. Calling this function on the browser process UI or IO threads is not allowed.
     *
     * <p>Definition generated from cef_file_util_capi.h
     *
     * <pre>
    ...
    /**
     * Returns {@code true} if the given path exists and is a directory. Calling this function on the browser process UI
    ...
    /**
     * Deletes the given path whether it's a file or a directory. If {@code path} is a directory all contents will be
     * deleted. If {@code recursive} is {@code true} any sub-directories and their contents will also be deleted
     * (equivalent to executing "rm -rf", so use with caution). On POSIX environments if {@code path} is a symbolic link
     * then only the symlink will be deleted. Returns {@code true} on successful deletion or if {@code path} does not
     * exist. Calling this function on the browser process UI or IO threads is not allowed.
     *
     * <p>Definition generated from cef_file_util_capi.h
     *
    ...
    /**
     * Writes the contents of {@code src_dir} into a zip archive at {@code dest_file}. If {@code include_hidden_files}
     * is {@code true} files starting with "." will be included. Returns {@code true} on success. Calling this function
     * on the browser process UI or IO threads is not allowed.
     *
     * <p>Definition generated from cef_file_util_capi.h
     *
     * <pre>
     ...
     *
     * @see <a
     *     href="https://cef-builds.spotifycdn.com/docs/150.0/cef__origin__whitelist_8h.html">cef_origin_whitelist.h:43</a>
     */
     ...
     *
     * @see <a
     *     href="https://cef-builds.spotifycdn.com/docs/150.0/cef__origin__whitelist_8h.html">cef_origin_whitelist.h:86</a>
     */
     ...
     *
     * @see <a
     *     href="https://cef-builds.spotifycdn.com/docs/150.0/cef__origin__whitelist_8h.html">cef_origin_whitelist.h:96</a>
     */
    ...
    private static native int executeProcess0(
            @Nonnull CefMainArgs args, @Nullable CefApp application, @Nullable NativePointer windowsSandboxInfo);
    ...
    private static native int createTempDirectoryInDirectory0(
            @Nullable String baseDir, @Nullable String prefix, @Nullable String newDir);
    ...
    private static native int resolveUrl0(
            @Nullable String baseUrl, @Nullable String relativeUrl, @Nullable String resolvedUrl);
    ...
    private static native String uridecode0(
            @Nullable String text, int convertToUtf8, @Nonnull CefUriUnescapeRule unescapeRule);
    ...
    private static native int registerExtension0(
            @Nullable String extensionName, @Nullable String javascriptCode, @Nullable CefV8Handler handler);
}
