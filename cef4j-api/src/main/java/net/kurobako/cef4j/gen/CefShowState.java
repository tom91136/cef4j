// GENERATED - do not edit. Regenerate via: mvn generate-sources -pl cef4j-native
package net.kurobako.cef4j.gen;

/**
 * Show states supported by CefWindowDelegate.getInitialShowState().
 *
 * <p>Definition generated from cef_types.h
 *
 * <pre>typedef enum {
 *   CEF_SHOW_STATE_NORMAL = 0,
 *   CEF_SHOW_STATE_MINIMIZED = 1,
 *   CEF_SHOW_STATE_MAXIMIZED = 2,
 *   CEF_SHOW_STATE_FULLSCREEN = 3,
 *   CEF_SHOW_STATE_HIDDEN = 4,
 *   ...
 * } cef_show_state_t;</pre>
 *
 * <p>Possible values: {@link Kind#NORMAL}, {@link Kind#MINIMIZED}, {@link Kind#MAXIMIZED}, {@link Kind#FULLSCREEN},
 * {@link Kind#HIDDEN}, {@link Kind#NUM_VALUES}
 *
 * @see <a href="https://cef-builds.spotifycdn.com/docs/146.0/cef__types_8h.html">cef_types.h</a>
 */
public final class CefShowState implements CefEnum<CefShowState> {

    /** Known constants for {@link CefShowState}. */
    public enum Kind {
        NORMAL(0, "0", "CEF_SHOW_STATE_NORMAL"),
        MINIMIZED(1, "1", "CEF_SHOW_STATE_MINIMIZED"),
        MAXIMIZED(2, "2", "CEF_SHOW_STATE_MAXIMIZED"),
        FULLSCREEN(3, "3", "CEF_SHOW_STATE_FULLSCREEN"),
        HIDDEN(4, "4", "CEF_SHOW_STATE_HIDDEN"),
        NUM_VALUES(5, "5", "CEF_SHOW_STATE_NUM_VALUES");

        private static final Kind[] VALUES = Kind.values();

        /** The underlying C enum numeric value. */
        public final long value;

        /** The original C expression (e.g., {@code "1 << 3"}), or the numeric string. */
        public final String expr;

        /** The C constant name (e.g., {@code "cef_show_state_t"}). */
        public final String name;

        Kind(long value, String expr, String name) {
            this.value = value;
            this.expr = expr;
            this.name = name;
        }

        @Override
        public String toString() {
            return name + "(expr=" + expr + ", value=" + value + ")";
        }
    }

    /** The underlying C enum numeric value. May not correspond to any known {@link Kind}. */
    public final long value;

    private CefShowState(long value) {
        this.value = value;
    }

    @Override
    public long value() {
        return value;
    }

    @Override
    public String expr() {
        return kind().map(k -> k.expr).orElse(String.valueOf(value));
    }

    @Override
    public String name() {
        return kind().map(k -> k.name).orElse("UNKNOWN(" + value + ")");
    }

    /**
     * Returns the {@link Kind} matching this value, or empty for unknown/composite values. Use this for exhaustive
     * switch over known constants.
     */
    public java.util.Optional<Kind> kind() {
        for (Kind k : Kind.VALUES) {
            if (k.value == value) return java.util.Optional.of(k);
        }
        return java.util.Optional.empty();
    }

    /** Returns an instance for the given raw value. No data is lost — unknown or composite values are preserved. */
    public static CefShowState of(long v) {
        return new CefShowState(v);
    }

    /** Returns an instance for the given known constant. */
    public static CefShowState of(Kind k) {
        return new CefShowState(k.value);
    }

    @Override
    public boolean equals(Object obj) {
        if (this == obj) return true;
        if (!(obj instanceof CefShowState)) return false;
        return this.value == ((CefShowState) obj).value;
    }

    @Override
    public int hashCode() {
        return Long.hashCode(value);
    }

    @Override
    public String toString() {
        return kind().map(Kind::toString).orElse("UNKNOWN(value=" + value + ")");
    }
}
