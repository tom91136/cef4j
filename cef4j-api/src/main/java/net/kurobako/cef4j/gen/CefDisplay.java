...
/**
 * This class typically, but not always, corresponds to a physical display connected to the system. A fake Display may
 * exist on a headless system, or a Display may correspond to a remote, virtual display. All size and position values
 * are in density independent pixel (DIP) coordinates unless otherwise indicated. Methods must be called on the browser
 * process UI thread unless otherwise indicated. For details on coordinate systems and usage see <a
...
public interface CefDisplay extends CefLibraryObject {
    ...
    /**
     * Returns this Display's device pixel scale factor. This specifies how much the UI should be scaled when the actual
     * output has more pixels than standard displays (which is around 100~120dpi). The potential return values differ by
     * platform. Windowed browsers with 1.0 zoom will have a JavaScript `window.devicePixelRatio` value matching the
     * associated Display's GetDeviceScaleFactor() value.
     *
     * <p>Definition generated from views/cef_display_capi.h
     *
    ...
    /**
     * Returns this Display's work area in DIP screen coordinates. This excludes areas of the display that are occupied
     * with window manager toolbars, etc.
     *
     * <p>Definition generated from views/cef_display_capi.h
     *
    ...
    /**
     * Convert {@code point} from DIP screen coordinates to pixel screen coordinates. This method is only used on
     * Windows.
     *
     * <p>Definition generated from views/cef_display_capi.h
     *
    ...
    /**
     * Convert {@code point} from pixel screen coordinates to DIP screen coordinates. This method is only used on
     * Windows.
     *
     * <p>Definition generated from views/cef_display_capi.h
     *
    ...
    /**
     * Convert {@code rect} from DIP screen coordinates to pixel screen coordinates. This method is only used on
     * Windows.
     *
     * <p>Definition generated from views/cef_display_capi.h
     *
    ...
    /**
     * Convert {@code rect} from pixel screen coordinates to DIP screen coordinates. This method is only used on
     * Windows.
     *
     * <p>Definition generated from views/cef_display_capi.h
     *
    ...
    final class NativePeer implements CefDisplay, AutoCloseable {
        ...
        private static native long getId0(long self);

        private static native float getDeviceScaleFactor0(long self);

        private static native void convertPointToPixels0(long self, CefPoint.Mutable point);

        private static native void convertPointFromPixels0(long self, CefPoint.Mutable point);

        private static native CefRect getBounds0(long self);

        private static native CefRect getWorkArea0(long self);

        private static native int getRotation0(long self);
