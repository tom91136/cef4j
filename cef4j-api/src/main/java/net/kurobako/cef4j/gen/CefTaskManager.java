...
public interface CefTaskManager extends CefLibraryObject {
    ...
    /**
     * Gets the list of task IDs currently tracked by the task manager. Tasks that share the same process id will always be consecutive. The list will be sorted in a way that reflects the process tree: the browser process will be first, followed by the gpu process if it exists. Related processes (e.g., a subframe process and its parent) will be kept together if possible. Callers can expect this ordering to be stable when a process is added or removed. The task IDs are unique within the application lifespan. Returns {@code false} if the method was called from the incorrect thread.
     * <p><b>The C API exposes this as a two-pass pattern: first call {@link #getTasksCount()} to obtain
     * the count, then allocate and populate the array. This method performs both steps and returns the
     * result directly.</b>
     * <p>Definition generated from cef_task_manager_capi.h
    ...
    /**
     * Gets information about the task with {@code task_id}. Returns {@code true} if the information about the task was successfully retrieved and {@code false} if the {@code task_id} is invalid or the method was called from the incorrect thread.
     * <p>Definition generated from cef_task_manager_capi.h
    ...
    /**
     * Returns the task ID associated with the main task for {@code browser_id} (value from {@link net.kurobako.cef4j.gen.CefBrowser#getIdentifier()}). Returns -1 if {@code browser_id} is invalid, does not currently have an associated task, or the method was called from the incorrect thread.
     * <p>Definition generated from cef_task_manager_capi.h
