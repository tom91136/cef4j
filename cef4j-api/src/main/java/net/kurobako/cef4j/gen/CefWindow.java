 ...
 * bar, icon and buttons that supports moving and resizing. All size and position values are in density independent
 * pixels (DIP) unless otherwise indicated. Methods must be called on the browser process UI thread unless otherwise
 * indicated.
 *
...
public interface CefWindow extends CefLibraryObject {
    ...
    /**
     * Returns the View that currently has focus in this Window, or {@code null} if no View currently has focus. A
     * Window may have a focused View even if it is not currently active. Any focus changes while a Window is not active
     * may be applied after that Window next becomes active.
     *
     * <p>Definition generated from views/cef_window_capi.h
     *
    ...
    /**
     * Set the Window App icon. This should be a larger icon for use in the host environment app switching UI. On
     * Windows, this is the ICON_BIG used in Alt-Tab list and Windows taskbar. The Window icon will be used by default
    ...
    /**
     * Add a View that will be overlayed on the Window contents with absolute positioning and high z-order. Positioning
     * is controlled by {@code docking_mode} as described below. Setting {@code can_activate} to {@code true} will allow
     ...
     *
     * <pre>
     * cef_overlay_controller_t* (CEF_CALLBACK* add_overlay_view)(struct _cef_window_t* self, struct _cef_view_t* view, cef_docking_mode_t docking_mode, int can_activate);
     * </pre>
     *
     * @see <a href="https://cef-builds.spotifycdn.com/docs/146.0/cef__window_8h.html">views/cef_window.h:243</a>
     */
    Optional<CefOverlayController> addOverlayView(
            @Nullable CefView view, @Nonnull CefDockingMode dockingMode, boolean canActivate);
    ...
    /**
     * Returns the Display that most closely intersects the bounds of this Window. May return {@code null} if this
     * Window is not currently displayed.
     *
     * <p>Definition generated from views/cef_window_capi.h
     *
    ...
    /**
     * Set the regions where mouse events will be intercepted by this Window to support drag operations. Call this
     * method with an empty vector to clear the draggable regions. The draggable region bounds should be in window
     * coordinates.
     *
     * <p>Definition generated from views/cef_window_capi.h
     *
    ...
    /**
     * Simulate a key press. {@code key_code} is the VKEY_* value from Chromium's ui/events/keycodes/keyboard_codes.h
     * header (VK_* values on Windows). {@code event_flags} is some combination of EVENTFLAG_SHIFT_DOWN,
     * EVENTFLAG_CONTROL_DOWN and/or EVENTFLAG_ALT_DOWN. This method is exposed primarily for testing purposes.
     *
     * <p>Definition generated from views/cef_window_capi.h
     *
    ...
    /**
     * Simulate a mouse move. The mouse cursor will be moved to the specified (screen_x, screen_y) position. This method
     * is exposed primarily for testing purposes.
     *
     * <p>Definition generated from views/cef_window_capi.h
     *
    ...
    /**
     * Simulate mouse down and/or mouse up events. {@code button} is the mouse button type. If {@code mouse_down} is
     * {@code true} a mouse down event will be sent. If {@code mouse_up} is {@code true} a mouse up event will be sent.
     * If both are {@code true} a mouse down event will be sent followed by a mouse up event (equivalent to clicking the
     * mouse button). The events will be sent using the current cursor position so make sure to call SendMouseMove()
     * first to position the mouse. This method is exposed primarily for testing purposes.
     *
     * <p>Definition generated from views/cef_window_capi.h
     *
     * <pre>
    ...
    /**
     * Override a standard theme color or add a custom color associated with {@code color_id}. See cef_color_ids.h for
    ...
    final class NativePeer implements CefWindow, AutoCloseable {
        ...
        @Override
        public Optional<CefOverlayController> addOverlayView(
                @Nullable CefView view, @Nonnull CefDockingMode dockingMode, boolean canActivate) {
            checkNotClosed();
        ...
        private static native void show0(long self);

        private static native void showAsBrowserModalDialog0(long self, CefBrowserView browserView);

        private static native void hide0(long self);

        private static native void centerWindow0(long self, CefSize size);

        private static native void cefClose0(long self);

        private static native boolean isClosed0(long self);

        private static native void activate0(long self);

        private static native void deactivate0(long self);

        private static native boolean isActive0(long self);

        private static native void bringToTop0(long self);

        private static native void setAlwaysOnTop0(long self, boolean onTop);

        private static native boolean isAlwaysOnTop0(long self);

        private static native void maximize0(long self);

        private static native void minimize0(long self);

        private static native void restore0(long self);

        private static native void setFullscreen0(long self, boolean fullscreen);

        private static native boolean isMaximized0(long self);

        private static native boolean isMinimized0(long self);

        private static native boolean isFullscreen0(long self);

        private static native CefView getFocusedView0(long self);

        private static native void setTitle0(long self, String title);

        private static native String getTitle0(long self);

        private static native void setWindowIcon0(long self, CefImage image);

        private static native CefImage getWindowIcon0(long self);

        private static native void setWindowAppIcon0(long self, CefImage image);

        private static native CefImage getWindowAppIcon0(long self);

        private static native CefOverlayController addOverlayView0(
                long self, CefView view, CefDockingMode dockingMode, boolean canActivate);
        ...
        private static native void cancelMenu0(long self);

        private static native CefDisplay getDisplay0(long self);

        private static native CefRect getClientAreaBoundsInScreen0(long self);

        private static native void setDraggableRegions0(long self, long regionsCount, NativePointer regions);

        private static native long getWindowHandle0(long self);

        private static native void sendKeyPress0(long self, int keyCode, int eventFlags);

        private static native void sendMouseMove0(long self, int screenX, int screenY);

        private static native void sendMouseEvents0(
                long self, CefMouseButtonType button, boolean mouseDown, boolean mouseUp);

        private static native void setAccelerator0(
                long self,
                int commandId,
                int keyCode,
                boolean shiftPressed,
        ...
        private static native void removeAccelerator0(long self, int commandId);

        private static native void removeAllAccelerators0(long self);

        private static native void setThemeColor0(long self, int colorId, int color);

        private static native void themeChanged0(long self);
