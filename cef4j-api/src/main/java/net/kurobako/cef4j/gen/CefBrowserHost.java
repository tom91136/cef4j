...
public interface CefBrowserHost extends CefLibraryObject {
    ...
    /**
     * Request that the browser close. Closing a browser is a multi-stage process that may complete either synchronously or asynchronously, and involves callbacks such as {@link net.kurobako.cef4j.gen.CefLifeSpanHandler#doClose(CefBrowser)} (Alloy style only), {@link net.kurobako.cef4j.gen.CefLifeSpanHandler#onBeforeClose(CefBrowser)}, and a top-level window close handler such as {@link net.kurobako.cef4j.gen.views.CefWindowDelegate#canClose(CefWindow)} (or platform-specific equivalent). In some cases a close request may be delayed or canceled by the user. Using TryCloseBrowser() instead of CloseBrowser() is recommended for most use cases. See {@link net.kurobako.cef4j.gen.CefLifeSpanHandler#doClose(CefBrowser)} documentation for detailed usage and examples.
     * <p>
     * If {@code force_close} is {@code false} then JavaScript unload handlers, if any, may be fired and the close may be delayed or canceled by the user. If {@code force_close} is {@code true} then the user will not be prompted and the close will proceed immediately (possibly asynchronously). If browser close is delayed and not canceled the default behavior is to call the top-level window close handler once the browser is ready to be closed. This default behavior can be changed for Alloy style browsers by implementing {@link net.kurobako.cef4j.gen.CefLifeSpanHandler#doClose(CefBrowser)}. IsReadyToBeClosed() can be used to detect mandatory browser close events when customizing close behavior on the browser process UI thread.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Returns {@code true} if the browser is ready to be closed, meaning that the close has already been initiated and that JavaScript unload handlers have already executed or should be ignored. This can be used from a top-level window close handler such as {@link net.kurobako.cef4j.gen.views.CefWindowDelegate#canClose(CefWindow)} (or platform-specific equivalent) to distringuish between potentially cancelable browser close events (like the user clicking the top-level window close button before browser close has started) and mandatory browser close events (like JavaScript `window.close()` or after browser close has started in response to [Try]CloseBrowser()). Not completing the browser close for mandatory close events (when this method returns {@code true}) will leave the browser in a partially closed state that interferes with proper functioning. See CloseBrowser() documentation for additional usage information. This method must be called on the browser process UI thread.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Get the default zoom level. This value will be 0.0 by default but can be configured. This method can only be called on the UI thread.
     * <p>Definition generated from cef_browser_capi.h
     ...
     * <p>Definition generated from cef_browser_capi.h
     * <pre>void (CEF_CALLBACK* run_file_dialog)(struct _cef_browser_host_t* self, cef_file_dialog_mode_t mode, const cef_string_t* title, const cef_string_t* default_file_path, cef_string_list_t accept_filters, struct _cef_run_file_dialog_callback_t* callback);</pre>
     *
    ...
    /**
     * Print the current browser contents to the PDF file specified by {@code path} and execute {@code callback} on completion. The caller is responsible for deleting {@code path} when done. For PDF printing to work on Linux you must implement the {@link net.kurobako.cef4j.gen.CefPrintHandler#getPdfPaperSize(CefBrowser, int)} method.
     * <p>Definition generated from cef_browser_capi.h
     * <pre>void (CEF_CALLBACK* print_to_pdf)(struct _cef_browser_host_t* self, const cef_string_t* path, const struct _cef_pdf_print_settings_t* settings, struct _cef_pdf_print_callback_t* callback);</pre>
     *
    ...
    /**
     * Search for {@code searchText}. {@code forward} indicates whether to search forward or backward within the page. {@code matchCase} indicates whether the search should be case-sensitive. {@code findNext} indicates whether this is the first request or a follow-up. The search will be restarted if {@code searchText} or {@code matchCase} change. The search will be stopped if {@code searchText} is empty. The CefFindHandler instance, if any, returned via {@link net.kurobako.cef4j.gen.CefClient#getFindHandler()} will be called to report find results.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Open developer tools (DevTools) in its own browser. The DevTools browser will remain associated with this browser. If the DevTools browser is already open then it will be focused, in which case the {@code windowInfo}, {@code client} and {@code settings} parameters will be ignored. If {@code inspect_element_at} is non-empty then the element at the specified (x,y) location will be inspected. The {@code windowInfo} parameter will be ignored if this browser is wrapped in a CefBrowserView.
     * <p>Definition generated from cef_browser_capi.h
     * <pre>void (CEF_CALLBACK* show_dev_tools)(struct _cef_browser_host_t* self, const struct _cef_window_info_t* windowInfo, struct _cef_client_t* client, const struct _cef_browser_settings_t* settings, const cef_point_t* inspect_element_at);</pre>
     *
     ...
     * <p>Definition generated from cef_browser_capi.h
     * <pre>int (CEF_CALLBACK* execute_dev_tools_method)(struct _cef_browser_host_t* self, int message_id, const cef_string_t* method, struct _cef_dictionary_value_t* params);</pre>
     *
    ...
    /**
     * Notify the browser that the widget has been resized. The browser will first call {@link net.kurobako.cef4j.gen.CefRenderHandler#getViewRect(CefBrowser, CefRect.Mutable)} to get the new size and then call {@link net.kurobako.cef4j.gen.CefRenderHandler#onPaint(CefBrowser, CefPaintElementType, long, CefRect[], java.nio.ByteBuffer, int, int)} asynchronously with the updated regions. This method is only used when window rendering is disabled.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Notify the browser that it has been hidden or shown. Layouting and {@link net.kurobako.cef4j.gen.CefRenderHandler#onPaint(CefBrowser, CefPaintElementType, long, CefRect[], java.nio.ByteBuffer, int, int)} notification will stop when the browser is hidden. This method is only used when window rendering is disabled.
     * <p>Definition generated from cef_browser_capi.h
     ...
     * <p>
     * With windowless rendering the browser will call {@link net.kurobako.cef4j.gen.CefRenderHandler#getScreenInfo(CefBrowser, CefScreenInfo.Mutable)}, {@link net.kurobako.cef4j.gen.CefRenderHandler#getRootScreenRect(CefBrowser, CefRect.Mutable)} and {@link net.kurobako.cef4j.gen.CefRenderHandler#getViewRect(CefBrowser, CefRect.Mutable)}. This simulates moving or resizing the root window in the current display, moving the root window from one display to another, or changing the properties of the current display.
     * <p>
     * With windowed rendering the browser will call {@link net.kurobako.cef4j.gen.CefDisplayHandler#getRootWindowScreenRect(CefBrowser, CefRect.Mutable)} and use the associated display properties.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Invalidate the view. The browser will call {@link net.kurobako.cef4j.gen.CefRenderHandler#onPaint(CefBrowser, CefPaintElementType, long, CefRect[], java.nio.ByteBuffer, int, int)} asynchronously. This method is only used when window rendering is disabled.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Issue a BeginFrame request to Chromium.  Only valid when net.kurobako.cef4j.gen.CefWindowInfo.externalBeginFrameEnabled() is set to {@code true}.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Returns the maximum rate in frames per second (fps) that {@link net.kurobako.cef4j.gen.CefRenderHandler#onPaint(CefBrowser, CefPaintElementType, long, CefRect[], java.nio.ByteBuffer, int, int)} will be called for a windowless browser. The actual fps may be lower if the browser cannot generate frames at the requested rate. The minimum value is 1 and the default value is 30. This method can only be called on the UI thread.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Begins a new composition or updates the existing composition. Blink has a special node (a composition node) that allows the input method to change text without affecting other DOM nodes. {@code text} is the optional text that will be inserted into the composition node. {@code underlines} is an optional set of ranges that will be underlined in the resulting text. {@code replacement_range} is an optional range of the existing text that will be replaced. {@code selection_range} is an optional range of the resulting text that will be selected after insertion or replacement. The {@code replacement_range} value is only used on OS X.
     * <p>
     ...
     * <p>Definition generated from cef_browser_capi.h
     * <pre>void (CEF_CALLBACK* ime_set_composition)(struct _cef_browser_host_t* self, const cef_string_t* text, size_t underlinesCount, cef_composition_underline_t const* underlines, const cef_range_t* replacement_range, const cef_range_t* selection_range);</pre>
     *
     ...
     * <p>Definition generated from cef_browser_capi.h
     * <pre>void (CEF_CALLBACK* ime_commit_text)(struct _cef_browser_host_t* self, const cef_string_t* text, const cef_range_t* replacement_range, int relative_cursor_pos);</pre>
     *
     * @param text may be null
     *
    ...
    /**
     * Call this method when the user drags the mouse into the web view (before calling DragTargetDragOver/DragTargetLeave/DragTargetDrop). {@code drag_data} should not contain file contents as this type of data is not allowed to be dragged into the web view. File contents can be removed using {@link net.kurobako.cef4j.gen.CefDragData#resetFileContents()} (for example, if {@code drag_data} comes from {@link net.kurobako.cef4j.gen.CefRenderHandler#startDragging(CefBrowser, CefDragData, CefDragOperationsMask, int, int)}). This method is only used when window rendering is disabled.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Enable notifications of auto resize via {@link net.kurobako.cef4j.gen.CefDisplayHandler#onAutoResize(CefBrowser, CefSize)}. Notifications are disabled by default. {@code min_size} and {@code max_size} define the range of allowed sizes.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Requests the renderer to exit browser fullscreen. In most cases exiting window fullscreen should also exit browser fullscreen. With Alloy style this method should be called in response to a user action such as clicking the green traffic light button on MacOS ({@link net.kurobako.cef4j.gen.views.CefWindowDelegate#onWindowFullscreenTransition(CefWindow, boolean)} callback) or pressing the "ESC" key ({@link net.kurobako.cef4j.gen.CefKeyboardHandler#onPreKeyEvent(CefBrowser, CefKeyEvent, long, int[])} callback). With Chrome style these standard exit actions are handled internally but new/additional user actions can use this method. Set {@code will_cause_resize} to {@code true} if exiting browser fullscreen will cause a view resize.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Returns {@code true} if the render process associated with this browser is currently unresponsive as indicated by a lack of input event processing for at least 15 seconds. To receive associated state change notifications and optionally handle an unresponsive render process implement {@link net.kurobako.cef4j.gen.CefRequestHandler#onRenderProcessUnresponsive(CefBrowser, CefUnresponsiveProcessCallback)}. This method can only be called on the UI thread.
     * <p>Definition generated from cef_browser_capi.h
    ...
    /**
     * Create a new browser using the window parameters specified by {@code windowInfo}. If {@code request_context} is empty the global request context will be used. This method can only be called on the browser process UI thread. The optional {@code extra_info} parameter provides an opportunity to specify extra information specific to the created browser that will be passed to {@link net.kurobako.cef4j.gen.CefRenderProcessHandler#onBrowserCreated(CefBrowser, CefDictionaryValue)} in the render process.
     * <p>Definition generated from cef_browser_capi.h
    ...
    final class NativePeer implements CefBrowserHost, AutoCloseable {
      ...
      public void setZoomLevel(double zoomLevel) {
        ...
        @Override
      public void runFileDialog(@Nonnull CefFileDialogMode mode, @Nullable String title, @Nullable String defaultFilePath, @Nullable List<String> acceptFilters, @Nullable CefRunFileDialogCallback callback) {
          checkNotClosed();
      ...
      public void startDownload(@Nullable String url) {
        ...
        @Override
      public void downloadImage(@Nullable String imageUrl, boolean isFavicon, int maxImageSize, boolean bypassCache, @Nullable CefDownloadImageCallback callback) {
          checkNotClosed();
      ...
      public void print() {
        ...
        @Override
      public void printToPdf(@Nullable String path, @Nonnull CefPdfPrintSettings settings, @Nullable CefPdfPrintCallback callback) {
          checkNotClosed();
      ...
      public void stopFinding(boolean clearSelection) {
        ...
        @Override
      public void showDevTools(@Nullable CefWindowInfo windowInfo, @Nullable CefClient client, @Nullable CefBrowserSettings settings, @Nullable CefPoint inspectElementAt) {
          checkNotClosed();
      ...
      public void sendKeyEvent(@Nonnull CefKeyEvent event) {
        ...
        @Override
      public void sendMouseClickEvent(@Nonnull CefMouseEvent event, @Nonnull CefMouseButtonType type, boolean mouseUp, int clickCount) {
          checkNotClosed();
      ...
      public void setWindowlessFrameRate(int frameRate) {
        ...
        @Override
      public void imeSetComposition(@Nullable String text, long underlinesCount, @Nullable CefCompositionUnderline[] underlines, @Nonnull CefRange replacementRange, @Nonnull CefRange selectionRange) {
          checkNotClosed();
      ...
      public void imeCancelComposition() {
        ...
        @Override
      public void dragTargetDragEnter(@Nullable CefDragData dragData, @Nonnull CefMouseEvent event, @Nonnull CefDragOperationsMask allowedOps) {
          checkNotClosed();
