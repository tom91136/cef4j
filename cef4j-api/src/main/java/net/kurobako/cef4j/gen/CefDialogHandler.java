...
public interface CefDialogHandler extends CefClientHandler {
    ...
    /**
     * Called to run a file chooser dialog. {@code mode} represents the type of dialog to display. {@code title} to the
     * title to be used for the dialog and may be empty to show the default title ("Open" or "Save" depending on the
     * mode). {@code default_file_path} is the path with optional directory and/or file name component that should be
     * initially selected in the dialog. {@code accept_filters} are used to restrict the selectable file types and may
     * be any combination of valid lower-cased MIME types (e.g. "text/*" or "image/*") and individual file extensions
     * (e.g. ".txt" or ".png"). {@code accept_extensions} provides the semicolon-delimited expansion of MIME types to
     * file extensions (if known, or empty string otherwise). {@code accept_descriptions} provides the descriptions for
     * MIME types (if known, or empty string otherwise). For example, the "image/*" mime type might have extensions
     * ".png;.jpg;.bmp;..." and description "Image Files". {@code accept_filters}, {@code accept_extensions} and
     * {@code accept_descriptions} will all be the same size. To display a custom dialog return {@code true} and execute
     * {@code callback} either inline or at a later time. To display the default dialog return {@code false}. If this
     * method returns {@code false} it may be called an additional time for the same dialog (both before and after MIME
     * type expansion).
     *
     * <p>Definition generated from cef_dialog_handler_capi.h
     *
     ...
     * @see <a
     *     href="https://cef-builds.spotifycdn.com/docs/150.0/cef__dialog__handler_8h.html">cef_dialog_handler.h:74</a>
     */
