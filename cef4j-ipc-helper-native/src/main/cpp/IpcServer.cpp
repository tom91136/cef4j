...
std::string makeInprocAddr() {
    ...
    char buf[64];
    std::snprintf(buf, sizeof(buf), "inproc://cef4j-helper-wake-%lld-%llu",
...
bool IpcServer::bind(const std::string& addr) {
    ...
    if (zmq_bind(mainSock_, addr.c_str()) != 0) {
        std::fprintf(stderr, "[cef4j-helper] zmq_bind(%s) failed: %s\n", addr.c_str(), zmq_strerror(zmq_errno()));
        return false;
    }
    ...
    if (zmq_getsockopt(mainSock_, ZMQ_LAST_ENDPOINT, resolved, &resolvedLen) != 0) {
        std::fprintf(stderr, "[cef4j-helper] failed to read ZMQ_LAST_ENDPOINT: %s\n", zmq_strerror(zmq_errno()));
        return false;
    }
...
void IpcServer::start(FrameHandler handler) {
...
bool IpcServer::send(Kind kind, std::uint8_t flags, std::int32_t corrId, std::int32_t messageId,
...
void IpcServer::drainOutbound() {
    ...
    for (auto& frame : batch) {
        if (zmq_send(mainSock_, frame.data(), frame.size(), 0) < 0) {
            std::fprintf(stderr, "[cef4j-helper] zmq_send failed: %s\n", zmq_strerror(zmq_errno()));
            return;
        }
    }
}
