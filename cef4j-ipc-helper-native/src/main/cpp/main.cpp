// cef4j-helper: CEF host process driven over a ZMQ PAIR socket.
//
...
#include "CreateBrowserRequest.h"
#include "EvaluateJavascriptRequest.h"
#include "EvaluateJavascriptResponse.h"
#include "JsFunctionCallEvent.h"
#include "RegisterJsFunctionRequest.h"
#include "RegisterJsFunctionResponse.h"
#include "V8ExecuteFunctionRequest.h"
#include "V8ExecuteFunctionResponse.h"
#include "V8GetArrayLengthRequest.h"
#include "V8GetArrayLengthResponse.h"
#include "V8GetKeysRequest.h"
#include "V8GetKeysResponse.h"
#include "V8GetPropertyRequest.h"
#include "V8GetPropertyResponse.h"
#include "V8GetStringValueRequest.h"
#include "V8GetStringValueResponse.h"
#include "V8GetValueByIndexRequest.h"
#include "V8GetValueByIndexResponse.h"
#include "V8HasPropertyRequest.h"
#include "V8HasPropertyResponse.h"
#include "V8ReleaseHandleRequest.h"
#include "V8ReleaseHandleResponse.h"
#include "V8SetPropertyRequest.h"
#include "V8SetPropertyResponse.h"
#include "OsrPaintEvent.h"
#include "ReleaseHandleRequest.h"
#include "RendererReleaseHandleRequest.h"
#include "RendererReleaseHandleResponse.h"
#include "SetViewportSizeRequest.h"
#include "SetViewportSizeResponse.h"
#include "TriggerInterceptRequest.h"
#include "TriggerInterceptResponse.h"
#include "V8ContextCreatedEvent.h"
...
// matching AST event and sends it via gendisp::tables + IpcServer. Helper hands them out from CefClient
...
// Helper→JVM sync callback waiter; lives in InterceptRegistry.h's process-wide singleton so generated
...
// `gendisp::tables::frame`, etc.). The helper just registers handles it mints itself (currently the initial
...
// Shrink-to-fit hysteresis: if the current shm is more than this multiple of the actual paint size, we
// reallocate down. Picked at 4x to absorb common resize oscillations (e.g. 800x600 ↔ 1024x768 = 1.6x area
// difference) without rotating the shm name on every layout pass. Combined with a minimum size below this
// covers the "resize from huge to tiny" case without creating a shrink loop.
static constexpr std::size_t kOsrShrinkRatio = 4;
// Floor on shm size to avoid micro-allocations when the viewport is briefly 1x1 during layout. 256x256 BGRA
// = 256 KB; smaller paints just leave the tail unused like before.
static constexpr std::size_t kOsrMinBytes = 256 * 256 * 4;

// Per-browser OSR shm. Keyed by cef_browser_t* identity so the helper can find the right buffer in on_paint
...
struct RenderHandler : cef_render_handler_t {
    ...
    RenderHandler() : cef_render_handler_t{} {
        ...
        on_paint = [](cef_render_handler_t* /*self*/, cef_browser_t* browser,
            ...
            // Lazy-allocate per-browser shm. Helper-side handle lookup: tables::browser stores +1 retains, so
            ...
            // Pick the shm size we want for this paint: enough to hold byteCount, never below kOsrMinBytes,
            ...
            // the JVM-side MmapFrameTransport sees a new shm name and re-mmaps; without the bump the JVM would
...
// helper binary is the same executable for browser and renderer subprocess; address spaces are independent
...
static void onIpcFrame(const Header& h, std::vector<std::uint8_t>&& payload) {
    ...
    // Helper→JVM intercepts wake up via Kind::InterceptResponse on the IpcServer worker thread; we hand
    ...
    switch (h.messageId) {
        case kMsgReleaseHandle: {
            // Drops the helper-side retain for `kind=cef_X_t`'s `handle`. The dispatcher's generated switch
        ...
        case kMsgSetViewportSize: {
            ...
            cef_post_task(TID_UI, new gendisp::LambdaTask([browser]() {
                ...
                if (host) {
                    // Mirrors cef4j-osr-jfx's requestViewRefresh: notify_screen_info_changed forces CEF to
        ...
        default: {
            ...
            // globals in `gendisp::tables`; the helper only seeds `tables::browser` after the initial browser
            // is created.
...
static std::string parseBindArg(int argc, char* argv[]) {
    for (int i = 1; i + 1 < argc; ++i) {
        if (std::strcmp(argv[i], "--bind") == 0) return argv[i + 1];
    }
...
int main(int argc, char* argv[]) {
    ...
    std::string bindAddr = parseBindArg(argc, argv);
    ...
    // (~/.config/cef_user_data on Linux); without overriding, two helper instances racing for
    // browser creation can deadlock on the singleton lock. Each test spawns a fresh helper, so
    // a per-pid temp dir gives every instance its own state. Caller can override via
    // CEF4J_HELPER_CACHE_DIR if they want persistent caches.
    {
        std::string cacheDir;
        if (const char* override = std::getenv("CEF4J_HELPER_CACHE_DIR")) {
            cacheDir = override;
        } else {
            const char* tmpDir = std::getenv("TMPDIR");
            if (!tmpDir || !*tmpDir) tmpDir = "/tmp";
            cacheDir = std::string{tmpDir} + "/cef4j-helper-" + std::to_string(::getpid());
        }
    ...
    if (const char* resDir = std::getenv("CEF_RESOURCES_DIR")) {
        ScopedCefString resPath(std::string{resDir});
        cef_string_set(resPath.get()->str, resPath.get()->length, &settings.resources_dir_path, 1);
        std::string localesPath = std::string{resDir} + "/locales";
    ...
    if (!cef_initialize(&args, &settings, browserApp, nullptr)) {
        std::fprintf(stderr, "[cef4j-helper] cef_initialize failed\n");
        return 1;
    }

    IpcServer ipc;
    if (!ipc.bind(bindAddr)) {
        std::fprintf(stderr, "[cef4j-helper] failed to bind %s\n", bindAddr.c_str());
        cef_shutdown();
        return 1;
    }
    g_ipc                 = &ipc;
    genhandlers::g_ipc    = &ipc; // Generated forwarders fire events through this pointer.
    ...
    if (!cef_browser_host_create_browser(
        ...
        std::fprintf(stderr, "[cef4j-helper] cef_browser_host_create_browser failed\n");
        ipc.stop();
        cef_shutdown();
        return 1;
    }
