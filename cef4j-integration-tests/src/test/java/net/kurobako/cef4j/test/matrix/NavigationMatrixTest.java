 ...
 * {@code Frame.loadUrl} through the session and acks immediately on the server side, but the page swap is asynchronous;
...
class NavigationMatrixTest {
    ...
    void secondLoadReplacesDocument(BrowserBackend backend) throws Exception {
        ...
        try (BrowserSession s = backend.openSession(cfg)) {
            ...
            // Navigate to a second document. The future resolves on the server-side load ack; we then poll the DOM
