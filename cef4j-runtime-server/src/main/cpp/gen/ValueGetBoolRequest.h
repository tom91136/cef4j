...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_BOOL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_BOOL_REQUEST_H_
...
struct ValueGetBoolRequest {
    static constexpr int32_t kMessageId = 74786766;
    ...
    /** Reads a ValueGetBoolRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueGetBoolRequest decode(const std::uint8_t* src, std::size_t len) {
        ValueGetBoolRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueGetBoolRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ValueGetBoolRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_BOOL_REQUEST_H_
