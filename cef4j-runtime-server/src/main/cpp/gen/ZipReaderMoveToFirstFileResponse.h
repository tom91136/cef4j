...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_MOVE_TO_FIRST_FILE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_MOVE_TO_FIRST_FILE_RESPONSE_H_
...
struct ZipReaderMoveToFirstFileResponse {
    ...
    /** Reads a ZipReaderMoveToFirstFileResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ZipReaderMoveToFirstFileResponse decode(const std::uint8_t* src, std::size_t len) {
        ZipReaderMoveToFirstFileResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ZipReaderMoveToFirstFileResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ZipReaderMoveToFirstFileResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_MOVE_TO_FIRST_FILE_RESPONSE_H_
