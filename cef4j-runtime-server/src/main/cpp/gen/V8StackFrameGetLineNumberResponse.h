...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_FRAME_GET_LINE_NUMBER_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_FRAME_GET_LINE_NUMBER_RESPONSE_H_
...
struct V8StackFrameGetLineNumberResponse {
    ...
    /** Reads a V8StackFrameGetLineNumberResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static V8StackFrameGetLineNumberResponse decode(const std::uint8_t* src, std::size_t len) {
        V8StackFrameGetLineNumberResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated V8StackFrameGetLineNumberResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing V8StackFrameGetLineNumberResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_FRAME_GET_LINE_NUMBER_RESPONSE_H_
