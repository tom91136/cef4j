...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_IS_SAME_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_IS_SAME_RESPONSE_H_
...
struct OverlayControllerIsSameResponse {
    ...
    /** Reads a OverlayControllerIsSameResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static OverlayControllerIsSameResponse decode(const std::uint8_t* src, std::size_t len) {
        OverlayControllerIsSameResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated OverlayControllerIsSameResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing OverlayControllerIsSameResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_IS_SAME_RESPONSE_H_
