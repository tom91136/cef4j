...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GET_MAIN_FRAME_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GET_MAIN_FRAME_REQUEST_H_
...
struct BrowserGetMainFrameRequest {
    static constexpr int32_t kMessageId = 154419806;
    ...
    /** Reads a BrowserGetMainFrameRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserGetMainFrameRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserGetMainFrameRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserGetMainFrameRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserGetMainFrameRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GET_MAIN_FRAME_REQUEST_H_
