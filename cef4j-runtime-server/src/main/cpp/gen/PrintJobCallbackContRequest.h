...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_JOB_CALLBACK_CONT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_JOB_CALLBACK_CONT_REQUEST_H_
...
struct PrintJobCallbackContRequest {
    static constexpr int32_t kMessageId = 2097998570;
    ...
    /** Reads a PrintJobCallbackContRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PrintJobCallbackContRequest decode(const std::uint8_t* src, std::size_t len) {
        PrintJobCallbackContRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PrintJobCallbackContRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PrintJobCallbackContRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_JOB_CALLBACK_CONT_REQUEST_H_
