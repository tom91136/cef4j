...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_PARENT_VIEW_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_PARENT_VIEW_REQUEST_H_
...
struct ViewGetParentViewRequest {
    static constexpr int32_t kMessageId = 357195479;
    ...
    /** Reads a ViewGetParentViewRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewGetParentViewRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewGetParentViewRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewGetParentViewRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewGetParentViewRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_PARENT_VIEW_REQUEST_H_
