...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_HAS_ATTRIBUTES_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_HAS_ATTRIBUTES_REQUEST_H_
...
struct XmlReaderHasAttributesRequest {
    static constexpr int32_t kMessageId = 652321295;
    ...
    /** Reads a XmlReaderHasAttributesRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static XmlReaderHasAttributesRequest decode(const std::uint8_t* src, std::size_t len) {
        XmlReaderHasAttributesRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated XmlReaderHasAttributesRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing XmlReaderHasAttributesRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_HAS_ATTRIBUTES_REQUEST_H_
