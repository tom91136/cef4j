...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_SOURCE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_SOURCE_REQUEST_H_
...
struct FrameGetSourceRequest {
    static constexpr int32_t kMessageId = 1895174678;
    ...
    /** Reads a FrameGetSourceRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameGetSourceRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameGetSourceRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameGetSourceRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameGetSourceRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_SOURCE_REQUEST_H_
