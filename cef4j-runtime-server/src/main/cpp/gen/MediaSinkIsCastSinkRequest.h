...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_IS_CAST_SINK_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_IS_CAST_SINK_REQUEST_H_
...
struct MediaSinkIsCastSinkRequest {
    static constexpr int32_t kMessageId = 51378288;
    ...
    /** Reads a MediaSinkIsCastSinkRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MediaSinkIsCastSinkRequest decode(const std::uint8_t* src, std::size_t len) {
        MediaSinkIsCastSinkRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MediaSinkIsCastSinkRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MediaSinkIsCastSinkRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_IS_CAST_SINK_REQUEST_H_
