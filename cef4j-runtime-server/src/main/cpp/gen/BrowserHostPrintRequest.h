...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_PRINT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_PRINT_REQUEST_H_
...
struct BrowserHostPrintRequest {
    static constexpr int32_t kMessageId = 98773846;
    ...
    /** Reads a BrowserHostPrintRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHostPrintRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserHostPrintRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHostPrintRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserHostPrintRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_PRINT_REQUEST_H_
