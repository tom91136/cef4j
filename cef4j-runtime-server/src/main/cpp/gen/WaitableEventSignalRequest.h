...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WAITABLE_EVENT_SIGNAL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WAITABLE_EVENT_SIGNAL_REQUEST_H_
...
struct WaitableEventSignalRequest {
    static constexpr int32_t kMessageId = 811592941;
    ...
    /** Reads a WaitableEventSignalRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WaitableEventSignalRequest decode(const std::uint8_t* src, std::size_t len) {
        WaitableEventSignalRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WaitableEventSignalRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WaitableEventSignalRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WAITABLE_EVENT_SIGNAL_REQUEST_H_
