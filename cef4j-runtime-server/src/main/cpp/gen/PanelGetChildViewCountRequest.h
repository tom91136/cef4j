...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_GET_CHILD_VIEW_COUNT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_GET_CHILD_VIEW_COUNT_REQUEST_H_
...
struct PanelGetChildViewCountRequest {
    ...
    /** Reads a PanelGetChildViewCountRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PanelGetChildViewCountRequest decode(const std::uint8_t* src, std::size_t len) {
        PanelGetChildViewCountRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PanelGetChildViewCountRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PanelGetChildViewCountRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_GET_CHILD_VIEW_COUNT_REQUEST_H_
