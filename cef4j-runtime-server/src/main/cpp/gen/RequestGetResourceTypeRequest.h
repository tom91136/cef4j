...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_RESOURCE_TYPE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_RESOURCE_TYPE_REQUEST_H_
...
struct RequestGetResourceTypeRequest {
    static constexpr int32_t kMessageId = 1649776071;
    ...
    /** Reads a RequestGetResourceTypeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestGetResourceTypeRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestGetResourceTypeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestGetResourceTypeRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestGetResourceTypeRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_RESOURCE_TYPE_REQUEST_H_
