...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_AS_MENU_BUTTON_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_AS_MENU_BUTTON_RESPONSE_H_
...
struct LabelButtonAsMenuButtonResponse {
    ...
    /** Reads a LabelButtonAsMenuButtonResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static LabelButtonAsMenuButtonResponse decode(const std::uint8_t* src, std::size_t len) {
        LabelButtonAsMenuButtonResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated LabelButtonAsMenuButtonResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing LabelButtonAsMenuButtonResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_AS_MENU_BUTTON_RESPONSE_H_
