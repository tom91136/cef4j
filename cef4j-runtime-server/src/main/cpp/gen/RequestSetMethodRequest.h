...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_SET_METHOD_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_SET_METHOD_REQUEST_H_
...
struct RequestSetMethodRequest {
    static constexpr int32_t kMessageId = 2031204869;
    ...
    /** Reads a RequestSetMethodRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestSetMethodRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestSetMethodRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestSetMethodRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing RequestSetMethodRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_SET_METHOD_REQUEST_H_
