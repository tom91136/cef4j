...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_AUTH_CALLBACK_CANCEL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_AUTH_CALLBACK_CANCEL_REQUEST_H_
...
struct AuthCallbackCancelRequest {
    static constexpr int32_t kMessageId = 1124007829;
    ...
    /** Reads a AuthCallbackCancelRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static AuthCallbackCancelRequest decode(const std::uint8_t* src, std::size_t len) {
        AuthCallbackCancelRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated AuthCallbackCancelRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing AuthCallbackCancelRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_AUTH_CALLBACK_CANCEL_REQUEST_H_
