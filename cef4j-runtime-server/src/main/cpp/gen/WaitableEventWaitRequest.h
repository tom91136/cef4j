...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WAITABLE_EVENT_WAIT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WAITABLE_EVENT_WAIT_REQUEST_H_
...
struct WaitableEventWaitRequest {
    static constexpr int32_t kMessageId = 879459250;
    ...
    /** Reads a WaitableEventWaitRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WaitableEventWaitRequest decode(const std::uint8_t* src, std::size_t len) {
        WaitableEventWaitRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WaitableEventWaitRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WaitableEventWaitRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WAITABLE_EVENT_WAIT_REQUEST_H_
