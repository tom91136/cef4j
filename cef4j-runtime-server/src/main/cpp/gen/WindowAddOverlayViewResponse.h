...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_ADD_OVERLAY_VIEW_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_ADD_OVERLAY_VIEW_RESPONSE_H_
...
struct WindowAddOverlayViewResponse {
    ...
    /** Reads a WindowAddOverlayViewResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowAddOverlayViewResponse decode(const std::uint8_t* src, std::size_t len) {
        WindowAddOverlayViewResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowAddOverlayViewResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowAddOverlayViewResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_ADD_OVERLAY_VIEW_RESPONSE_H_
