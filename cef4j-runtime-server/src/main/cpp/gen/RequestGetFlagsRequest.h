...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_FLAGS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_FLAGS_REQUEST_H_
...
struct RequestGetFlagsRequest {
    static constexpr int32_t kMessageId = 877941355;
    ...
    /** Reads a RequestGetFlagsRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestGetFlagsRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestGetFlagsRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestGetFlagsRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestGetFlagsRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_FLAGS_REQUEST_H_
