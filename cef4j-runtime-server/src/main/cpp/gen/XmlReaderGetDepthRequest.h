...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_GET_DEPTH_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_GET_DEPTH_REQUEST_H_
...
struct XmlReaderGetDepthRequest {
    static constexpr int32_t kMessageId = 1859900746;
    ...
    /** Reads a XmlReaderGetDepthRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static XmlReaderGetDepthRequest decode(const std::uint8_t* src, std::size_t len) {
        XmlReaderGetDepthRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated XmlReaderGetDepthRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing XmlReaderGetDepthRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_GET_DEPTH_REQUEST_H_
