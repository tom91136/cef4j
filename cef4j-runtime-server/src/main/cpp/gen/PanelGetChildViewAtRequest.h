...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_GET_CHILD_VIEW_AT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_GET_CHILD_VIEW_AT_REQUEST_H_
...
struct PanelGetChildViewAtRequest {
    static constexpr int32_t kMessageId = 1558043479;
    ...
    /** Reads a PanelGetChildViewAtRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PanelGetChildViewAtRequest decode(const std::uint8_t* src, std::size_t len) {
        PanelGetChildViewAtRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PanelGetChildViewAtRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PanelGetChildViewAtRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_GET_CHILD_VIEW_AT_REQUEST_H_
