...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOWNLOAD_ITEM_IS_PAUSED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOWNLOAD_ITEM_IS_PAUSED_REQUEST_H_
...
struct DownloadItemIsPausedRequest {
    static constexpr int32_t kMessageId = 1175188651;
    ...
    /** Reads a DownloadItemIsPausedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DownloadItemIsPausedRequest decode(const std::uint8_t* src, std::size_t len) {
        DownloadItemIsPausedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DownloadItemIsPausedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DownloadItemIsPausedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOWNLOAD_ITEM_IS_PAUSED_REQUEST_H_
