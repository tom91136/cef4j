...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_VALUE_GET_INT_VALUE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_VALUE_GET_INT_VALUE_RESPONSE_H_
...
struct V8ValueGetIntValueResponse {
    ...
    /** Reads a V8ValueGetIntValueResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static V8ValueGetIntValueResponse decode(const std::uint8_t* src, std::size_t len) {
        V8ValueGetIntValueResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated V8ValueGetIntValueResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing V8ValueGetIntValueResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_VALUE_GET_INT_VALUE_RESPONSE_H_
