...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_GET_MEDIA_ROUTER_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_GET_MEDIA_ROUTER_REQUEST_H_
...
struct RequestContextGetMediaRouterRequest {
    static constexpr int32_t kMessageId = 1960226003;
    ...
    /** Reads a RequestContextGetMediaRouterRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestContextGetMediaRouterRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestContextGetMediaRouterRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestContextGetMediaRouterRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestContextGetMediaRouterRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_GET_MEDIA_ROUTER_REQUEST_H_
