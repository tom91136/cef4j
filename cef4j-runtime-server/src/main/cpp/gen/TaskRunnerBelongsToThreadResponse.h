...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TASK_RUNNER_BELONGS_TO_THREAD_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TASK_RUNNER_BELONGS_TO_THREAD_RESPONSE_H_
...
struct TaskRunnerBelongsToThreadResponse {
    ...
    /** Reads a TaskRunnerBelongsToThreadResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static TaskRunnerBelongsToThreadResponse decode(const std::uint8_t* src, std::size_t len) {
        TaskRunnerBelongsToThreadResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated TaskRunnerBelongsToThreadResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing TaskRunnerBelongsToThreadResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TASK_RUNNER_BELONGS_TO_THREAD_RESPONSE_H_
