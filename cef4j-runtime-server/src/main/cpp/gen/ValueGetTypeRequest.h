...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_TYPE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_TYPE_REQUEST_H_
...
struct ValueGetTypeRequest {
    static constexpr int32_t kMessageId = 1101625569;
    ...
    /** Reads a ValueGetTypeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueGetTypeRequest decode(const std::uint8_t* src, std::size_t len) {
        ValueGetTypeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueGetTypeRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ValueGetTypeRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_TYPE_REQUEST_H_
