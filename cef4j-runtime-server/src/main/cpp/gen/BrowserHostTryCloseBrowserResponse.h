...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_TRY_CLOSE_BROWSER_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_TRY_CLOSE_BROWSER_RESPONSE_H_
...
struct BrowserHostTryCloseBrowserResponse {
    ...
    /** Reads a BrowserHostTryCloseBrowserResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHostTryCloseBrowserResponse decode(const std::uint8_t* src, std::size_t len) {
        BrowserHostTryCloseBrowserResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHostTryCloseBrowserResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserHostTryCloseBrowserResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_TRY_CLOSE_BROWSER_RESPONSE_H_
