...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_AS_WINDOW_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_AS_WINDOW_REQUEST_H_
...
struct PanelAsWindowRequest {
    static constexpr int32_t kMessageId = 263156860;
    ...
    /** Reads a PanelAsWindowRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PanelAsWindowRequest decode(const std::uint8_t* src, std::size_t len) {
        PanelAsWindowRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PanelAsWindowRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PanelAsWindowRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_AS_WINDOW_REQUEST_H_
