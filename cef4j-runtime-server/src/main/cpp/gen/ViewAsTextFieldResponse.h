...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_TEXT_FIELD_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_TEXT_FIELD_RESPONSE_H_
...
struct ViewAsTextFieldResponse {
    ...
    /** Reads a ViewAsTextFieldResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewAsTextFieldResponse decode(const std::uint8_t* src, std::size_t len) {
        ViewAsTextFieldResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewAsTextFieldResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewAsTextFieldResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_TEXT_FIELD_RESPONSE_H_
