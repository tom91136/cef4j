...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HAS_DOCUMENT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HAS_DOCUMENT_REQUEST_H_
...
struct BrowserHasDocumentRequest {
    static constexpr int32_t kMessageId = 1193299213;
    ...
    /** Reads a BrowserHasDocumentRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHasDocumentRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserHasDocumentRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHasDocumentRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserHasDocumentRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HAS_DOCUMENT_REQUEST_H_
