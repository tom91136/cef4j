...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_SET_INT_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_SET_INT_RESPONSE_H_
...
struct DictionaryValueSetIntResponse {
    ...
    /** Reads a DictionaryValueSetIntResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DictionaryValueSetIntResponse decode(const std::uint8_t* src, std::size_t len) {
        DictionaryValueSetIntResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DictionaryValueSetIntResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DictionaryValueSetIntResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_SET_INT_RESPONSE_H_
