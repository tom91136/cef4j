...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_SEND_DEV_TOOLS_MESSAGE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_SEND_DEV_TOOLS_MESSAGE_RESPONSE_H_
...
struct BrowserHostSendDevToolsMessageResponse {
    ...
    /** Reads a BrowserHostSendDevToolsMessageResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHostSendDevToolsMessageResponse decode(const std::uint8_t* src, std::size_t len) {
        BrowserHostSendDevToolsMessageResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHostSendDevToolsMessageResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserHostSendDevToolsMessageResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_SEND_DEV_TOOLS_MESSAGE_RESPONSE_H_
