...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_REFERRER_POLICY_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_REFERRER_POLICY_RESPONSE_H_
...
struct RequestGetReferrerPolicyResponse {
    ...
    /** Reads a RequestGetReferrerPolicyResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestGetReferrerPolicyResponse decode(const std::uint8_t* src, std::size_t len) {
        RequestGetReferrerPolicyResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestGetReferrerPolicyResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestGetReferrerPolicyResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_REFERRER_POLICY_RESPONSE_H_
