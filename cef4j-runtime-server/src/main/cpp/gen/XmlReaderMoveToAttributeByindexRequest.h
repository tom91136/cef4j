...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_MOVE_TO_ATTRIBUTE_BYINDEX_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_MOVE_TO_ATTRIBUTE_BYINDEX_REQUEST_H_
...
struct XmlReaderMoveToAttributeByindexRequest {
    static constexpr int32_t kMessageId = 564435289;
    ...
    /** Reads a XmlReaderMoveToAttributeByindexRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static XmlReaderMoveToAttributeByindexRequest decode(const std::uint8_t* src, std::size_t len) {
        XmlReaderMoveToAttributeByindexRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated XmlReaderMoveToAttributeByindexRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing XmlReaderMoveToAttributeByindexRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_MOVE_TO_ATTRIBUTE_BYINDEX_REQUEST_H_
