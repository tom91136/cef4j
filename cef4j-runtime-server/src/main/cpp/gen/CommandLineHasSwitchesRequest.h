...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_HAS_SWITCHES_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_HAS_SWITCHES_REQUEST_H_
...
struct CommandLineHasSwitchesRequest {
    static constexpr int32_t kMessageId = 970034919;
    ...
    /** Reads a CommandLineHasSwitchesRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CommandLineHasSwitchesRequest decode(const std::uint8_t* src, std::size_t len) {
        CommandLineHasSwitchesRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CommandLineHasSwitchesRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CommandLineHasSwitchesRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_HAS_SWITCHES_REQUEST_H_
