...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SCROLL_VIEW_GET_VISIBLE_CONTENT_RECT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SCROLL_VIEW_GET_VISIBLE_CONTENT_RECT_REQUEST_H_
...
struct ScrollViewGetVisibleContentRectRequest {
    ...
    /** Reads a ScrollViewGetVisibleContentRectRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ScrollViewGetVisibleContentRectRequest decode(const std::uint8_t* src, std::size_t len) {
        ScrollViewGetVisibleContentRectRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ScrollViewGetVisibleContentRectRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ScrollViewGetVisibleContentRectRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SCROLL_VIEW_GET_VISIBLE_CONTENT_RECT_REQUEST_H_
