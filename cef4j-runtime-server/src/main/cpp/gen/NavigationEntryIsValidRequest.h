...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_NAVIGATION_ENTRY_IS_VALID_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_NAVIGATION_ENTRY_IS_VALID_REQUEST_H_
...
struct NavigationEntryIsValidRequest {
    static constexpr int32_t kMessageId = 187401063;
    ...
    /** Reads a NavigationEntryIsValidRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static NavigationEntryIsValidRequest decode(const std::uint8_t* src, std::size_t len) {
        NavigationEntryIsValidRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated NavigationEntryIsValidRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing NavigationEntryIsValidRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_NAVIGATION_ENTRY_IS_VALID_REQUEST_H_
