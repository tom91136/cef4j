...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_GET_LOCAL_NAME_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_GET_LOCAL_NAME_REQUEST_H_
...
struct XmlReaderGetLocalNameRequest {
    ...
    /** Reads a XmlReaderGetLocalNameRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static XmlReaderGetLocalNameRequest decode(const std::uint8_t* src, std::size_t len) {
        XmlReaderGetLocalNameRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated XmlReaderGetLocalNameRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing XmlReaderGetLocalNameRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_GET_LOCAL_NAME_REQUEST_H_
