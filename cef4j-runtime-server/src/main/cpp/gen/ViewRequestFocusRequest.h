...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_REQUEST_FOCUS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_REQUEST_FOCUS_REQUEST_H_
...
struct ViewRequestFocusRequest {
    static constexpr int32_t kMessageId = 1905201877;
    ...
    /** Reads a ViewRequestFocusRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewRequestFocusRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewRequestFocusRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewRequestFocusRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewRequestFocusRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_REQUEST_FOCUS_REQUEST_H_
