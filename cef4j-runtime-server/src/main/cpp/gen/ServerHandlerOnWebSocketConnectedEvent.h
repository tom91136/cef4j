...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_HANDLER_ON_WEB_SOCKET_CONNECTED_EVENT_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_HANDLER_ON_WEB_SOCKET_CONNECTED_EVENT_H_
...
struct ServerHandlerOnWebSocketConnectedEvent {
    ...
    /** Reads a ServerHandlerOnWebSocketConnectedEvent from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ServerHandlerOnWebSocketConnectedEvent decode(const std::uint8_t* src, std::size_t len) {
        ServerHandlerOnWebSocketConnectedEvent out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ServerHandlerOnWebSocketConnectedEvent payload");
        };
        requireAvailable(4);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_HANDLER_ON_WEB_SOCKET_CONNECTED_EVENT_H_
