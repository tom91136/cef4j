...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DISPLAY_GET_ID_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DISPLAY_GET_ID_RESPONSE_H_
...
struct DisplayGetIdResponse {
    static constexpr int32_t kMessageId = 1884097299;
    ...
    /** Reads a DisplayGetIdResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DisplayGetIdResponse decode(const std::uint8_t* src, std::size_t len) {
        DisplayGetIdResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DisplayGetIdResponse payload");
        };
        requireAvailable(8);
        ...
        pos += 8;
        if (pos != len)
            throw std::invalid_argument("trailing DisplayGetIdResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DISPLAY_GET_ID_RESPONSE_H_
