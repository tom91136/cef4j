...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_ENABLED_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_ENABLED_RESPONSE_H_
...
struct ViewIsEnabledResponse {
    ...
    /** Reads a ViewIsEnabledResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewIsEnabledResponse decode(const std::uint8_t* src, std::size_t len) {
        ViewIsEnabledResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewIsEnabledResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewIsEnabledResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_ENABLED_RESPONSE_H_
