...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_HAS_SWITCH_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_HAS_SWITCH_REQUEST_H_
...
struct CommandLineHasSwitchRequest {
    static constexpr int32_t kMessageId = 967172106;
    ...
    /** Reads a CommandLineHasSwitchRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CommandLineHasSwitchRequest decode(const std::uint8_t* src, std::size_t len) {
        CommandLineHasSwitchRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CommandLineHasSwitchRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing CommandLineHasSwitchRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_HAS_SWITCH_REQUEST_H_
