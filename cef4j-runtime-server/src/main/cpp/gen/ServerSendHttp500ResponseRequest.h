...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_HTTP500_RESPONSE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_HTTP500_RESPONSE_REQUEST_H_
...
struct ServerSendHttp500ResponseRequest {
    ...
    /** Reads a ServerSendHttp500ResponseRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ServerSendHttp500ResponseRequest decode(const std::uint8_t* src, std::size_t len) {
        ServerSendHttp500ResponseRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ServerSendHttp500ResponseRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing ServerSendHttp500ResponseRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_HTTP500_RESPONSE_REQUEST_H_
