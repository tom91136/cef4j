...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_REMOVE_ELEMENTS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_REMOVE_ELEMENTS_REQUEST_H_
...
struct PostDataRemoveElementsRequest {
    static constexpr int32_t kMessageId = 966477676;
    ...
    /** Reads a PostDataRemoveElementsRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PostDataRemoveElementsRequest decode(const std::uint8_t* src, std::size_t len) {
        PostDataRemoveElementsRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PostDataRemoveElementsRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PostDataRemoveElementsRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_REMOVE_ELEMENTS_REQUEST_H_
