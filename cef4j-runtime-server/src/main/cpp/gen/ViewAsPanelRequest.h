...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_PANEL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_PANEL_REQUEST_H_
...
struct ViewAsPanelRequest {
    static constexpr int32_t kMessageId = 252082951;
    ...
    /** Reads a ViewAsPanelRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewAsPanelRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewAsPanelRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewAsPanelRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewAsPanelRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_PANEL_REQUEST_H_
