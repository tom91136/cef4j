...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_HAS_ACCELERATOR_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_HAS_ACCELERATOR_RESPONSE_H_
...
struct MenuModelHasAcceleratorResponse {
    ...
    /** Reads a MenuModelHasAcceleratorResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MenuModelHasAcceleratorResponse decode(const std::uint8_t* src, std::size_t len) {
        MenuModelHasAcceleratorResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MenuModelHasAcceleratorResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MenuModelHasAcceleratorResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_HAS_ACCELERATOR_RESPONSE_H_
