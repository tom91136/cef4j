...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_WINDOW_ICON_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_WINDOW_ICON_REQUEST_H_
...
struct WindowGetWindowIconRequest {
    static constexpr int32_t kMessageId = 1518992598;
    ...
    /** Reads a WindowGetWindowIconRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowGetWindowIconRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowGetWindowIconRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowGetWindowIconRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowGetWindowIconRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_WINDOW_ICON_REQUEST_H_
