...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_ALL_COOKIES_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_ALL_COOKIES_RESPONSE_H_
...
struct CookieManagerVisitAllCookiesResponse {
    ...
    /** Reads a CookieManagerVisitAllCookiesResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CookieManagerVisitAllCookiesResponse decode(const std::uint8_t* src, std::size_t len) {
        CookieManagerVisitAllCookiesResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CookieManagerVisitAllCookiesResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CookieManagerVisitAllCookiesResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_ALL_COOKIES_RESPONSE_H_
