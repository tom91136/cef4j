...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_CLOSE_ALL_CONNECTIONS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_CLOSE_ALL_CONNECTIONS_REQUEST_H_
...
struct RequestContextCloseAllConnectionsRequest {
    static constexpr int32_t kMessageId = 1082702399;
    ...
    /** Reads a RequestContextCloseAllConnectionsRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestContextCloseAllConnectionsRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestContextCloseAllConnectionsRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestContextCloseAllConnectionsRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestContextCloseAllConnectionsRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_CLOSE_ALL_CONNECTIONS_REQUEST_H_
