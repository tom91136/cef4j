...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_LIST_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_LIST_REQUEST_H_
...
struct ValueGetListRequest {
    static constexpr int32_t kMessageId = 1948802772;
    ...
    /** Reads a ValueGetListRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueGetListRequest decode(const std::uint8_t* src, std::size_t len) {
        ValueGetListRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueGetListRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ValueGetListRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_LIST_REQUEST_H_
