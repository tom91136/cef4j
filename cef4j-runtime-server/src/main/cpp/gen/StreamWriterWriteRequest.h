...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_WRITER_WRITE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_WRITER_WRITE_REQUEST_H_
...
struct StreamWriterWriteRequest {
    static constexpr int32_t kMessageId = 130572826;
    ...
    void encodeInto(std::uint8_t* dst) const noexcept {
        ...
        {
            std::uint32_t v = static_cast<std::uint32_t>(ptr.size());
            dst[pos + 0] = static_cast<std::uint8_t>(v & 0xFF);
            dst[pos + 1] = static_cast<std::uint8_t>((v >> 8) & 0xFF);
            dst[pos + 2] = static_cast<std::uint8_t>((v >> 16) & 0xFF);
            dst[pos + 3] = static_cast<std::uint8_t>((v >> 24) & 0xFF);
        ...
        if (!ptr.empty()) std::memcpy(dst + pos, ptr.data(), ptr.size());
        pos += ptr.size();
        {
    ...
    /** Reads a StreamWriterWriteRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static StreamWriterWriteRequest decode(const std::uint8_t* src, std::size_t len) {
        StreamWriterWriteRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated StreamWriterWriteRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 8;
        if (pos != len)
            throw std::invalid_argument("trailing StreamWriterWriteRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_WRITER_WRITE_REQUEST_H_
