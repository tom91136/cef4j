...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_GET_LAYOUT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_GET_LAYOUT_REQUEST_H_
...
struct PanelGetLayoutRequest {
    static constexpr int32_t kMessageId = 811142095;
    ...
    /** Reads a PanelGetLayoutRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PanelGetLayoutRequest decode(const std::uint8_t* src, std::size_t len) {
        PanelGetLayoutRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PanelGetLayoutRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PanelGetLayoutRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_GET_LAYOUT_REQUEST_H_
