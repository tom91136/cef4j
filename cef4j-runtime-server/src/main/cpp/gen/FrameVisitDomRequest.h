...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_VISIT_DOM_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_VISIT_DOM_REQUEST_H_
...
struct FrameVisitDomRequest {
    static constexpr int32_t kMessageId = 96501728;
    ...
    /** Reads a FrameVisitDomRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameVisitDomRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameVisitDomRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameVisitDomRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameVisitDomRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_VISIT_DOM_REQUEST_H_
