...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_INTERCEPTOR_SET_BYINDEX_EVENT_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_INTERCEPTOR_SET_BYINDEX_EVENT_H_
...
struct V8InterceptorSetByindexEvent {
    ...
    /** Reads a V8InterceptorSetByindexEvent from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static V8InterceptorSetByindexEvent decode(const std::uint8_t* src, std::size_t len) {
        V8InterceptorSetByindexEvent out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated V8InterceptorSetByindexEvent payload");
        };
        requireAvailable(4);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_INTERCEPTOR_SET_BYINDEX_EVENT_H_
