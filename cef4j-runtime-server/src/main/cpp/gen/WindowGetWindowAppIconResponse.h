...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_WINDOW_APP_ICON_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_WINDOW_APP_ICON_RESPONSE_H_
...
struct WindowGetWindowAppIconResponse {
    ...
    /** Reads a WindowGetWindowAppIconResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowGetWindowAppIconResponse decode(const std::uint8_t* src, std::size_t len) {
        WindowGetWindowAppIconResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowGetWindowAppIconResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowGetWindowAppIconResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_WINDOW_APP_ICON_RESPONSE_H_
