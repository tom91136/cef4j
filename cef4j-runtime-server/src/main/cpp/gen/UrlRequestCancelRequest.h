...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_CANCEL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_CANCEL_REQUEST_H_
...
struct UrlRequestCancelRequest {
    static constexpr int32_t kMessageId = 941827738;
    ...
    /** Reads a UrlRequestCancelRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static UrlRequestCancelRequest decode(const std::uint8_t* src, std::size_t len) {
        UrlRequestCancelRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated UrlRequestCancelRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing UrlRequestCancelRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_CANCEL_REQUEST_H_
