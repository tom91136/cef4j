...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_SET_SIZE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_SET_SIZE_REQUEST_H_
...
struct ViewSetSizeRequest {
    ...
    /** Reads a ViewSetSizeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewSetSizeRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewSetSizeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewSetSizeRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing ViewSetSizeRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_SET_SIZE_REQUEST_H_
