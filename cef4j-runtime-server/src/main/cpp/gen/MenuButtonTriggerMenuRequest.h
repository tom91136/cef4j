...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_BUTTON_TRIGGER_MENU_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_BUTTON_TRIGGER_MENU_REQUEST_H_
...
struct MenuButtonTriggerMenuRequest {
    static constexpr int32_t kMessageId = 720713994;
    ...
    /** Reads a MenuButtonTriggerMenuRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MenuButtonTriggerMenuRequest decode(const std::uint8_t* src, std::size_t len) {
        MenuButtonTriggerMenuRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MenuButtonTriggerMenuRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MenuButtonTriggerMenuRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_BUTTON_TRIGGER_MENU_REQUEST_H_
