...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_SET_INSETS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_SET_INSETS_REQUEST_H_
...
struct ViewSetInsetsRequest {
    static constexpr int32_t kMessageId = 1097397774;
    ...
    void encodeInto(std::uint8_t* dst) const noexcept {
        ...
        insets.encodeInto(dst + pos);
        pos += insets.encodedSize();
    }

    /** Reads a ViewSetInsetsRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewSetInsetsRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewSetInsetsRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewSetInsetsRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing ViewSetInsetsRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_SET_INSETS_REQUEST_H_
