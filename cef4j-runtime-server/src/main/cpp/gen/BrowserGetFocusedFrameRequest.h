...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GET_FOCUSED_FRAME_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GET_FOCUSED_FRAME_REQUEST_H_
...
struct BrowserGetFocusedFrameRequest {
    static constexpr int32_t kMessageId = 1112242189;
    ...
    /** Reads a BrowserGetFocusedFrameRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserGetFocusedFrameRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserGetFocusedFrameRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserGetFocusedFrameRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserGetFocusedFrameRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GET_FOCUSED_FRAME_REQUEST_H_
