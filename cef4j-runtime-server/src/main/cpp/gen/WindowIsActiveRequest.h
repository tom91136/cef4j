...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_ACTIVE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_ACTIVE_REQUEST_H_
...
struct WindowIsActiveRequest {
    static constexpr int32_t kMessageId = 224569187;
    ...
    /** Reads a WindowIsActiveRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowIsActiveRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowIsActiveRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowIsActiveRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowIsActiveRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_ACTIVE_REQUEST_H_
