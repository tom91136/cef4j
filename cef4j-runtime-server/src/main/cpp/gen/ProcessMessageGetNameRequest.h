...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_GET_NAME_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_GET_NAME_REQUEST_H_
...
struct ProcessMessageGetNameRequest {
    ...
    /** Reads a ProcessMessageGetNameRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ProcessMessageGetNameRequest decode(const std::uint8_t* src, std::size_t len) {
        ProcessMessageGetNameRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ProcessMessageGetNameRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ProcessMessageGetNameRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_GET_NAME_REQUEST_H_
