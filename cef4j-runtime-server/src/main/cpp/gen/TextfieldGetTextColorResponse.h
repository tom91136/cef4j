...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXTFIELD_GET_TEXT_COLOR_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXTFIELD_GET_TEXT_COLOR_RESPONSE_H_
...
struct TextfieldGetTextColorResponse {
    ...
    /** Reads a TextfieldGetTextColorResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static TextfieldGetTextColorResponse decode(const std::uint8_t* src, std::size_t len) {
        TextfieldGetTextColorResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated TextfieldGetTextColorResponse payload");
        };
        requireAvailable(4);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXTFIELD_GET_TEXT_COLOR_RESPONSE_H_
