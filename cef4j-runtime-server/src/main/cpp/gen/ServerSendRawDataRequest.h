...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_RAW_DATA_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_RAW_DATA_REQUEST_H_
...
struct ServerSendRawDataRequest {
    static constexpr int32_t kMessageId = 1157394539;
    ...
    /** Reads a ServerSendRawDataRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ServerSendRawDataRequest decode(const std::uint8_t* src, std::size_t len) {
        ServerSendRawDataRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ServerSendRawDataRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing ServerSendRawDataRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_RAW_DATA_REQUEST_H_
