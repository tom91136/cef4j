...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_CLONE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_CLONE_REQUEST_H_
...
struct DragDataCloneRequest {
    static constexpr int32_t kMessageId = 691641419;
    ...
    /** Reads a DragDataCloneRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DragDataCloneRequest decode(const std::uint8_t* src, std::size_t len) {
        DragDataCloneRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DragDataCloneRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DragDataCloneRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_CLONE_REQUEST_H_
