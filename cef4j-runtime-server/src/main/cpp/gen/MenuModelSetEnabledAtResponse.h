...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_SET_ENABLED_AT_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_SET_ENABLED_AT_RESPONSE_H_
...
struct MenuModelSetEnabledAtResponse {
    ...
    /** Reads a MenuModelSetEnabledAtResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MenuModelSetEnabledAtResponse decode(const std::uint8_t* src, std::size_t len) {
        MenuModelSetEnabledAtResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MenuModelSetEnabledAtResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MenuModelSetEnabledAtResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_SET_ENABLED_AT_RESPONSE_H_
