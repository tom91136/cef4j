...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_DELEGATE_ON_KEY_EVENT_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_DELEGATE_ON_KEY_EVENT_RESPONSE_H_
...
struct TextFieldDelegateOnKeyEventResponse {
    ...
    /** Reads a TextFieldDelegateOnKeyEventResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static TextFieldDelegateOnKeyEventResponse decode(const std::uint8_t* src, std::size_t len) {
        TextFieldDelegateOnKeyEventResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated TextFieldDelegateOnKeyEventResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing TextFieldDelegateOnKeyEventResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_DELEGATE_ON_KEY_EVENT_RESPONSE_H_
