...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_IS_READ_ONLY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_IS_READ_ONLY_REQUEST_H_
...
struct CommandLineIsReadOnlyRequest {
    static constexpr int32_t kMessageId = 896211202;
    ...
    /** Reads a CommandLineIsReadOnlyRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CommandLineIsReadOnlyRequest decode(const std::uint8_t* src, std::size_t len) {
        CommandLineIsReadOnlyRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CommandLineIsReadOnlyRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CommandLineIsReadOnlyRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_IS_READ_ONLY_REQUEST_H_
