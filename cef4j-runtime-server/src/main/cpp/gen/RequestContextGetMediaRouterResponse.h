...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_GET_MEDIA_ROUTER_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_GET_MEDIA_ROUTER_RESPONSE_H_
...
struct RequestContextGetMediaRouterResponse {
    ...
    /** Reads a RequestContextGetMediaRouterResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestContextGetMediaRouterResponse decode(const std::uint8_t* src, std::size_t len) {
        RequestContextGetMediaRouterResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestContextGetMediaRouterResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestContextGetMediaRouterResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_GET_MEDIA_ROUTER_RESPONSE_H_
