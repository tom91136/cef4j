...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_GET_FILE_SIZE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_GET_FILE_SIZE_REQUEST_H_
...
struct ZipReaderGetFileSizeRequest {
    ...
    /** Reads a ZipReaderGetFileSizeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ZipReaderGetFileSizeRequest decode(const std::uint8_t* src, std::size_t len) {
        ZipReaderGetFileSizeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ZipReaderGetFileSizeRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ZipReaderGetFileSizeRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_GET_FILE_SIZE_REQUEST_H_
