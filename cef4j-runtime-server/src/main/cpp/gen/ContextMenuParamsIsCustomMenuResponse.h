...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CONTEXT_MENU_PARAMS_IS_CUSTOM_MENU_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CONTEXT_MENU_PARAMS_IS_CUSTOM_MENU_RESPONSE_H_
...
struct ContextMenuParamsIsCustomMenuResponse {
    ...
    /** Reads a ContextMenuParamsIsCustomMenuResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ContextMenuParamsIsCustomMenuResponse decode(const std::uint8_t* src, std::size_t len) {
        ContextMenuParamsIsCustomMenuResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ContextMenuParamsIsCustomMenuResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ContextMenuParamsIsCustomMenuResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CONTEXT_MENU_PARAMS_IS_CUSTOM_MENU_RESPONSE_H_
