...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_EOF_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_EOF_REQUEST_H_
...
struct ZipReaderEofRequest {
    static constexpr int32_t kMessageId = 1451188180;
    ...
    /** Reads a ZipReaderEofRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ZipReaderEofRequest decode(const std::uint8_t* src, std::size_t len) {
        ZipReaderEofRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ZipReaderEofRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ZipReaderEofRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_EOF_REQUEST_H_
