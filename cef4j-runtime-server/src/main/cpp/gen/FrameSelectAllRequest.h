...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_SELECT_ALL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_SELECT_ALL_REQUEST_H_
...
struct FrameSelectAllRequest {
    static constexpr int32_t kMessageId = 791306895;
    ...
    /** Reads a FrameSelectAllRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameSelectAllRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameSelectAllRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameSelectAllRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameSelectAllRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_SELECT_ALL_REQUEST_H_
