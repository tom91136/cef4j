...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_VALUE_HAS_EXCEPTION_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_VALUE_HAS_EXCEPTION_RESPONSE_H_
...
struct V8ValueHasExceptionResponse {
    ...
    /** Reads a V8ValueHasExceptionResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static V8ValueHasExceptionResponse decode(const std::uint8_t* src, std::size_t len) {
        V8ValueHasExceptionResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated V8ValueHasExceptionResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing V8ValueHasExceptionResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_VALUE_HAS_EXCEPTION_RESPONSE_H_
