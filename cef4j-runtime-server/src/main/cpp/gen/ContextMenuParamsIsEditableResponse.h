...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CONTEXT_MENU_PARAMS_IS_EDITABLE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CONTEXT_MENU_PARAMS_IS_EDITABLE_RESPONSE_H_
...
struct ContextMenuParamsIsEditableResponse {
    ...
    /** Reads a ContextMenuParamsIsEditableResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ContextMenuParamsIsEditableResponse decode(const std::uint8_t* src, std::size_t len) {
        ContextMenuParamsIsEditableResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ContextMenuParamsIsEditableResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ContextMenuParamsIsEditableResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CONTEXT_MENU_PARAMS_IS_EDITABLE_RESPONSE_H_
