...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_IMAGE_IS_EMPTY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_IMAGE_IS_EMPTY_REQUEST_H_
...
struct ImageIsEmptyRequest {
    static constexpr int32_t kMessageId = 111373671;
    ...
    /** Reads a ImageIsEmptyRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ImageIsEmptyRequest decode(const std::uint8_t* src, std::size_t len) {
        ImageIsEmptyRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ImageIsEmptyRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ImageIsEmptyRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_IMAGE_IS_EMPTY_REQUEST_H_
