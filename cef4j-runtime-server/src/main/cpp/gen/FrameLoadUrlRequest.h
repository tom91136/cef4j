...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_LOAD_URL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_LOAD_URL_REQUEST_H_
...
struct FrameLoadUrlRequest {
    static constexpr int32_t kMessageId = 1835059987;
    ...
    /** Reads a FrameLoadUrlRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameLoadUrlRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameLoadUrlRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameLoadUrlRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing FrameLoadUrlRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_LOAD_URL_REQUEST_H_
