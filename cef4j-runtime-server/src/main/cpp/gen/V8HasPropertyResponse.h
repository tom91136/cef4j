...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_HAS_PROPERTY_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_HAS_PROPERTY_RESPONSE_H_
...
struct V8HasPropertyResponse {
    ...
    void encodeInto(std::uint8_t* dst) const noexcept {
        std::size_t pos = 0;
        dst[pos] = static_cast<std::uint8_t>(has ? 1 : 0);
        pos += 1;
    }

    /** Reads a V8HasPropertyResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static V8HasPropertyResponse decode(const std::uint8_t* src, std::size_t len) {
        V8HasPropertyResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated V8HasPropertyResponse payload");
        };
        requireAvailable(1);
        out.has = src[pos] != 0;
        pos += 1;
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_HAS_PROPERTY_RESPONSE_H_
