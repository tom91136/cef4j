...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_HAS_IMAGE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_HAS_IMAGE_REQUEST_H_
...
struct DragDataHasImageRequest {
    static constexpr int32_t kMessageId = 1581898757;
    ...
    /** Reads a DragDataHasImageRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DragDataHasImageRequest decode(const std::uint8_t* src, std::size_t len) {
        DragDataHasImageRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DragDataHasImageRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DragDataHasImageRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_HAS_IMAGE_REQUEST_H_
