...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_IS_READ_ONLY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_IS_READ_ONLY_REQUEST_H_
...
struct ValueIsReadOnlyRequest {
    static constexpr int32_t kMessageId = 953450840;
    ...
    /** Reads a ValueIsReadOnlyRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueIsReadOnlyRequest decode(const std::uint8_t* src, std::size_t len) {
        ValueIsReadOnlyRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueIsReadOnlyRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ValueIsReadOnlyRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_IS_READ_ONLY_REQUEST_H_
