...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_PREFERRED_SIZE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_PREFERRED_SIZE_REQUEST_H_
...
struct ViewGetPreferredSizeRequest {
    ...
    /** Reads a ViewGetPreferredSizeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewGetPreferredSizeRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewGetPreferredSizeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewGetPreferredSizeRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewGetPreferredSizeRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_PREFERRED_SIZE_REQUEST_H_
