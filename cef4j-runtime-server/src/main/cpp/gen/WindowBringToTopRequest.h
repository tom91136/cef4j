...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_BRING_TO_TOP_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_BRING_TO_TOP_REQUEST_H_
...
struct WindowBringToTopRequest {
    static constexpr int32_t kMessageId = 1517750903;
    ...
    /** Reads a WindowBringToTopRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowBringToTopRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowBringToTopRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowBringToTopRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowBringToTopRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_BRING_TO_TOP_REQUEST_H_
