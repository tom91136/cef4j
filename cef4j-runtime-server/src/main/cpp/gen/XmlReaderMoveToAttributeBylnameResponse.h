...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_MOVE_TO_ATTRIBUTE_BYLNAME_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_MOVE_TO_ATTRIBUTE_BYLNAME_RESPONSE_H_
...
struct XmlReaderMoveToAttributeBylnameResponse {
    ...
    /** Reads a XmlReaderMoveToAttributeBylnameResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static XmlReaderMoveToAttributeBylnameResponse decode(const std::uint8_t* src, std::size_t len) {
        XmlReaderMoveToAttributeBylnameResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated XmlReaderMoveToAttributeBylnameResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing XmlReaderMoveToAttributeBylnameResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_MOVE_TO_ATTRIBUTE_BYLNAME_RESPONSE_H_
