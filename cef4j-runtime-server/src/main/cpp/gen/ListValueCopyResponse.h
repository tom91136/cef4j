...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_COPY_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_COPY_RESPONSE_H_
...
struct ListValueCopyResponse {
    ...
    /** Reads a ListValueCopyResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ListValueCopyResponse decode(const std::uint8_t* src, std::size_t len) {
        ListValueCopyResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ListValueCopyResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ListValueCopyResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_COPY_RESPONSE_H_
