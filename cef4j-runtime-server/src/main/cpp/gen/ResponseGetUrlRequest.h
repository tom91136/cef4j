...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_GET_URL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_GET_URL_REQUEST_H_
...
struct ResponseGetUrlRequest {
    ...
    /** Reads a ResponseGetUrlRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ResponseGetUrlRequest decode(const std::uint8_t* src, std::size_t len) {
        ResponseGetUrlRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ResponseGetUrlRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ResponseGetUrlRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_GET_URL_REQUEST_H_
