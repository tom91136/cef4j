...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_IS_VALID_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_IS_VALID_REQUEST_H_
...
struct BrowserIsValidRequest {
    static constexpr int32_t kMessageId = 789249755;
    ...
    /** Reads a BrowserIsValidRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserIsValidRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserIsValidRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserIsValidRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserIsValidRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_IS_VALID_REQUEST_H_
