...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_FRAME_GET_COLUMN_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_FRAME_GET_COLUMN_RESPONSE_H_
...
struct V8StackFrameGetColumnResponse {
    ...
    /** Reads a V8StackFrameGetColumnResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static V8StackFrameGetColumnResponse decode(const std::uint8_t* src, std::size_t len) {
        V8StackFrameGetColumnResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated V8StackFrameGetColumnResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing V8StackFrameGetColumnResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_FRAME_GET_COLUMN_RESPONSE_H_
