...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_LAYOUT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_LAYOUT_REQUEST_H_
...
struct PanelLayoutRequest {
    static constexpr int32_t kMessageId = 1238957071;
    ...
    /** Reads a PanelLayoutRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PanelLayoutRequest decode(const std::uint8_t* src, std::size_t len) {
        PanelLayoutRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PanelLayoutRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PanelLayoutRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_LAYOUT_REQUEST_H_
