...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_TELL_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_TELL_RESPONSE_H_
...
struct ZipReaderTellResponse {
    static constexpr int32_t kMessageId = 1080769304;
    ...
    /** Reads a ZipReaderTellResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ZipReaderTellResponse decode(const std::uint8_t* src, std::size_t len) {
        ZipReaderTellResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ZipReaderTellResponse payload");
        };
        requireAvailable(8);
        ...
        pos += 8;
        if (pos != len)
            throw std::invalid_argument("trailing ZipReaderTellResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_TELL_RESPONSE_H_
