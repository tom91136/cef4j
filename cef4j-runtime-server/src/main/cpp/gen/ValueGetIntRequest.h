...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_INT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_INT_REQUEST_H_
...
struct ValueGetIntRequest {
    static constexpr int32_t kMessageId = 2024125977;
    ...
    /** Reads a ValueGetIntRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueGetIntRequest decode(const std::uint8_t* src, std::size_t len) {
        ValueGetIntRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueGetIntRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ValueGetIntRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_INT_REQUEST_H_
