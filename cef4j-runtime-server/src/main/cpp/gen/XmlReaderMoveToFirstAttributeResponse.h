...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_MOVE_TO_FIRST_ATTRIBUTE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_MOVE_TO_FIRST_ATTRIBUTE_RESPONSE_H_
...
struct XmlReaderMoveToFirstAttributeResponse {
    ...
    /** Reads a XmlReaderMoveToFirstAttributeResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static XmlReaderMoveToFirstAttributeResponse decode(const std::uint8_t* src, std::size_t len) {
        XmlReaderMoveToFirstAttributeResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated XmlReaderMoveToFirstAttributeResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing XmlReaderMoveToFirstAttributeResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_MOVE_TO_FIRST_ATTRIBUTE_RESPONSE_H_
