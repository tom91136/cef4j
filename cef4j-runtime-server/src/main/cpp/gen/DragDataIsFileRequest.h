...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_IS_FILE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_IS_FILE_REQUEST_H_
...
struct DragDataIsFileRequest {
    static constexpr int32_t kMessageId = 5967723;
    ...
    /** Reads a DragDataIsFileRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DragDataIsFileRequest decode(const std::uint8_t* src, std::size_t len) {
        DragDataIsFileRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DragDataIsFileRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DragDataIsFileRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_IS_FILE_REQUEST_H_
