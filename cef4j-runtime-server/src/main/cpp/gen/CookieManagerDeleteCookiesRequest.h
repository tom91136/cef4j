...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_DELETE_COOKIES_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_DELETE_COOKIES_REQUEST_H_
...
struct CookieManagerDeleteCookiesRequest {
    static constexpr int32_t kMessageId = 927990966;
    ...
    std::size_t encodedSize() const noexcept {
        return 16 + url.size() + cookieName.size();
    }
    ...
    void encodeInto(std::uint8_t* dst) const noexcept {
        ...
        pos += url.size();
        {
            std::uint32_t v = static_cast<std::uint32_t>(cookieName.size());
            dst[pos + 0] = static_cast<std::uint8_t>(v & 0xFF);
            dst[pos + 1] = static_cast<std::uint8_t>((v >> 8) & 0xFF);
            dst[pos + 2] = static_cast<std::uint8_t>((v >> 16) & 0xFF);
            dst[pos + 3] = static_cast<std::uint8_t>((v >> 24) & 0xFF);
        ...
        std::memcpy(dst + pos, cookieName.data(), cookieName.size());
        pos += cookieName.size();
        {
    ...
    /** Reads a CookieManagerDeleteCookiesRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CookieManagerDeleteCookiesRequest decode(const std::uint8_t* src, std::size_t len) {
        CookieManagerDeleteCookiesRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CookieManagerDeleteCookiesRequest payload");
        };
        requireAvailable(4);
        ...
        }
        {
            ...
            std::int32_t n = static_cast<std::int32_t>(static_cast<std::uint32_t>(src[pos]) | (static_cast<std::uint32_t>(src[pos + 1]) << 8) | (static_cast<std::uint32_t>(src[pos + 2]) << 16) | (static_cast<std::uint32_t>(src[pos + 3]) << 24));
            pos += 4;
            if (n < 0) throw std::invalid_argument("negative length for cookieName");
            if (static_cast<std::size_t>(n) > kMaxFieldBytes) throw std::invalid_argument("oversized length for cookieName");
            requireAvailable(static_cast<std::size_t>(n));
            out.cookieName.assign(reinterpret_cast<const char*>(src + pos), static_cast<std::size_t>(n));
            pos += static_cast<std::size_t>(n);
        }
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CookieManagerDeleteCookiesRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_DELETE_COOKIES_REQUEST_H_
