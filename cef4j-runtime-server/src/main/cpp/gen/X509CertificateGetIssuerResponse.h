...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_X509_CERTIFICATE_GET_ISSUER_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_X509_CERTIFICATE_GET_ISSUER_RESPONSE_H_
...
struct X509CertificateGetIssuerResponse {
    ...
    /** Reads a X509CertificateGetIssuerResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static X509CertificateGetIssuerResponse decode(const std::uint8_t* src, std::size_t len) {
        X509CertificateGetIssuerResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated X509CertificateGetIssuerResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing X509CertificateGetIssuerResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_X509_CERTIFICATE_GET_ISSUER_RESPONSE_H_
