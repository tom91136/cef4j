...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PREFERENCE_MANAGER_SET_PREFERENCE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PREFERENCE_MANAGER_SET_PREFERENCE_RESPONSE_H_
...
struct PreferenceManagerSetPreferenceResponse {
    ...
    /** Reads a PreferenceManagerSetPreferenceResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PreferenceManagerSetPreferenceResponse decode(const std::uint8_t* src, std::size_t len) {
        PreferenceManagerSetPreferenceResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PreferenceManagerSetPreferenceResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PreferenceManagerSetPreferenceResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PREFERENCE_MANAGER_SET_PREFERENCE_RESPONSE_H_
