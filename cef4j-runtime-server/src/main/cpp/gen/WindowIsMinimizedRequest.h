...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_MINIMIZED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_MINIMIZED_REQUEST_H_
...
struct WindowIsMinimizedRequest {
    static constexpr int32_t kMessageId = 1974064642;
    ...
    /** Reads a WindowIsMinimizedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowIsMinimizedRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowIsMinimizedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowIsMinimizedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowIsMinimizedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_MINIMIZED_REQUEST_H_
