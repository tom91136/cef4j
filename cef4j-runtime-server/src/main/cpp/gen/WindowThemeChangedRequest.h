...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_THEME_CHANGED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_THEME_CHANGED_REQUEST_H_
...
struct WindowThemeChangedRequest {
    static constexpr int32_t kMessageId = 1123576112;
    ...
    /** Reads a WindowThemeChangedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowThemeChangedRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowThemeChangedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowThemeChangedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowThemeChangedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_THEME_CHANGED_REQUEST_H_
