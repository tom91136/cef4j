...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_COPY_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_COPY_RESPONSE_H_
...
struct ProcessMessageCopyResponse {
    ...
    /** Reads a ProcessMessageCopyResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ProcessMessageCopyResponse decode(const std::uint8_t* src, std::size_t len) {
        ProcessMessageCopyResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ProcessMessageCopyResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ProcessMessageCopyResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_COPY_RESPONSE_H_
