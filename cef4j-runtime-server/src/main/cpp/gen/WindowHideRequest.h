...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_HIDE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_HIDE_REQUEST_H_
...
struct WindowHideRequest {
    static constexpr int32_t kMessageId = 1444038990;
    ...
    /** Reads a WindowHideRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowHideRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowHideRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowHideRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowHideRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_HIDE_REQUEST_H_
