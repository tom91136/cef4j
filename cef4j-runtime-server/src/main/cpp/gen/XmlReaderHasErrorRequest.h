...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_HAS_ERROR_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_HAS_ERROR_REQUEST_H_
...
struct XmlReaderHasErrorRequest {
    static constexpr int32_t kMessageId = 1170206817;
    ...
    /** Reads a XmlReaderHasErrorRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static XmlReaderHasErrorRequest decode(const std::uint8_t* src, std::size_t len) {
        XmlReaderHasErrorRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated XmlReaderHasErrorRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing XmlReaderHasErrorRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_HAS_ERROR_REQUEST_H_
