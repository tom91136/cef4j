...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_GET_BROWSER_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_GET_BROWSER_REQUEST_H_
...
struct BrowserHostGetBrowserRequest {
    static constexpr int32_t kMessageId = 514806902;
    ...
    /** Reads a BrowserHostGetBrowserRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHostGetBrowserRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserHostGetBrowserRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHostGetBrowserRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserHostGetBrowserRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_GET_BROWSER_REQUEST_H_
