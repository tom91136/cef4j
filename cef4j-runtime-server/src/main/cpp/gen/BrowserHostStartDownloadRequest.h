...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_START_DOWNLOAD_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_START_DOWNLOAD_REQUEST_H_
...
struct BrowserHostStartDownloadRequest {
    ...
    /** Reads a BrowserHostStartDownloadRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHostStartDownloadRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserHostStartDownloadRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHostStartDownloadRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing BrowserHostStartDownloadRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_START_DOWNLOAD_REQUEST_H_
