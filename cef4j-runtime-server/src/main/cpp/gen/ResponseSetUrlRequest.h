...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_SET_URL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_SET_URL_REQUEST_H_
...
struct ResponseSetUrlRequest {
    static constexpr int32_t kMessageId = 1282124042;
    ...
    /** Reads a ResponseSetUrlRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ResponseSetUrlRequest decode(const std::uint8_t* src, std::size_t len) {
        ResponseSetUrlRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ResponseSetUrlRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing ResponseSetUrlRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_SET_URL_REQUEST_H_
