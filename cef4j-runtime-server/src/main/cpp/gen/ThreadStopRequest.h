...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_THREAD_STOP_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_THREAD_STOP_REQUEST_H_
...
struct ThreadStopRequest {
    static constexpr int32_t kMessageId = 636208775;
    ...
    /** Reads a ThreadStopRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ThreadStopRequest decode(const std::uint8_t* src, std::size_t len) {
        ThreadStopRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ThreadStopRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ThreadStopRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_THREAD_STOP_REQUEST_H_
