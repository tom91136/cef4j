...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_CUT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_CUT_REQUEST_H_
...
struct FrameCutRequest {
    static constexpr int32_t kMessageId = 526422829;
    ...
    /** Reads a FrameCutRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameCutRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameCutRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameCutRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameCutRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_CUT_REQUEST_H_
