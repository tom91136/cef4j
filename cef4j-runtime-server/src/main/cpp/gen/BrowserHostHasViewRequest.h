...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_HAS_VIEW_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_HAS_VIEW_REQUEST_H_
...
struct BrowserHostHasViewRequest {
    static constexpr int32_t kMessageId = 238259190;
    ...
    /** Reads a BrowserHostHasViewRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHostHasViewRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserHostHasViewRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHostHasViewRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserHostHasViewRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_HAS_VIEW_REQUEST_H_
