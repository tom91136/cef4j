...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_RUNTIME_STYLE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_RUNTIME_STYLE_REQUEST_H_
...
struct WindowGetRuntimeStyleRequest {
    static constexpr int32_t kMessageId = 739433192;
    ...
    /** Reads a WindowGetRuntimeStyleRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowGetRuntimeStyleRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowGetRuntimeStyleRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowGetRuntimeStyleRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowGetRuntimeStyleRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_RUNTIME_STYLE_REQUEST_H_
