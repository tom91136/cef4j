...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_CLIENT_GET_AUTH_CREDENTIALS_EVENT_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_CLIENT_GET_AUTH_CREDENTIALS_EVENT_H_
...
struct UrlRequestClientGetAuthCredentialsEvent {
    static constexpr int32_t kMessageId = 23367709;
    ...
    std::size_t encodedSize() const noexcept {
        return 24 + host.size() + realm.size() + scheme.size();
    }
    ...
    /** Reads a UrlRequestClientGetAuthCredentialsEvent from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static UrlRequestClientGetAuthCredentialsEvent decode(const std::uint8_t* src, std::size_t len) {
        UrlRequestClientGetAuthCredentialsEvent out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated UrlRequestClientGetAuthCredentialsEvent payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing UrlRequestClientGetAuthCredentialsEvent payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_CLIENT_GET_AUTH_CREDENTIALS_EVENT_H_
