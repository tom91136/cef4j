...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_EOF_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_EOF_REQUEST_H_
...
struct StreamReaderEofRequest {
    static constexpr int32_t kMessageId = 1662168037;
    ...
    /** Reads a StreamReaderEofRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static StreamReaderEofRequest decode(const std::uint8_t* src, std::size_t len) {
        StreamReaderEofRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated StreamReaderEofRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing StreamReaderEofRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_EOF_REQUEST_H_
