...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BINARY_VALUE_COPY_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BINARY_VALUE_COPY_RESPONSE_H_
...
struct BinaryValueCopyResponse {
    ...
    /** Reads a BinaryValueCopyResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BinaryValueCopyResponse decode(const std::uint8_t* src, std::size_t len) {
        BinaryValueCopyResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BinaryValueCopyResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BinaryValueCopyResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BINARY_VALUE_COPY_RESPONSE_H_
