...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOM_NODE_GET_DOCUMENT_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOM_NODE_GET_DOCUMENT_RESPONSE_H_
...
struct DomNodeGetDocumentResponse {
    ...
    /** Reads a DomNodeGetDocumentResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DomNodeGetDocumentResponse decode(const std::uint8_t* src, std::size_t len) {
        DomNodeGetDocumentResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DomNodeGetDocumentResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DomNodeGetDocumentResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOM_NODE_GET_DOCUMENT_RESPONSE_H_
