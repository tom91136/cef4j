...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LAYOUT_AS_BOX_LAYOUT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LAYOUT_AS_BOX_LAYOUT_REQUEST_H_
...
struct LayoutAsBoxLayoutRequest {
    static constexpr int32_t kMessageId = 1531706965;
    ...
    /** Reads a LayoutAsBoxLayoutRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static LayoutAsBoxLayoutRequest decode(const std::uint8_t* src, std::size_t len) {
        LayoutAsBoxLayoutRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated LayoutAsBoxLayoutRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing LayoutAsBoxLayoutRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LAYOUT_AS_BOX_LAYOUT_REQUEST_H_
