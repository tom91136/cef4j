...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_SET_LINK_URL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_SET_LINK_URL_REQUEST_H_
...
struct DragDataSetLinkUrlRequest {
    static constexpr int32_t kMessageId = 1348492174;
    ...
    /** Reads a DragDataSetLinkUrlRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DragDataSetLinkUrlRequest decode(const std::uint8_t* src, std::size_t len) {
        DragDataSetLinkUrlRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DragDataSetLinkUrlRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing DragDataSetLinkUrlRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_SET_LINK_URL_REQUEST_H_
