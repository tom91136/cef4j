...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_IS_OWNED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_IS_OWNED_REQUEST_H_
...
struct DictionaryValueIsOwnedRequest {
    static constexpr int32_t kMessageId = 542381846;
    ...
    /** Reads a DictionaryValueIsOwnedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DictionaryValueIsOwnedRequest decode(const std::uint8_t* src, std::size_t len) {
        DictionaryValueIsOwnedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DictionaryValueIsOwnedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DictionaryValueIsOwnedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_IS_OWNED_REQUEST_H_
