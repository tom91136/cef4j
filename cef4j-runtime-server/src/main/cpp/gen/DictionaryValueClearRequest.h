...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_CLEAR_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_CLEAR_REQUEST_H_
...
struct DictionaryValueClearRequest {
    static constexpr int32_t kMessageId = 1825157096;
    ...
    /** Reads a DictionaryValueClearRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DictionaryValueClearRequest decode(const std::uint8_t* src, std::size_t len) {
        DictionaryValueClearRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DictionaryValueClearRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DictionaryValueClearRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_CLEAR_REQUEST_H_
