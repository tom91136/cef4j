...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_WRITER_MAY_BLOCK_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_WRITER_MAY_BLOCK_REQUEST_H_
...
struct StreamWriterMayBlockRequest {
    static constexpr int32_t kMessageId = 1042024917;
    ...
    /** Reads a StreamWriterMayBlockRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static StreamWriterMayBlockRequest decode(const std::uint8_t* src, std::size_t len) {
        StreamWriterMayBlockRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated StreamWriterMayBlockRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing StreamWriterMayBlockRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_WRITER_MAY_BLOCK_REQUEST_H_
