...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_SET_MINIMUM_SIZE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_SET_MINIMUM_SIZE_REQUEST_H_
...
struct LabelButtonSetMinimumSizeRequest {
    ...
    /** Reads a LabelButtonSetMinimumSizeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static LabelButtonSetMinimumSizeRequest decode(const std::uint8_t* src, std::size_t len) {
        LabelButtonSetMinimumSizeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated LabelButtonSetMinimumSizeRequest payload");
        };
        requireAvailable(4);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_SET_MINIMUM_SIZE_REQUEST_H_
