...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_METHOD_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_METHOD_REQUEST_H_
...
struct RequestGetMethodRequest {
    ...
    /** Reads a RequestGetMethodRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestGetMethodRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestGetMethodRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestGetMethodRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestGetMethodRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_METHOD_REQUEST_H_
