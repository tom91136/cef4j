...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_FLUSH_STORE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_FLUSH_STORE_RESPONSE_H_
...
struct CookieManagerFlushStoreResponse {
    ...
    /** Reads a CookieManagerFlushStoreResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CookieManagerFlushStoreResponse decode(const std::uint8_t* src, std::size_t len) {
        CookieManagerFlushStoreResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CookieManagerFlushStoreResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CookieManagerFlushStoreResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_FLUSH_STORE_RESPONSE_H_
