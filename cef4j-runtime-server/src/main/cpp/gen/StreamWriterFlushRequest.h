...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_WRITER_FLUSH_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_WRITER_FLUSH_REQUEST_H_
...
struct StreamWriterFlushRequest {
    static constexpr int32_t kMessageId = 1873529702;
    ...
    /** Reads a StreamWriterFlushRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static StreamWriterFlushRequest decode(const std::uint8_t* src, std::size_t len) {
        StreamWriterFlushRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated StreamWriterFlushRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing StreamWriterFlushRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_WRITER_FLUSH_REQUEST_H_
