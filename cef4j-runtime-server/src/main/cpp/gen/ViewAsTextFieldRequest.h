...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_TEXT_FIELD_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_TEXT_FIELD_REQUEST_H_
...
struct ViewAsTextFieldRequest {
    static constexpr int32_t kMessageId = 852160333;
    ...
    /** Reads a ViewAsTextFieldRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewAsTextFieldRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewAsTextFieldRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewAsTextFieldRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewAsTextFieldRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_TEXT_FIELD_REQUEST_H_
