...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_IS_VALID_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_IS_VALID_REQUEST_H_
...
struct ValueIsValidRequest {
    static constexpr int32_t kMessageId = 285493374;
    ...
    /** Reads a ValueIsValidRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueIsValidRequest decode(const std::uint8_t* src, std::size_t len) {
        ValueIsValidRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueIsValidRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ValueIsValidRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_IS_VALID_REQUEST_H_
