...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_MAXIMIZE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_MAXIMIZE_REQUEST_H_
...
struct WindowMaximizeRequest {
    static constexpr int32_t kMessageId = 1187420755;
    ...
    /** Reads a WindowMaximizeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowMaximizeRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowMaximizeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowMaximizeRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowMaximizeRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_MAXIMIZE_REQUEST_H_
