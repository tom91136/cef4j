...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_IS_FOCUSED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_IS_FOCUSED_REQUEST_H_
...
struct FrameIsFocusedRequest {
    static constexpr int32_t kMessageId = 97362650;
    ...
    /** Reads a FrameIsFocusedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameIsFocusedRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameIsFocusedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameIsFocusedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameIsFocusedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_IS_FOCUSED_REQUEST_H_
