...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_DICTIONARY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_DICTIONARY_REQUEST_H_
...
struct ValueGetDictionaryRequest {
    static constexpr int32_t kMessageId = 1448040839;
    ...
    /** Reads a ValueGetDictionaryRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueGetDictionaryRequest decode(const std::uint8_t* src, std::size_t len) {
        ValueGetDictionaryRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueGetDictionaryRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ValueGetDictionaryRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_DICTIONARY_REQUEST_H_
