...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_CLOSE_FILE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_CLOSE_FILE_REQUEST_H_
...
struct ZipReaderCloseFileRequest {
    static constexpr int32_t kMessageId = 491649831;
    ...
    /** Reads a ZipReaderCloseFileRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ZipReaderCloseFileRequest decode(const std::uint8_t* src, std::size_t len) {
        ZipReaderCloseFileRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ZipReaderCloseFileRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ZipReaderCloseFileRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_CLOSE_FILE_REQUEST_H_
