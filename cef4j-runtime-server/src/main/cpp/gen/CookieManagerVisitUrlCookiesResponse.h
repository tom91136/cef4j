...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_URL_COOKIES_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_URL_COOKIES_RESPONSE_H_
...
struct CookieManagerVisitUrlCookiesResponse {
    ...
    /** Reads a CookieManagerVisitUrlCookiesResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CookieManagerVisitUrlCookiesResponse decode(const std::uint8_t* src, std::size_t len) {
        CookieManagerVisitUrlCookiesResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CookieManagerVisitUrlCookiesResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CookieManagerVisitUrlCookiesResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_URL_COOKIES_RESPONSE_H_
