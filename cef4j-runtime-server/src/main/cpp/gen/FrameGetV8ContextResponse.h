...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_V8_CONTEXT_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_V8_CONTEXT_RESPONSE_H_
...
struct FrameGetV8ContextResponse {
    ...
    /** Reads a FrameGetV8ContextResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameGetV8ContextResponse decode(const std::uint8_t* src, std::size_t len) {
        FrameGetV8ContextResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameGetV8ContextResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameGetV8ContextResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_V8_CONTEXT_RESPONSE_H_
