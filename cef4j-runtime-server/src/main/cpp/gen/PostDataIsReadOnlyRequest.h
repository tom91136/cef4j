...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_IS_READ_ONLY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_IS_READ_ONLY_REQUEST_H_
...
struct PostDataIsReadOnlyRequest {
    static constexpr int32_t kMessageId = 2141064493;
    ...
    /** Reads a PostDataIsReadOnlyRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PostDataIsReadOnlyRequest decode(const std::uint8_t* src, std::size_t len) {
        PostDataIsReadOnlyRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PostDataIsReadOnlyRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PostDataIsReadOnlyRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_IS_READ_ONLY_REQUEST_H_
