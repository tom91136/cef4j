...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SCROLL_VIEW_HAS_VERTICAL_SCROLLBAR_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SCROLL_VIEW_HAS_VERTICAL_SCROLLBAR_RESPONSE_H_
...
struct ScrollViewHasVerticalScrollbarResponse {
    ...
    /** Reads a ScrollViewHasVerticalScrollbarResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ScrollViewHasVerticalScrollbarResponse decode(const std::uint8_t* src, std::size_t len) {
        ScrollViewHasVerticalScrollbarResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ScrollViewHasVerticalScrollbarResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ScrollViewHasVerticalScrollbarResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SCROLL_VIEW_HAS_VERTICAL_SCROLLBAR_RESPONSE_H_
