...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_SSLVERSION_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_SSLVERSION_REQUEST_H_
...
struct SslStatusGetSslversionRequest {
    static constexpr int32_t kMessageId = 1640697109;
    ...
    /** Reads a SslStatusGetSslversionRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static SslStatusGetSslversionRequest decode(const std::uint8_t* src, std::size_t len) {
        SslStatusGetSslversionRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated SslStatusGetSslversionRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing SslStatusGetSslversionRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_SSLVERSION_REQUEST_H_
