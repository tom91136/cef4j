...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_CAN_GO_BACK_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_CAN_GO_BACK_REQUEST_H_
...
struct BrowserCanGoBackRequest {
    static constexpr int32_t kMessageId = 1175850963;
    ...
    /** Reads a BrowserCanGoBackRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserCanGoBackRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserCanGoBackRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserCanGoBackRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserCanGoBackRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_CAN_GO_BACK_REQUEST_H_
