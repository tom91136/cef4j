...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_CENTER_WINDOW_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_CENTER_WINDOW_REQUEST_H_
...
struct WindowCenterWindowRequest {
    ...
    /** Reads a WindowCenterWindowRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowCenterWindowRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowCenterWindowRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowCenterWindowRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing WindowCenterWindowRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_CENTER_WINDOW_REQUEST_H_
