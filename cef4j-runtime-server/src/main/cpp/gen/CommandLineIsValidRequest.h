...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_IS_VALID_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_IS_VALID_REQUEST_H_
...
struct CommandLineIsValidRequest {
    static constexpr int32_t kMessageId = 209858953;
    ...
    /** Reads a CommandLineIsValidRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CommandLineIsValidRequest decode(const std::uint8_t* src, std::size_t len) {
        CommandLineIsValidRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CommandLineIsValidRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CommandLineIsValidRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_IS_VALID_REQUEST_H_
