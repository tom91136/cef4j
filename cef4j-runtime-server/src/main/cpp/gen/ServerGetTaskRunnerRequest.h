...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_GET_TASK_RUNNER_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_GET_TASK_RUNNER_REQUEST_H_
...
struct ServerGetTaskRunnerRequest {
    static constexpr int32_t kMessageId = 500751793;
    ...
    /** Reads a ServerGetTaskRunnerRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ServerGetTaskRunnerRequest decode(const std::uint8_t* src, std::size_t len) {
        ServerGetTaskRunnerRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ServerGetTaskRunnerRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ServerGetTaskRunnerRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_GET_TASK_RUNNER_REQUEST_H_
