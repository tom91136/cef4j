...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_SET_SIZE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_SET_SIZE_REQUEST_H_
...
struct OverlayControllerSetSizeRequest {
    static constexpr int32_t kMessageId = 108152513;
    ...
    /** Reads a OverlayControllerSetSizeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static OverlayControllerSetSizeRequest decode(const std::uint8_t* src, std::size_t len) {
        OverlayControllerSetSizeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated OverlayControllerSetSizeRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing OverlayControllerSetSizeRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_SET_SIZE_REQUEST_H_
