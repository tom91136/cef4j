...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_GET_COOKIE_MANAGER_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_GET_COOKIE_MANAGER_REQUEST_H_
...
struct RequestContextGetCookieManagerRequest {
    static constexpr int32_t kMessageId = 668334684;
    ...
    /** Reads a RequestContextGetCookieManagerRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestContextGetCookieManagerRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestContextGetCookieManagerRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestContextGetCookieManagerRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestContextGetCookieManagerRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_GET_COOKIE_MANAGER_REQUEST_H_
