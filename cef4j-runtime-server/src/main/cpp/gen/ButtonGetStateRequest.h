...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BUTTON_GET_STATE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BUTTON_GET_STATE_REQUEST_H_
...
struct ButtonGetStateRequest {
    static constexpr int32_t kMessageId = 813799507;
    ...
    /** Reads a ButtonGetStateRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ButtonGetStateRequest decode(const std::uint8_t* src, std::size_t len) {
        ButtonGetStateRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ButtonGetStateRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ButtonGetStateRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BUTTON_GET_STATE_REQUEST_H_
