...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_EXECUTE_DEV_TOOLS_METHOD_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_EXECUTE_DEV_TOOLS_METHOD_RESPONSE_H_
...
struct BrowserHostExecuteDevToolsMethodResponse {
    ...
    /** Reads a BrowserHostExecuteDevToolsMethodResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHostExecuteDevToolsMethodResponse decode(const std::uint8_t* src, std::size_t len) {
        BrowserHostExecuteDevToolsMethodResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHostExecuteDevToolsMethodResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserHostExecuteDevToolsMethodResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_EXECUTE_DEV_TOOLS_METHOD_RESPONSE_H_
