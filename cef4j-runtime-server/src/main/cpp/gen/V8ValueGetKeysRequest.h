...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_VALUE_GET_KEYS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_VALUE_GET_KEYS_REQUEST_H_
...
struct V8ValueGetKeysRequest {
    static constexpr int32_t kMessageId = 341739378;
    ...
    std::size_t encodedSize() const noexcept {
        return 12 + [&]() { std::size_t n = 0; for (const auto& __s : keys) n += 4 + __s.size(); return n; }();
    }
    ...
    /** Reads a V8ValueGetKeysRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static V8ValueGetKeysRequest decode(const std::uint8_t* src, std::size_t len) {
        V8ValueGetKeysRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated V8ValueGetKeysRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing V8ValueGetKeysRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_VALUE_GET_KEYS_REQUEST_H_
