...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_PASTE_AND_MATCH_STYLE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_PASTE_AND_MATCH_STYLE_REQUEST_H_
...
struct FramePasteAndMatchStyleRequest {
    ...
    /** Reads a FramePasteAndMatchStyleRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FramePasteAndMatchStyleRequest decode(const std::uint8_t* src, std::size_t len) {
        FramePasteAndMatchStyleRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FramePasteAndMatchStyleRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FramePasteAndMatchStyleRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_PASTE_AND_MATCH_STYLE_REQUEST_H_
