...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_GET_FILE_NAMES_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_GET_FILE_NAMES_RESPONSE_H_
...
struct DragDataGetFileNamesResponse {
    ...
    /** Reads a DragDataGetFileNamesResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DragDataGetFileNamesResponse decode(const std::uint8_t* src, std::size_t len) {
        DragDataGetFileNamesResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DragDataGetFileNamesResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DragDataGetFileNamesResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_GET_FILE_NAMES_RESPONSE_H_
