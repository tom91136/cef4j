...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_TEXT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_TEXT_REQUEST_H_
...
struct FrameGetTextRequest {
    static constexpr int32_t kMessageId = 1405448944;
    ...
    /** Reads a FrameGetTextRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameGetTextRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameGetTextRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameGetTextRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameGetTextRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_TEXT_REQUEST_H_
