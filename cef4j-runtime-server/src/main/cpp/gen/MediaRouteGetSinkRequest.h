...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_ROUTE_GET_SINK_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_ROUTE_GET_SINK_REQUEST_H_
...
struct MediaRouteGetSinkRequest {
    static constexpr int32_t kMessageId = 1365888035;
    ...
    /** Reads a MediaRouteGetSinkRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MediaRouteGetSinkRequest decode(const std::uint8_t* src, std::size_t len) {
        MediaRouteGetSinkRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MediaRouteGetSinkRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MediaRouteGetSinkRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_ROUTE_GET_SINK_REQUEST_H_
