...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_HAS_FOCUS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_HAS_FOCUS_REQUEST_H_
...
struct ViewHasFocusRequest {
    static constexpr int32_t kMessageId = 763654531;
    ...
    /** Reads a ViewHasFocusRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewHasFocusRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewHasFocusRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewHasFocusRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewHasFocusRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_HAS_FOCUS_REQUEST_H_
