...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_GET_LINE_NUMBER_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_GET_LINE_NUMBER_REQUEST_H_
...
struct XmlReaderGetLineNumberRequest {
    static constexpr int32_t kMessageId = 1926405585;
    ...
    /** Reads a XmlReaderGetLineNumberRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static XmlReaderGetLineNumberRequest decode(const std::uint8_t* src, std::size_t len) {
        XmlReaderGetLineNumberRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated XmlReaderGetLineNumberRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing XmlReaderGetLineNumberRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_GET_LINE_NUMBER_REQUEST_H_
