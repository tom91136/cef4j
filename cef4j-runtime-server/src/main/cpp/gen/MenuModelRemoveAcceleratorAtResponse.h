...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_REMOVE_ACCELERATOR_AT_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_REMOVE_ACCELERATOR_AT_RESPONSE_H_
...
struct MenuModelRemoveAcceleratorAtResponse {
    ...
    /** Reads a MenuModelRemoveAcceleratorAtResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MenuModelRemoveAcceleratorAtResponse decode(const std::uint8_t* src, std::size_t len) {
        MenuModelRemoveAcceleratorAtResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MenuModelRemoveAcceleratorAtResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MenuModelRemoveAcceleratorAtResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_REMOVE_ACCELERATOR_AT_RESPONSE_H_
