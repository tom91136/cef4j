...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_IS_DIAL_SINK_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_IS_DIAL_SINK_RESPONSE_H_
...
struct MediaSinkIsDialSinkResponse {
    ...
    /** Reads a MediaSinkIsDialSinkResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MediaSinkIsDialSinkResponse decode(const std::uint8_t* src, std::size_t len) {
        MediaSinkIsDialSinkResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MediaSinkIsDialSinkResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MediaSinkIsDialSinkResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_IS_DIAL_SINK_RESPONSE_H_
