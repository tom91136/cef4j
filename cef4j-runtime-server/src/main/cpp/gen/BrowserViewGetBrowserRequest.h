...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_VIEW_GET_BROWSER_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_VIEW_GET_BROWSER_REQUEST_H_
...
struct BrowserViewGetBrowserRequest {
    static constexpr int32_t kMessageId = 2082040741;
    ...
    /** Reads a BrowserViewGetBrowserRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserViewGetBrowserRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserViewGetBrowserRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserViewGetBrowserRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserViewGetBrowserRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_VIEW_GET_BROWSER_REQUEST_H_
