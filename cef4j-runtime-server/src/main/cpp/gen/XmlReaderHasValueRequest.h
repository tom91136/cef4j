...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_HAS_VALUE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_HAS_VALUE_REQUEST_H_
...
struct XmlReaderHasValueRequest {
    static constexpr int32_t kMessageId = 2128303255;
    ...
    /** Reads a XmlReaderHasValueRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static XmlReaderHasValueRequest decode(const std::uint8_t* src, std::size_t len) {
        XmlReaderHasValueRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated XmlReaderHasValueRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing XmlReaderHasValueRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_XML_READER_HAS_VALUE_REQUEST_H_
