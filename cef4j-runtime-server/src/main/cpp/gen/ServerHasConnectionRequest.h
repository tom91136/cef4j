...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_HAS_CONNECTION_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_HAS_CONNECTION_REQUEST_H_
...
struct ServerHasConnectionRequest {
    static constexpr int32_t kMessageId = 536676128;
    ...
    /** Reads a ServerHasConnectionRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ServerHasConnectionRequest decode(const std::uint8_t* src, std::size_t len) {
        ServerHasConnectionRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ServerHasConnectionRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ServerHasConnectionRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_HAS_CONNECTION_REQUEST_H_
