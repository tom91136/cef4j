...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_TRACE_GET_FRAME_COUNT_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_TRACE_GET_FRAME_COUNT_RESPONSE_H_
...
struct V8StackTraceGetFrameCountResponse {
    ...
    /** Reads a V8StackTraceGetFrameCountResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static V8StackTraceGetFrameCountResponse decode(const std::uint8_t* src, std::size_t len) {
        V8StackTraceGetFrameCountResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated V8StackTraceGetFrameCountResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing V8StackTraceGetFrameCountResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_TRACE_GET_FRAME_COUNT_RESPONSE_H_
