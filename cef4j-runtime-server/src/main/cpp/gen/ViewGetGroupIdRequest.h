...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_GROUP_ID_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_GROUP_ID_REQUEST_H_
...
struct ViewGetGroupIdRequest {
    static constexpr int32_t kMessageId = 418450507;
    ...
    /** Reads a ViewGetGroupIdRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewGetGroupIdRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewGetGroupIdRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewGetGroupIdRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewGetGroupIdRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_GROUP_ID_REQUEST_H_
