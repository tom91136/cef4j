...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_HAS_DEV_TOOLS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_HAS_DEV_TOOLS_REQUEST_H_
...
struct BrowserHostHasDevToolsRequest {
    static constexpr int32_t kMessageId = 954009789;
    ...
    /** Reads a BrowserHostHasDevToolsRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHostHasDevToolsRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserHostHasDevToolsRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHostHasDevToolsRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserHostHasDevToolsRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_HAS_DEV_TOOLS_REQUEST_H_
