...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_IS_GLOBAL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_IS_GLOBAL_REQUEST_H_
...
struct RequestContextIsGlobalRequest {
    static constexpr int32_t kMessageId = 556309209;
    ...
    /** Reads a RequestContextIsGlobalRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestContextIsGlobalRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestContextIsGlobalRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestContextIsGlobalRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestContextIsGlobalRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_IS_GLOBAL_REQUEST_H_
