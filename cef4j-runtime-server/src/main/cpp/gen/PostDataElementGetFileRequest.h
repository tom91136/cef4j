...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_GET_FILE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_GET_FILE_REQUEST_H_
...
struct PostDataElementGetFileRequest {
    ...
    /** Reads a PostDataElementGetFileRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PostDataElementGetFileRequest decode(const std::uint8_t* src, std::size_t len) {
        PostDataElementGetFileRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PostDataElementGetFileRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PostDataElementGetFileRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_GET_FILE_REQUEST_H_
