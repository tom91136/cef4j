...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_IMAGE_GET_WIDTH_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_IMAGE_GET_WIDTH_RESPONSE_H_
...
struct ImageGetWidthResponse {
    static constexpr int32_t kMessageId = 1402752512;
    ...
    /** Reads a ImageGetWidthResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ImageGetWidthResponse decode(const std::uint8_t* src, std::size_t len) {
        ImageGetWidthResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ImageGetWidthResponse payload");
        };
        requireAvailable(8);
        ...
        pos += 8;
        if (pos != len)
            throw std::invalid_argument("trailing ImageGetWidthResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_IMAGE_GET_WIDTH_RESPONSE_H_
