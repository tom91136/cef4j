...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_IS_POPUP_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_IS_POPUP_REQUEST_H_
...
struct BrowserIsPopupRequest {
    static constexpr int32_t kMessageId = 1370657869;
    ...
    /** Reads a BrowserIsPopupRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserIsPopupRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserIsPopupRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserIsPopupRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserIsPopupRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_IS_POPUP_REQUEST_H_
