...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_SET_INSETS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_SET_INSETS_REQUEST_H_
...
struct OverlayControllerSetInsetsRequest {
    ...
    /** Reads a OverlayControllerSetInsetsRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static OverlayControllerSetInsetsRequest decode(const std::uint8_t* src, std::size_t len) {
        OverlayControllerSetInsetsRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated OverlayControllerSetInsetsRequest payload");
        };
        requireAvailable(4);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_SET_INSETS_REQUEST_H_
