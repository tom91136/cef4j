...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_V8_CONTEXT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_V8_CONTEXT_REQUEST_H_
...
struct FrameGetV8ContextRequest {
    static constexpr int32_t kMessageId = 1106633178;
    ...
    /** Reads a FrameGetV8ContextRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameGetV8ContextRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameGetV8ContextRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameGetV8ContextRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameGetV8ContextRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_GET_V8_CONTEXT_REQUEST_H_
