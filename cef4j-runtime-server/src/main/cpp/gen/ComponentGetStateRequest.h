...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMPONENT_GET_STATE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMPONENT_GET_STATE_REQUEST_H_
...
struct ComponentGetStateRequest {
    static constexpr int32_t kMessageId = 184068214;
    ...
    /** Reads a ComponentGetStateRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ComponentGetStateRequest decode(const std::uint8_t* src, std::size_t len) {
        ComponentGetStateRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ComponentGetStateRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ComponentGetStateRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMPONENT_GET_STATE_REQUEST_H_
