...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_URL_COOKIES_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_URL_COOKIES_REQUEST_H_
...
struct CookieManagerVisitUrlCookiesRequest {
    static constexpr int32_t kMessageId = 155519484;
    static constexpr std::size_t kMaxFieldBytes = 64U * 1024U * 1024U;
    static constexpr std::size_t kMaxCollectionItems = 1000000U;

    std::int32_t self = 0;
    std::string url;
    std::int32_t includeHttpOnly = 0;
    ...
    void encodeInto(std::uint8_t* dst) const noexcept {
        ...
        pos += url.size();
        {
            std::uint32_t v = static_cast<std::uint32_t>(includeHttpOnly);
            dst[pos + 0] = static_cast<std::uint8_t>(v & 0xFF);
            dst[pos + 1] = static_cast<std::uint8_t>((v >> 8) & 0xFF);
            dst[pos + 2] = static_cast<std::uint8_t>((v >> 16) & 0xFF);
            dst[pos + 3] = static_cast<std::uint8_t>((v >> 24) & 0xFF);
        ...
        {
            std::uint32_t v = static_cast<std::uint32_t>(visitor);
            dst[pos + 0] = static_cast<std::uint8_t>(v & 0xFF);
            dst[pos + 1] = static_cast<std::uint8_t>((v >> 8) & 0xFF);
            dst[pos + 2] = static_cast<std::uint8_t>((v >> 16) & 0xFF);
            dst[pos + 3] = static_cast<std::uint8_t>((v >> 24) & 0xFF);
    ...
    /** Reads a CookieManagerVisitUrlCookiesRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CookieManagerVisitUrlCookiesRequest decode(const std::uint8_t* src, std::size_t len) {
        CookieManagerVisitUrlCookiesRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CookieManagerVisitUrlCookiesRequest payload");
        };
        requireAvailable(4);
        ...
        requireAvailable(4);
        out.includeHttpOnly = static_cast<std::int32_t>(static_cast<std::uint32_t>(src[pos]) | (static_cast<std::uint32_t>(src[pos + 1]) << 8) | (static_cast<std::uint32_t>(src[pos + 2]) << 16) | (static_cast<std::uint32_t>(src[pos + 3]) << 24));
        pos += 4;
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CookieManagerVisitUrlCookiesRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_URL_COOKIES_REQUEST_H_
