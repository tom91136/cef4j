...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SHARED_MEMORY_REGION_SIZE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SHARED_MEMORY_REGION_SIZE_REQUEST_H_
...
struct SharedMemoryRegionSizeRequest {
    ...
    /** Reads a SharedMemoryRegionSizeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static SharedMemoryRegionSizeRequest decode(const std::uint8_t* src, std::size_t len) {
        SharedMemoryRegionSizeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated SharedMemoryRegionSizeRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing SharedMemoryRegionSizeRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SHARED_MEMORY_REGION_SIZE_REQUEST_H_
