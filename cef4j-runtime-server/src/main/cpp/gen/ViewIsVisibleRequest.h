...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_VISIBLE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_VISIBLE_REQUEST_H_
...
struct ViewIsVisibleRequest {
    static constexpr int32_t kMessageId = 269487433;
    ...
    /** Reads a ViewIsVisibleRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewIsVisibleRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewIsVisibleRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewIsVisibleRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewIsVisibleRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_VISIBLE_REQUEST_H_
