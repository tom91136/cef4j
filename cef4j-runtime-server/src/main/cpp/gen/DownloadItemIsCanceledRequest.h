...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOWNLOAD_ITEM_IS_CANCELED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOWNLOAD_ITEM_IS_CANCELED_REQUEST_H_
...
struct DownloadItemIsCanceledRequest {
    static constexpr int32_t kMessageId = 1852968329;
    ...
    /** Reads a DownloadItemIsCanceledRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DownloadItemIsCanceledRequest decode(const std::uint8_t* src, std::size_t len) {
        DownloadItemIsCanceledRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DownloadItemIsCanceledRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DownloadItemIsCanceledRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOWNLOAD_ITEM_IS_CANCELED_REQUEST_H_
