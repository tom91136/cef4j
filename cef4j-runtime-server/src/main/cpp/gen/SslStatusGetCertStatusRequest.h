...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_CERT_STATUS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_CERT_STATUS_REQUEST_H_
...
struct SslStatusGetCertStatusRequest {
    static constexpr int32_t kMessageId = 1915479920;
    ...
    /** Reads a SslStatusGetCertStatusRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static SslStatusGetCertStatusRequest decode(const std::uint8_t* src, std::size_t len) {
        SslStatusGetCertStatusRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated SslStatusGetCertStatusRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing SslStatusGetCertStatusRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_CERT_STATUS_REQUEST_H_
