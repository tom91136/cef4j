...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_GET_CHARSET_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_GET_CHARSET_REQUEST_H_
...
struct ResponseGetCharsetRequest {
    ...
    /** Reads a ResponseGetCharsetRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ResponseGetCharsetRequest decode(const std::uint8_t* src, std::size_t len) {
        ResponseGetCharsetRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ResponseGetCharsetRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ResponseGetCharsetRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_GET_CHARSET_REQUEST_H_
