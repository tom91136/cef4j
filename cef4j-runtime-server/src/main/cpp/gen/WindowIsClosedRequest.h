...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_CLOSED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_CLOSED_REQUEST_H_
...
struct WindowIsClosedRequest {
    static constexpr int32_t kMessageId = 1302830721;
    ...
    /** Reads a WindowIsClosedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowIsClosedRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowIsClosedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowIsClosedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowIsClosedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_CLOSED_REQUEST_H_
