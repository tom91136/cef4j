...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_INSETS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_INSETS_REQUEST_H_
...
struct ViewGetInsetsRequest {
    ...
    /** Reads a ViewGetInsetsRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewGetInsetsRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewGetInsetsRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewGetInsetsRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewGetInsetsRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_INSETS_REQUEST_H_
