...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_HAS_KEY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_HAS_KEY_REQUEST_H_
...
struct DictionaryValueHasKeyRequest {
    static constexpr int32_t kMessageId = 546413991;
    ...
    /** Reads a DictionaryValueHasKeyRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DictionaryValueHasKeyRequest decode(const std::uint8_t* src, std::size_t len) {
        DictionaryValueHasKeyRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DictionaryValueHasKeyRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing DictionaryValueHasKeyRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_HAS_KEY_REQUEST_H_
