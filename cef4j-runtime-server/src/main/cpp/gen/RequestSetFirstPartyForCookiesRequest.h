...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_SET_FIRST_PARTY_FOR_COOKIES_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_SET_FIRST_PARTY_FOR_COOKIES_REQUEST_H_
...
struct RequestSetFirstPartyForCookiesRequest {
    ...
    /** Reads a RequestSetFirstPartyForCookiesRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestSetFirstPartyForCookiesRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestSetFirstPartyForCookiesRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestSetFirstPartyForCookiesRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing RequestSetFirstPartyForCookiesRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_SET_FIRST_PARTY_FOR_COOKIES_REQUEST_H_
