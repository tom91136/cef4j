...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_UNDO_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_UNDO_REQUEST_H_
...
struct FrameUndoRequest {
    static constexpr int32_t kMessageId = 246720214;
    ...
    /** Reads a FrameUndoRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameUndoRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameUndoRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameUndoRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameUndoRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_UNDO_REQUEST_H_
