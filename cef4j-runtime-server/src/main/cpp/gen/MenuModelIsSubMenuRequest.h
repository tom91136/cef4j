...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_IS_SUB_MENU_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_IS_SUB_MENU_REQUEST_H_
...
struct MenuModelIsSubMenuRequest {
    static constexpr int32_t kMessageId = 1284087670;
    ...
    /** Reads a MenuModelIsSubMenuRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MenuModelIsSubMenuRequest decode(const std::uint8_t* src, std::size_t len) {
        MenuModelIsSubMenuRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MenuModelIsSubMenuRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MenuModelIsSubMenuRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_IS_SUB_MENU_REQUEST_H_
