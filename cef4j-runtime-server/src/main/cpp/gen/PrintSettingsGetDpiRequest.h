...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_SETTINGS_GET_DPI_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_SETTINGS_GET_DPI_REQUEST_H_
...
struct PrintSettingsGetDpiRequest {
    static constexpr int32_t kMessageId = 2058080521;
    ...
    /** Reads a PrintSettingsGetDpiRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PrintSettingsGetDpiRequest decode(const std::uint8_t* src, std::size_t len) {
        PrintSettingsGetDpiRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PrintSettingsGetDpiRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PrintSettingsGetDpiRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_SETTINGS_GET_DPI_REQUEST_H_
