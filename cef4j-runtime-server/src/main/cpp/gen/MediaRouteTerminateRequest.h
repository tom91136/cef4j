...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_ROUTE_TERMINATE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_ROUTE_TERMINATE_REQUEST_H_
...
struct MediaRouteTerminateRequest {
    ...
    /** Reads a MediaRouteTerminateRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MediaRouteTerminateRequest decode(const std::uint8_t* src, std::size_t len) {
        MediaRouteTerminateRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MediaRouteTerminateRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MediaRouteTerminateRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_ROUTE_TERMINATE_REQUEST_H_
