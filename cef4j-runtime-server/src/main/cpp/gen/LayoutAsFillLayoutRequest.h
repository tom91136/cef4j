...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LAYOUT_AS_FILL_LAYOUT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LAYOUT_AS_FILL_LAYOUT_REQUEST_H_
...
struct LayoutAsFillLayoutRequest {
    static constexpr int32_t kMessageId = 1510804845;
    ...
    /** Reads a LayoutAsFillLayoutRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static LayoutAsFillLayoutRequest decode(const std::uint8_t* src, std::size_t len) {
        LayoutAsFillLayoutRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated LayoutAsFillLayoutRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing LayoutAsFillLayoutRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LAYOUT_AS_FILL_LAYOUT_REQUEST_H_
