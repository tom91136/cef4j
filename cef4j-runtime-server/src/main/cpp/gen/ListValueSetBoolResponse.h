...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_SET_BOOL_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_SET_BOOL_RESPONSE_H_
...
struct ListValueSetBoolResponse {
    ...
    /** Reads a ListValueSetBoolResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ListValueSetBoolResponse decode(const std::uint8_t* src, std::size_t len) {
        ListValueSetBoolResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ListValueSetBoolResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ListValueSetBoolResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_SET_BOOL_RESPONSE_H_
