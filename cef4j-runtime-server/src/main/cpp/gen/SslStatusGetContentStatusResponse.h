...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_CONTENT_STATUS_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_CONTENT_STATUS_RESPONSE_H_
...
struct SslStatusGetContentStatusResponse {
    ...
    /** Reads a SslStatusGetContentStatusResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static SslStatusGetContentStatusResponse decode(const std::uint8_t* src, std::size_t len) {
        SslStatusGetContentStatusResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated SslStatusGetContentStatusResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing SslStatusGetContentStatusResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_CONTENT_STATUS_RESPONSE_H_
