...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_HAS_SELECTION_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_HAS_SELECTION_REQUEST_H_
...
struct TextFieldHasSelectionRequest {
    static constexpr int32_t kMessageId = 1749163084;
    ...
    /** Reads a TextFieldHasSelectionRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static TextFieldHasSelectionRequest decode(const std::uint8_t* src, std::size_t len) {
        TextFieldHasSelectionRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated TextFieldHasSelectionRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing TextFieldHasSelectionRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_HAS_SELECTION_REQUEST_H_
