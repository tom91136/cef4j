...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_SET_MAXIMUM_SIZE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_SET_MAXIMUM_SIZE_REQUEST_H_
...
struct LabelButtonSetMaximumSizeRequest {
    static constexpr int32_t kMessageId = 528477251;
    ...
    /** Reads a LabelButtonSetMaximumSizeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static LabelButtonSetMaximumSizeRequest decode(const std::uint8_t* src, std::size_t len) {
        LabelButtonSetMaximumSizeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated LabelButtonSetMaximumSizeRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing LabelButtonSetMaximumSizeRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_SET_MAXIMUM_SIZE_REQUEST_H_
