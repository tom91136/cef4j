...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_PANEL_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_PANEL_RESPONSE_H_
...
struct ViewAsPanelResponse {
    ...
    /** Reads a ViewAsPanelResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewAsPanelResponse decode(const std::uint8_t* src, std::size_t len) {
        ViewAsPanelResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewAsPanelResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewAsPanelResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_PANEL_RESPONSE_H_
