...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOM_DOCUMENT_GET_ELEMENT_BY_ID_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOM_DOCUMENT_GET_ELEMENT_BY_ID_RESPONSE_H_
...
struct DomDocumentGetElementByIdResponse {
    ...
    /** Reads a DomDocumentGetElementByIdResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DomDocumentGetElementByIdResponse decode(const std::uint8_t* src, std::size_t len) {
        DomDocumentGetElementByIdResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DomDocumentGetElementByIdResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DomDocumentGetElementByIdResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOM_DOCUMENT_GET_ELEMENT_BY_ID_RESPONSE_H_
