...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_RESOURCE_TYPE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_RESOURCE_TYPE_RESPONSE_H_
...
struct RequestGetResourceTypeResponse {
    ...
    /** Reads a RequestGetResourceTypeResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestGetResourceTypeResponse decode(const std::uint8_t* src, std::size_t len) {
        RequestGetResourceTypeResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestGetResourceTypeResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestGetResourceTypeResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_RESOURCE_TYPE_RESPONSE_H_
