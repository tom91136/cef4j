...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_GET_BYTES_COUNT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_GET_BYTES_COUNT_REQUEST_H_
...
struct PostDataElementGetBytesCountRequest {
    ...
    /** Reads a PostDataElementGetBytesCountRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PostDataElementGetBytesCountRequest decode(const std::uint8_t* src, std::size_t len) {
        PostDataElementGetBytesCountRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PostDataElementGetBytesCountRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PostDataElementGetBytesCountRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_GET_BYTES_COUNT_REQUEST_H_
