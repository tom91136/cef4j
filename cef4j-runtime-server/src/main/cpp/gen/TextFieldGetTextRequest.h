...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_GET_TEXT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_GET_TEXT_REQUEST_H_
...
struct TextFieldGetTextRequest {
    ...
    /** Reads a TextFieldGetTextRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static TextFieldGetTextRequest decode(const std::uint8_t* src, std::size_t len) {
        TextFieldGetTextRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated TextFieldGetTextRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing TextFieldGetTextRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_GET_TEXT_REQUEST_H_
