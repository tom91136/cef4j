...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_POST_DATA_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_POST_DATA_REQUEST_H_
...
struct RequestGetPostDataRequest {
    static constexpr int32_t kMessageId = 1050656678;
    ...
    /** Reads a RequestGetPostDataRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestGetPostDataRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestGetPostDataRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestGetPostDataRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestGetPostDataRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_GET_POST_DATA_REQUEST_H_
