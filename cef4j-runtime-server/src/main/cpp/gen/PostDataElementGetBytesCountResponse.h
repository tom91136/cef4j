...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_GET_BYTES_COUNT_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_GET_BYTES_COUNT_RESPONSE_H_
...
struct PostDataElementGetBytesCountResponse {
    ...
    /** Reads a PostDataElementGetBytesCountResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PostDataElementGetBytesCountResponse decode(const std::uint8_t* src, std::size_t len) {
        PostDataElementGetBytesCountResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PostDataElementGetBytesCountResponse payload");
        };
        requireAvailable(8);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_GET_BYTES_COUNT_RESPONSE_H_
