...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_COPY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_COPY_REQUEST_H_
...
struct CommandLineCopyRequest {
    static constexpr int32_t kMessageId = 2127355012;
    ...
    /** Reads a CommandLineCopyRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CommandLineCopyRequest decode(const std::uint8_t* src, std::size_t len) {
        CommandLineCopyRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CommandLineCopyRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CommandLineCopyRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_COPY_REQUEST_H_
