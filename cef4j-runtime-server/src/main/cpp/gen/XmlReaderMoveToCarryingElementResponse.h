...
struct XmlReaderMoveToCarryingElementResponse {
    ...
    static XmlReaderMoveToCarryingElementResponse decode(const std::uint8_t* src, std::size_t len) {
        XmlReaderMoveToCarryingElementResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated XmlReaderMoveToCarryingElementResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing XmlReaderMoveToCarryingElementResponse payload");
        return out;
    }
};
