...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_FLUSH_STORE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_FLUSH_STORE_REQUEST_H_
...
struct CookieManagerFlushStoreRequest {
    static constexpr int32_t kMessageId = 1392902084;
    ...
    /** Reads a CookieManagerFlushStoreRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CookieManagerFlushStoreRequest decode(const std::uint8_t* src, std::size_t len) {
        CookieManagerFlushStoreRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CookieManagerFlushStoreRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CookieManagerFlushStoreRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_FLUSH_STORE_REQUEST_H_
