...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_IS_READ_ONLY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_IS_READ_ONLY_REQUEST_H_
...
struct ResponseIsReadOnlyRequest {
    static constexpr int32_t kMessageId = 288197779;
    ...
    /** Reads a ResponseIsReadOnlyRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ResponseIsReadOnlyRequest decode(const std::uint8_t* src, std::size_t len) {
        ResponseIsReadOnlyRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ResponseIsReadOnlyRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ResponseIsReadOnlyRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_RESPONSE_IS_READ_ONLY_REQUEST_H_
