...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_X509_CERTIFICATE_GET_PEMENCODED_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_X509_CERTIFICATE_GET_PEMENCODED_RESPONSE_H_
...
struct X509CertificateGetPemencodedResponse {
    ...
    /** Reads a X509CertificateGetPemencodedResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static X509CertificateGetPemencodedResponse decode(const std::uint8_t* src, std::size_t len) {
        X509CertificateGetPemencodedResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated X509CertificateGetPemencodedResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing X509CertificateGetPemencodedResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_X509_CERTIFICATE_GET_PEMENCODED_RESPONSE_H_
