...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_IS_COMMAND_ENABLED_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_IS_COMMAND_ENABLED_RESPONSE_H_
...
struct TextFieldIsCommandEnabledResponse {
    ...
    /** Reads a TextFieldIsCommandEnabledResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static TextFieldIsCommandEnabledResponse decode(const std::uint8_t* src, std::size_t len) {
        TextFieldIsCommandEnabledResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated TextFieldIsCommandEnabledResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing TextFieldIsCommandEnabledResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_TEXT_FIELD_IS_COMMAND_ENABLED_RESPONSE_H_
