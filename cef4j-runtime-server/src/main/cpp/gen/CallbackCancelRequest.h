...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CALLBACK_CANCEL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CALLBACK_CANCEL_REQUEST_H_
...
struct CallbackCancelRequest {
    static constexpr int32_t kMessageId = 1531504144;
    ...
    /** Reads a CallbackCancelRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CallbackCancelRequest decode(const std::uint8_t* src, std::size_t len) {
        CallbackCancelRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CallbackCancelRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CallbackCancelRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CALLBACK_CANCEL_REQUEST_H_
