...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CALLBACK_CONT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CALLBACK_CONT_REQUEST_H_
...
struct CallbackContRequest {
    static constexpr int32_t kMessageId = 1220021737;
    ...
    /** Reads a CallbackContRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CallbackContRequest decode(const std::uint8_t* src, std::size_t len) {
        CallbackContRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CallbackContRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CallbackContRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CALLBACK_CONT_REQUEST_H_
