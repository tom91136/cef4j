...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GET_HOST_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GET_HOST_REQUEST_H_
...
struct BrowserGetHostRequest {
    static constexpr int32_t kMessageId = 1309001117;
    ...
    /** Reads a BrowserGetHostRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserGetHostRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserGetHostRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserGetHostRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserGetHostRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GET_HOST_REQUEST_H_
