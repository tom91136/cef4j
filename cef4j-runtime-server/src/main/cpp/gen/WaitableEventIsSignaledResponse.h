...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WAITABLE_EVENT_IS_SIGNALED_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WAITABLE_EVENT_IS_SIGNALED_RESPONSE_H_
...
struct WaitableEventIsSignaledResponse {
    ...
    /** Reads a WaitableEventIsSignaledResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WaitableEventIsSignaledResponse decode(const std::uint8_t* src, std::size_t len) {
        WaitableEventIsSignaledResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WaitableEventIsSignaledResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WaitableEventIsSignaledResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WAITABLE_EVENT_IS_SIGNALED_RESPONSE_H_
