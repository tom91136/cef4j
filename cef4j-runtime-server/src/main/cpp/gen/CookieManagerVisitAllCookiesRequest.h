...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_ALL_COOKIES_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_ALL_COOKIES_REQUEST_H_
...
struct CookieManagerVisitAllCookiesRequest {
    ...
    /** Reads a CookieManagerVisitAllCookiesRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CookieManagerVisitAllCookiesRequest decode(const std::uint8_t* src, std::size_t len) {
        CookieManagerVisitAllCookiesRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CookieManagerVisitAllCookiesRequest payload");
        };
        requireAvailable(4);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COOKIE_MANAGER_VISIT_ALL_COOKIES_REQUEST_H_
