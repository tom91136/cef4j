...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_GET_IMAGE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_GET_IMAGE_REQUEST_H_
...
struct LabelButtonGetImageRequest {
    ...
    /** Reads a LabelButtonGetImageRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static LabelButtonGetImageRequest decode(const std::uint8_t* src, std::size_t len) {
        LabelButtonGetImageRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated LabelButtonGetImageRequest payload");
        };
        requireAvailable(4);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LABEL_BUTTON_GET_IMAGE_REQUEST_H_
