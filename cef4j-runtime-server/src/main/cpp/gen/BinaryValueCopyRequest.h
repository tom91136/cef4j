...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BINARY_VALUE_COPY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BINARY_VALUE_COPY_REQUEST_H_
...
struct BinaryValueCopyRequest {
    static constexpr int32_t kMessageId = 1519196286;
    ...
    /** Reads a BinaryValueCopyRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BinaryValueCopyRequest decode(const std::uint8_t* src, std::size_t len) {
        BinaryValueCopyRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BinaryValueCopyRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BinaryValueCopyRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BINARY_VALUE_COPY_REQUEST_H_
