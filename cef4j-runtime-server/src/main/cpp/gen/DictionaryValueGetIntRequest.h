...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_GET_INT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_GET_INT_REQUEST_H_
...
struct DictionaryValueGetIntRequest {
    static constexpr int32_t kMessageId = 470777885;
    ...
    /** Reads a DictionaryValueGetIntRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DictionaryValueGetIntRequest decode(const std::uint8_t* src, std::size_t len) {
        DictionaryValueGetIntRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DictionaryValueGetIntRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing DictionaryValueGetIntRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_GET_INT_REQUEST_H_
