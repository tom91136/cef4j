...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_CLOSE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_CLOSE_REQUEST_H_
...
struct ZipReaderCloseRequest {
    static constexpr int32_t kMessageId = 1484338384;
    ...
    /** Reads a ZipReaderCloseRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ZipReaderCloseRequest decode(const std::uint8_t* src, std::size_t len) {
        ZipReaderCloseRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ZipReaderCloseRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ZipReaderCloseRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_CLOSE_REQUEST_H_
