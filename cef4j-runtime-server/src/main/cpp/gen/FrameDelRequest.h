...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_DEL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_DEL_REQUEST_H_
...
struct FrameDelRequest {
    static constexpr int32_t kMessageId = 1195730242;
    ...
    /** Reads a FrameDelRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameDelRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameDelRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameDelRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameDelRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_DEL_REQUEST_H_
