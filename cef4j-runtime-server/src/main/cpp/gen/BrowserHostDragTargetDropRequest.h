...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_DRAG_TARGET_DROP_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_DRAG_TARGET_DROP_REQUEST_H_
...
struct BrowserHostDragTargetDropRequest {
    ...
    /** Reads a BrowserHostDragTargetDropRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHostDragTargetDropRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserHostDragTargetDropRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHostDragTargetDropRequest payload");
        };
        requireAvailable(4);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_DRAG_TARGET_DROP_REQUEST_H_
