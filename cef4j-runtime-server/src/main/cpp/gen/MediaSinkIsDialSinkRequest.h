...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_IS_DIAL_SINK_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_IS_DIAL_SINK_REQUEST_H_
...
struct MediaSinkIsDialSinkRequest {
    static constexpr int32_t kMessageId = 757963904;
    ...
    /** Reads a MediaSinkIsDialSinkRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MediaSinkIsDialSinkRequest decode(const std::uint8_t* src, std::size_t len) {
        MediaSinkIsDialSinkRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MediaSinkIsDialSinkRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MediaSinkIsDialSinkRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_IS_DIAL_SINK_REQUEST_H_
