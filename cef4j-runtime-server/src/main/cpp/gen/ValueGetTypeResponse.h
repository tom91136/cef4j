...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_TYPE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_TYPE_RESPONSE_H_
...
struct ValueGetTypeResponse {
    ...
    /** Reads a ValueGetTypeResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueGetTypeResponse decode(const std::uint8_t* src, std::size_t len) {
        ValueGetTypeResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueGetTypeResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ValueGetTypeResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_GET_TYPE_RESPONSE_H_
