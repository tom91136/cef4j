...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_AUTH_CALLBACK_CONT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_AUTH_CALLBACK_CONT_REQUEST_H_
...
struct AuthCallbackContRequest {
    static constexpr int32_t kMessageId = 608486756;
    ...
    std::size_t encodedSize() const noexcept {
        return 12 + username.size() + password.size();
    }
    ...
    /** Reads a AuthCallbackContRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static AuthCallbackContRequest decode(const std::uint8_t* src, std::size_t len) {
        AuthCallbackContRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated AuthCallbackContRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing AuthCallbackContRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_AUTH_CALLBACK_CONT_REQUEST_H_
