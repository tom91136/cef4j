...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_RESOLVE_HOST_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_RESOLVE_HOST_REQUEST_H_
...
struct RequestContextResolveHostRequest {
    static constexpr int32_t kMessageId = 1418901286;
    ...
    /** Reads a RequestContextResolveHostRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestContextResolveHostRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestContextResolveHostRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestContextResolveHostRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestContextResolveHostRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_CONTEXT_RESOLVE_HOST_REQUEST_H_
