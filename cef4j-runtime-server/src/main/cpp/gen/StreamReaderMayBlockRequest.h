...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_MAY_BLOCK_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_MAY_BLOCK_REQUEST_H_
...
struct StreamReaderMayBlockRequest {
    static constexpr int32_t kMessageId = 1624286276;
    ...
    /** Reads a StreamReaderMayBlockRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static StreamReaderMayBlockRequest decode(const std::uint8_t* src, std::size_t len) {
        StreamReaderMayBlockRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated StreamReaderMayBlockRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing StreamReaderMayBlockRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_MAY_BLOCK_REQUEST_H_
