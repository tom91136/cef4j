...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_PASTE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_PASTE_REQUEST_H_
...
struct FramePasteRequest {
    static constexpr int32_t kMessageId = 1916939678;
    ...
    /** Reads a FramePasteRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FramePasteRequest decode(const std::uint8_t* src, std::size_t len) {
        FramePasteRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FramePasteRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FramePasteRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_PASTE_REQUEST_H_
