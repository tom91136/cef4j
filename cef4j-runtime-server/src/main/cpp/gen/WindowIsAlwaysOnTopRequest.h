...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_ALWAYS_ON_TOP_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_ALWAYS_ON_TOP_REQUEST_H_
...
struct WindowIsAlwaysOnTopRequest {
    static constexpr int32_t kMessageId = 336577631;
    ...
    /** Reads a WindowIsAlwaysOnTopRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowIsAlwaysOnTopRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowIsAlwaysOnTopRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowIsAlwaysOnTopRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowIsAlwaysOnTopRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_IS_ALWAYS_ON_TOP_REQUEST_H_
