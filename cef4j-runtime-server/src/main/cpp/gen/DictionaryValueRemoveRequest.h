...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_REMOVE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_REMOVE_REQUEST_H_
...
struct DictionaryValueRemoveRequest {
    static constexpr int32_t kMessageId = 1466486331;
    ...
    /** Reads a DictionaryValueRemoveRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DictionaryValueRemoveRequest decode(const std::uint8_t* src, std::size_t len) {
        DictionaryValueRemoveRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DictionaryValueRemoveRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing DictionaryValueRemoveRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DICTIONARY_VALUE_REMOVE_REQUEST_H_
