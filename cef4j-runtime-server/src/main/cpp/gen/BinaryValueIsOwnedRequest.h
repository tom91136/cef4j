...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BINARY_VALUE_IS_OWNED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BINARY_VALUE_IS_OWNED_REQUEST_H_
...
struct BinaryValueIsOwnedRequest {
    static constexpr int32_t kMessageId = 516006838;
    ...
    /** Reads a BinaryValueIsOwnedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BinaryValueIsOwnedRequest decode(const std::uint8_t* src, std::size_t len) {
        BinaryValueIsOwnedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BinaryValueIsOwnedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BinaryValueIsOwnedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BINARY_VALUE_IS_OWNED_REQUEST_H_
