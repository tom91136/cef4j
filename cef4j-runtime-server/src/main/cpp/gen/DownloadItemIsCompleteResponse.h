...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOWNLOAD_ITEM_IS_COMPLETE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOWNLOAD_ITEM_IS_COMPLETE_RESPONSE_H_
...
struct DownloadItemIsCompleteResponse {
    ...
    /** Reads a DownloadItemIsCompleteResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DownloadItemIsCompleteResponse decode(const std::uint8_t* src, std::size_t len) {
        DownloadItemIsCompleteResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DownloadItemIsCompleteResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DownloadItemIsCompleteResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOWNLOAD_ITEM_IS_COMPLETE_RESPONSE_H_
