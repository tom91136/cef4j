...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_IS_READ_ONLY_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_IS_READ_ONLY_RESPONSE_H_
...
struct ProcessMessageIsReadOnlyResponse {
    ...
    /** Reads a ProcessMessageIsReadOnlyResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ProcessMessageIsReadOnlyResponse decode(const std::uint8_t* src, std::size_t len) {
        ProcessMessageIsReadOnlyResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ProcessMessageIsReadOnlyResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ProcessMessageIsReadOnlyResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_IS_READ_ONLY_RESPONSE_H_
