...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BEFORE_DOWNLOAD_CALLBACK_CONT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BEFORE_DOWNLOAD_CALLBACK_CONT_REQUEST_H_
...
struct BeforeDownloadCallbackContRequest {
    ...
    void encodeInto(std::uint8_t* dst) const noexcept {
        ...
        {
            std::uint32_t v = static_cast<std::uint32_t>(downloadPath.size());
            dst[pos + 0] = static_cast<std::uint8_t>(v & 0xFF);
            dst[pos + 1] = static_cast<std::uint8_t>((v >> 8) & 0xFF);
            dst[pos + 2] = static_cast<std::uint8_t>((v >> 16) & 0xFF);
            dst[pos + 3] = static_cast<std::uint8_t>((v >> 24) & 0xFF);
        ...
        std::memcpy(dst + pos, downloadPath.data(), downloadPath.size());
        pos += downloadPath.size();
        {
            std::uint32_t v = static_cast<std::uint32_t>(showDialog);
            dst[pos + 0] = static_cast<std::uint8_t>(v & 0xFF);
            dst[pos + 1] = static_cast<std::uint8_t>((v >> 8) & 0xFF);
            dst[pos + 2] = static_cast<std::uint8_t>((v >> 16) & 0xFF);
            dst[pos + 3] = static_cast<std::uint8_t>((v >> 24) & 0xFF);
    ...
    /** Reads a BeforeDownloadCallbackContRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BeforeDownloadCallbackContRequest decode(const std::uint8_t* src, std::size_t len) {
        BeforeDownloadCallbackContRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BeforeDownloadCallbackContRequest payload");
        };
        requireAvailable(4);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BEFORE_DOWNLOAD_CALLBACK_CONT_REQUEST_H_
