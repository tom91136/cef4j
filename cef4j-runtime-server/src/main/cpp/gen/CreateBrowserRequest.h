...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CREATE_BROWSER_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CREATE_BROWSER_REQUEST_H_
...
struct CreateBrowserRequest {
    ...
    std::size_t encodedSize() const noexcept {
        return 4 + url.size() + settings.encodedSize();
    }
    ...
    /** Reads a CreateBrowserRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CreateBrowserRequest decode(const std::uint8_t* src, std::size_t len) {
        CreateBrowserRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CreateBrowserRequest payload");
        };
        {
            requireAvailable(4);
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CREATE_BROWSER_REQUEST_H_
