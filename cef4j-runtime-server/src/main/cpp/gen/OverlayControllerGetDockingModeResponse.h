...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_GET_DOCKING_MODE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_GET_DOCKING_MODE_RESPONSE_H_
...
struct OverlayControllerGetDockingModeResponse {
    ...
    /** Reads a OverlayControllerGetDockingModeResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static OverlayControllerGetDockingModeResponse decode(const std::uint8_t* src, std::size_t len) {
        OverlayControllerGetDockingModeResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated OverlayControllerGetDockingModeResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing OverlayControllerGetDockingModeResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_OVERLAY_CONTROLLER_GET_DOCKING_MODE_RESPONSE_H_
