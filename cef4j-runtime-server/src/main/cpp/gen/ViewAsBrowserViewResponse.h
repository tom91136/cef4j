...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_BROWSER_VIEW_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_BROWSER_VIEW_RESPONSE_H_
...
struct ViewAsBrowserViewResponse {
    ...
    /** Reads a ViewAsBrowserViewResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewAsBrowserViewResponse decode(const std::uint8_t* src, std::size_t len) {
        ViewAsBrowserViewResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewAsBrowserViewResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewAsBrowserViewResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_BROWSER_VIEW_RESPONSE_H_
