...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_SETTINGS_IS_VALID_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_SETTINGS_IS_VALID_REQUEST_H_
...
struct PrintSettingsIsValidRequest {
    static constexpr int32_t kMessageId = 2005357121;
    ...
    /** Reads a PrintSettingsIsValidRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PrintSettingsIsValidRequest decode(const std::uint8_t* src, std::size_t len) {
        PrintSettingsIsValidRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PrintSettingsIsValidRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PrintSettingsIsValidRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_SETTINGS_IS_VALID_REQUEST_H_
