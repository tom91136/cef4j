...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_GET_IMAGE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_GET_IMAGE_REQUEST_H_
...
struct DragDataGetImageRequest {
    static constexpr int32_t kMessageId = 185954820;
    ...
    /** Reads a DragDataGetImageRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DragDataGetImageRequest decode(const std::uint8_t* src, std::size_t len) {
        DragDataGetImageRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DragDataGetImageRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DragDataGetImageRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DRAG_DATA_GET_IMAGE_REQUEST_H_
