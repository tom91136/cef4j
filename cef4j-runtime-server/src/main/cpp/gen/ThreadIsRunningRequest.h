...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_THREAD_IS_RUNNING_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_THREAD_IS_RUNNING_REQUEST_H_
...
struct ThreadIsRunningRequest {
    static constexpr int32_t kMessageId = 259850469;
    ...
    /** Reads a ThreadIsRunningRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ThreadIsRunningRequest decode(const std::uint8_t* src, std::size_t len) {
        ThreadIsRunningRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ThreadIsRunningRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ThreadIsRunningRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_THREAD_IS_RUNNING_REQUEST_H_
