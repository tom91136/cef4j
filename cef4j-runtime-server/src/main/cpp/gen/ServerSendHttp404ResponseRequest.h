...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_HTTP404_RESPONSE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_HTTP404_RESPONSE_REQUEST_H_
...
struct ServerSendHttp404ResponseRequest {
    static constexpr int32_t kMessageId = 1487106236;
    ...
    /** Reads a ServerSendHttp404ResponseRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ServerSendHttp404ResponseRequest decode(const std::uint8_t* src, std::size_t len) {
        ServerSendHttp404ResponseRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ServerSendHttp404ResponseRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ServerSendHttp404ResponseRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_HTTP404_RESPONSE_REQUEST_H_
