...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_DELEGATE_ACCEPTS_FIRST_MOUSE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_DELEGATE_ACCEPTS_FIRST_MOUSE_RESPONSE_H_
...
struct WindowDelegateAcceptsFirstMouseResponse {
    ...
    /** Reads a WindowDelegateAcceptsFirstMouseResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowDelegateAcceptsFirstMouseResponse decode(const std::uint8_t* src, std::size_t len) {
        WindowDelegateAcceptsFirstMouseResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowDelegateAcceptsFirstMouseResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowDelegateAcceptsFirstMouseResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_DELEGATE_ACCEPTS_FIRST_MOUSE_RESPONSE_H_
