...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOM_NODE_GET_PREVIOUS_SIBLING_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOM_NODE_GET_PREVIOUS_SIBLING_RESPONSE_H_
...
struct DomNodeGetPreviousSiblingResponse {
    ...
    /** Reads a DomNodeGetPreviousSiblingResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DomNodeGetPreviousSiblingResponse decode(const std::uint8_t* src, std::size_t len) {
        DomNodeGetPreviousSiblingResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DomNodeGetPreviousSiblingResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DomNodeGetPreviousSiblingResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DOM_NODE_GET_PREVIOUS_SIBLING_RESPONSE_H_
