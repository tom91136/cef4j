...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_DISPLAY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_DISPLAY_REQUEST_H_
...
struct WindowGetDisplayRequest {
    static constexpr int32_t kMessageId = 1353049761;
    ...
    /** Reads a WindowGetDisplayRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowGetDisplayRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowGetDisplayRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowGetDisplayRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowGetDisplayRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_DISPLAY_REQUEST_H_
