...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_GET_INDEX_OF_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_GET_INDEX_OF_RESPONSE_H_
...
struct MenuModelGetIndexOfResponse {
    ...
    /** Reads a MenuModelGetIndexOfResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MenuModelGetIndexOfResponse decode(const std::uint8_t* src, std::size_t len) {
        MenuModelGetIndexOfResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MenuModelGetIndexOfResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MenuModelGetIndexOfResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_GET_INDEX_OF_RESPONSE_H_
