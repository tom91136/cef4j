...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_IS_RUNNING_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_IS_RUNNING_REQUEST_H_
...
struct ServerIsRunningRequest {
    static constexpr int32_t kMessageId = 600005090;
    ...
    /** Reads a ServerIsRunningRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ServerIsRunningRequest decode(const std::uint8_t* src, std::size_t len) {
        ServerIsRunningRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ServerIsRunningRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ServerIsRunningRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_IS_RUNNING_REQUEST_H_
