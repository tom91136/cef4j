...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LAYOUT_IS_VALID_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LAYOUT_IS_VALID_REQUEST_H_
...
struct LayoutIsValidRequest {
    static constexpr int32_t kMessageId = 1001613371;
    ...
    /** Reads a LayoutIsValidRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static LayoutIsValidRequest decode(const std::uint8_t* src, std::size_t len) {
        LayoutIsValidRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated LayoutIsValidRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing LayoutIsValidRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LAYOUT_IS_VALID_REQUEST_H_
