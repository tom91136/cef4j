...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_CLOSE_CONNECTION_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_CLOSE_CONNECTION_REQUEST_H_
...
struct ServerCloseConnectionRequest {
    static constexpr int32_t kMessageId = 76542049;
    ...
    /** Reads a ServerCloseConnectionRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ServerCloseConnectionRequest decode(const std::uint8_t* src, std::size_t len) {
        ServerCloseConnectionRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ServerCloseConnectionRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ServerCloseConnectionRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_CLOSE_CONNECTION_REQUEST_H_
