...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_TRACE_IS_VALID_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_TRACE_IS_VALID_RESPONSE_H_
...
struct V8StackTraceIsValidResponse {
    ...
    /** Reads a V8StackTraceIsValidResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static V8StackTraceIsValidResponse decode(const std::uint8_t* src, std::size_t len) {
        V8StackTraceIsValidResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated V8StackTraceIsValidResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing V8StackTraceIsValidResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_V8_STACK_TRACE_IS_VALID_RESPONSE_H_
