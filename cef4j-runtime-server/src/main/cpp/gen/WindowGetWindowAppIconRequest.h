...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_WINDOW_APP_ICON_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_WINDOW_APP_ICON_REQUEST_H_
...
struct WindowGetWindowAppIconRequest {
    static constexpr int32_t kMessageId = 1457340081;
    ...
    /** Reads a WindowGetWindowAppIconRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowGetWindowAppIconRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowGetWindowAppIconRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowGetWindowAppIconRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowGetWindowAppIconRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_GET_WINDOW_APP_ICON_REQUEST_H_
