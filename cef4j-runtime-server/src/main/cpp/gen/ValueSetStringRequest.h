...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_SET_STRING_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_SET_STRING_REQUEST_H_
...
struct ValueSetStringRequest {
    static constexpr int32_t kMessageId = 367253564;
    ...
    /** Reads a ValueSetStringRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueSetStringRequest decode(const std::uint8_t* src, std::size_t len) {
        ValueSetStringRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueSetStringRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing ValueSetStringRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_SET_STRING_REQUEST_H_
