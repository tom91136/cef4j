...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_NAVIGATION_ENTRY_IS_VALID_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_NAVIGATION_ENTRY_IS_VALID_RESPONSE_H_
...
struct NavigationEntryIsValidResponse {
    ...
    /** Reads a NavigationEntryIsValidResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static NavigationEntryIsValidResponse decode(const std::uint8_t* src, std::size_t len) {
        NavigationEntryIsValidResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated NavigationEntryIsValidResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing NavigationEntryIsValidResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_NAVIGATION_ENTRY_IS_VALID_RESPONSE_H_
