...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_SET_TO_FILL_LAYOUT_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_SET_TO_FILL_LAYOUT_RESPONSE_H_
...
struct PanelSetToFillLayoutResponse {
    ...
    /** Reads a PanelSetToFillLayoutResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PanelSetToFillLayoutResponse decode(const std::uint8_t* src, std::size_t len) {
        PanelSetToFillLayoutResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PanelSetToFillLayoutResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PanelSetToFillLayoutResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_SET_TO_FILL_LAYOUT_RESPONSE_H_
