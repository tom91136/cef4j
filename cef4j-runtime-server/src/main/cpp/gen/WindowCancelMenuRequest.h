...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_CANCEL_MENU_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_CANCEL_MENU_REQUEST_H_
...
struct WindowCancelMenuRequest {
    static constexpr int32_t kMessageId = 1039654324;
    ...
    /** Reads a WindowCancelMenuRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowCancelMenuRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowCancelMenuRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowCancelMenuRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowCancelMenuRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_CANCEL_MENU_REQUEST_H_
