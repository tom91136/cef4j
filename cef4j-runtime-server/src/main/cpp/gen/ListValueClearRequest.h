...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_CLEAR_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_CLEAR_REQUEST_H_
...
struct ListValueClearRequest {
    static constexpr int32_t kMessageId = 898824090;
    ...
    /** Reads a ListValueClearRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ListValueClearRequest decode(const std::uint8_t* src, std::size_t len) {
        ListValueClearRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ListValueClearRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ListValueClearRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_CLEAR_REQUEST_H_
