...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_IS_READ_ONLY_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_IS_READ_ONLY_RESPONSE_H_
...
struct PostDataElementIsReadOnlyResponse {
    ...
    /** Reads a PostDataElementIsReadOnlyResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PostDataElementIsReadOnlyResponse decode(const std::uint8_t* src, std::size_t len) {
        PostDataElementIsReadOnlyResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PostDataElementIsReadOnlyResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PostDataElementIsReadOnlyResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_POST_DATA_ELEMENT_IS_READ_ONLY_RESPONSE_H_
