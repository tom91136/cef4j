...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_SET_TO_FILL_LAYOUT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_SET_TO_FILL_LAYOUT_REQUEST_H_
...
struct PanelSetToFillLayoutRequest {
    static constexpr int32_t kMessageId = 1671538609;
    ...
    /** Reads a PanelSetToFillLayoutRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PanelSetToFillLayoutRequest decode(const std::uint8_t* src, std::size_t len) {
        PanelSetToFillLayoutRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PanelSetToFillLayoutRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PanelSetToFillLayoutRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PANEL_SET_TO_FILL_LAYOUT_REQUEST_H_
