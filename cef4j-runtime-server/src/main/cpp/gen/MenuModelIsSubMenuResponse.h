...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_IS_SUB_MENU_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_IS_SUB_MENU_RESPONSE_H_
...
struct MenuModelIsSubMenuResponse {
    ...
    /** Reads a MenuModelIsSubMenuResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MenuModelIsSubMenuResponse decode(const std::uint8_t* src, std::size_t len) {
        MenuModelIsSubMenuResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MenuModelIsSubMenuResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MenuModelIsSubMenuResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_IS_SUB_MENU_RESPONSE_H_
