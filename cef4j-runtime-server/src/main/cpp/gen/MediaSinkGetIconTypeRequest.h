...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_GET_ICON_TYPE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_GET_ICON_TYPE_REQUEST_H_
...
struct MediaSinkGetIconTypeRequest {
    static constexpr int32_t kMessageId = 950127450;
    ...
    /** Reads a MediaSinkGetIconTypeRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MediaSinkGetIconTypeRequest decode(const std::uint8_t* src, std::size_t len) {
        MediaSinkGetIconTypeRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MediaSinkGetIconTypeRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MediaSinkGetIconTypeRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_SINK_GET_ICON_TYPE_REQUEST_H_
