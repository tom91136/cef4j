...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_TELL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_TELL_REQUEST_H_
...
struct StreamReaderTellRequest {
    ...
    /** Reads a StreamReaderTellRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static StreamReaderTellRequest decode(const std::uint8_t* src, std::size_t len) {
        StreamReaderTellRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated StreamReaderTellRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing StreamReaderTellRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_TELL_REQUEST_H_
