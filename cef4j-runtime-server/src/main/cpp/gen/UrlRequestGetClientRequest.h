...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_GET_CLIENT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_GET_CLIENT_REQUEST_H_
...
struct UrlRequestGetClientRequest {
    static constexpr int32_t kMessageId = 886940205;
    ...
    /** Reads a UrlRequestGetClientRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static UrlRequestGetClientRequest decode(const std::uint8_t* src, std::size_t len) {
        UrlRequestGetClientRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated UrlRequestGetClientRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing UrlRequestGetClientRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_GET_CLIENT_REQUEST_H_
