...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_SSLVERSION_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_SSLVERSION_RESPONSE_H_
...
struct SslStatusGetSslversionResponse {
    ...
    /** Reads a SslStatusGetSslversionResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static SslStatusGetSslversionResponse decode(const std::uint8_t* src, std::size_t len) {
        SslStatusGetSslversionResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated SslStatusGetSslversionResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing SslStatusGetSslversionResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_SSLVERSION_RESPONSE_H_
