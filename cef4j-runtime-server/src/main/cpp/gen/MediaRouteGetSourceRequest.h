...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_ROUTE_GET_SOURCE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_ROUTE_GET_SOURCE_REQUEST_H_
...
struct MediaRouteGetSourceRequest {
    static constexpr int32_t kMessageId = 43453093;
    ...
    /** Reads a MediaRouteGetSourceRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MediaRouteGetSourceRequest decode(const std::uint8_t* src, std::size_t len) {
        MediaRouteGetSourceRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MediaRouteGetSourceRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MediaRouteGetSourceRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MEDIA_ROUTE_GET_SOURCE_REQUEST_H_
