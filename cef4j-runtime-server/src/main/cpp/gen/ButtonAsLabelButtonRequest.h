...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BUTTON_AS_LABEL_BUTTON_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BUTTON_AS_LABEL_BUTTON_REQUEST_H_
...
struct ButtonAsLabelButtonRequest {
    static constexpr int32_t kMessageId = 1971001053;
    ...
    /** Reads a ButtonAsLabelButtonRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ButtonAsLabelButtonRequest decode(const std::uint8_t* src, std::size_t len) {
        ButtonAsLabelButtonRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ButtonAsLabelButtonRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ButtonAsLabelButtonRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BUTTON_AS_LABEL_BUTTON_REQUEST_H_
