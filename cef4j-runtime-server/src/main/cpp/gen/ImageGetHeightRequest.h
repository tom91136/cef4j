...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_IMAGE_GET_HEIGHT_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_IMAGE_GET_HEIGHT_REQUEST_H_
...
struct ImageGetHeightRequest {
    ...
    /** Reads a ImageGetHeightRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ImageGetHeightRequest decode(const std::uint8_t* src, std::size_t len) {
        ImageGetHeightRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ImageGetHeightRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ImageGetHeightRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_IMAGE_GET_HEIGHT_REQUEST_H_
