...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_INFO_GET_CERT_STATUS_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_INFO_GET_CERT_STATUS_REQUEST_H_
...
struct SslInfoGetCertStatusRequest {
    static constexpr int32_t kMessageId = 2123776495;
    ...
    /** Reads a SslInfoGetCertStatusRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static SslInfoGetCertStatusRequest decode(const std::uint8_t* src, std::size_t len) {
        SslInfoGetCertStatusRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated SslInfoGetCertStatusRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing SslInfoGetCertStatusRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_INFO_GET_CERT_STATUS_REQUEST_H_
