...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_WAS_RESIZED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_WAS_RESIZED_REQUEST_H_
...
struct BrowserHostWasResizedRequest {
    static constexpr int32_t kMessageId = 494564481;
    ...
    /** Reads a BrowserHostWasResizedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserHostWasResizedRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserHostWasResizedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserHostWasResizedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserHostWasResizedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_HOST_WAS_RESIZED_REQUEST_H_
