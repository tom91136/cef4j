...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_IS_OWNED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_IS_OWNED_REQUEST_H_
...
struct ValueIsOwnedRequest {
    static constexpr int32_t kMessageId = 688338645;
    ...
    /** Reads a ValueIsOwnedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueIsOwnedRequest decode(const std::uint8_t* src, std::size_t len) {
        ValueIsOwnedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueIsOwnedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ValueIsOwnedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_IS_OWNED_REQUEST_H_
