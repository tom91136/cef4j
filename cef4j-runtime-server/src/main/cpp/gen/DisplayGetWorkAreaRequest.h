...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DISPLAY_GET_WORK_AREA_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DISPLAY_GET_WORK_AREA_REQUEST_H_
...
struct DisplayGetWorkAreaRequest {
    ...
    /** Reads a DisplayGetWorkAreaRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static DisplayGetWorkAreaRequest decode(const std::uint8_t* src, std::size_t len) {
        DisplayGetWorkAreaRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated DisplayGetWorkAreaRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing DisplayGetWorkAreaRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_DISPLAY_GET_WORK_AREA_REQUEST_H_
