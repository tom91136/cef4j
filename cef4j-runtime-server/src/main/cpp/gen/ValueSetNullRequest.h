...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_SET_NULL_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_SET_NULL_REQUEST_H_
...
struct ValueSetNullRequest {
    static constexpr int32_t kMessageId = 289040231;
    ...
    /** Reads a ValueSetNullRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ValueSetNullRequest decode(const std::uint8_t* src, std::size_t len) {
        ValueSetNullRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ValueSetNullRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ValueSetNullRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VALUE_SET_NULL_REQUEST_H_
