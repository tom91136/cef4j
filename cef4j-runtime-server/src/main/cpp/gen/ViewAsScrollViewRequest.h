...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_SCROLL_VIEW_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_SCROLL_VIEW_REQUEST_H_
...
struct ViewAsScrollViewRequest {
    static constexpr int32_t kMessageId = 1482938540;
    ...
    /** Reads a ViewAsScrollViewRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewAsScrollViewRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewAsScrollViewRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewAsScrollViewRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewAsScrollViewRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_SCROLL_VIEW_REQUEST_H_
