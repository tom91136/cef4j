...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_WEB_SOCKET_MESSAGE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_WEB_SOCKET_MESSAGE_REQUEST_H_
...
struct ServerSendWebSocketMessageRequest {
    static constexpr int32_t kMessageId = 422968274;
    ...
    /** Reads a ServerSendWebSocketMessageRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ServerSendWebSocketMessageRequest decode(const std::uint8_t* src, std::size_t len) {
        ServerSendWebSocketMessageRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ServerSendWebSocketMessageRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing ServerSendWebSocketMessageRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SERVER_SEND_WEB_SOCKET_MESSAGE_REQUEST_H_
