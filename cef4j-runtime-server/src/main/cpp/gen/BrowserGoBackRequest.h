...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GO_BACK_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GO_BACK_REQUEST_H_
...
struct BrowserGoBackRequest {
    static constexpr int32_t kMessageId = 1121766362;
    ...
    /** Reads a BrowserGoBackRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserGoBackRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserGoBackRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserGoBackRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserGoBackRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_GO_BACK_REQUEST_H_
