...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_BACKGROUND_COLOR_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_BACKGROUND_COLOR_REQUEST_H_
...
struct ViewGetBackgroundColorRequest {
    static constexpr int32_t kMessageId = 362089625;
    ...
    /** Reads a ViewGetBackgroundColorRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewGetBackgroundColorRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewGetBackgroundColorRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewGetBackgroundColorRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewGetBackgroundColorRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_BACKGROUND_COLOR_REQUEST_H_
