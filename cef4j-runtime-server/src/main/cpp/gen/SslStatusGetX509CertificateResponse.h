...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_X509_CERTIFICATE_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_X509_CERTIFICATE_RESPONSE_H_
...
struct SslStatusGetX509CertificateResponse {
    ...
    /** Reads a SslStatusGetX509CertificateResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static SslStatusGetX509CertificateResponse decode(const std::uint8_t* src, std::size_t len) {
        SslStatusGetX509CertificateResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated SslStatusGetX509CertificateResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing SslStatusGetX509CertificateResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SSL_STATUS_GET_X509_CERTIFICATE_RESPONSE_H_
