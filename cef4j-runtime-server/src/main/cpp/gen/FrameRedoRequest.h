...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_REDO_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_REDO_REQUEST_H_
...
struct FrameRedoRequest {
    static constexpr int32_t kMessageId = 1460914226;
    ...
    /** Reads a FrameRedoRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static FrameRedoRequest decode(const std::uint8_t* src, std::size_t len) {
        FrameRedoRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated FrameRedoRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing FrameRedoRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_FRAME_REDO_REQUEST_H_
