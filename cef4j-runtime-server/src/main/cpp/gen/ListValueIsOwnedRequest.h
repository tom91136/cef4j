...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_IS_OWNED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_IS_OWNED_REQUEST_H_
...
struct ListValueIsOwnedRequest {
    static constexpr int32_t kMessageId = 952699166;
    ...
    /** Reads a ListValueIsOwnedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ListValueIsOwnedRequest decode(const std::uint8_t* src, std::size_t len) {
        ListValueIsOwnedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ListValueIsOwnedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ListValueIsOwnedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_LIST_VALUE_IS_OWNED_REQUEST_H_
