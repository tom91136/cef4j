...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_IS_READ_ONLY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_IS_READ_ONLY_REQUEST_H_
...
struct RequestIsReadOnlyRequest {
    static constexpr int32_t kMessageId = 1789692372;
    ...
    /** Reads a RequestIsReadOnlyRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static RequestIsReadOnlyRequest decode(const std::uint8_t* src, std::size_t len) {
        RequestIsReadOnlyRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated RequestIsReadOnlyRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing RequestIsReadOnlyRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_REQUEST_IS_READ_ONLY_REQUEST_H_
