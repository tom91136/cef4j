...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_THEME_COLOR_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_THEME_COLOR_RESPONSE_H_
...
struct ViewGetThemeColorResponse {
    ...
    /** Reads a ViewGetThemeColorResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewGetThemeColorResponse decode(const std::uint8_t* src, std::size_t len) {
        ViewGetThemeColorResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewGetThemeColorResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewGetThemeColorResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_GET_THEME_COLOR_RESPONSE_H_
