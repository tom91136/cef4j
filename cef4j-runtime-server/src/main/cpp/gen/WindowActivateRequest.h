...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_ACTIVATE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_ACTIVATE_REQUEST_H_
...
struct WindowActivateRequest {
    static constexpr int32_t kMessageId = 9731910;
    ...
    /** Reads a WindowActivateRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static WindowActivateRequest decode(const std::uint8_t* src, std::size_t len) {
        WindowActivateRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated WindowActivateRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing WindowActivateRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_WINDOW_ACTIVATE_REQUEST_H_
