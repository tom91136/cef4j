...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_GET_PROGRAM_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_GET_PROGRAM_REQUEST_H_
...
struct CommandLineGetProgramRequest {
    ...
    /** Reads a CommandLineGetProgramRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static CommandLineGetProgramRequest decode(const std::uint8_t* src, std::size_t len) {
        CommandLineGetProgramRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated CommandLineGetProgramRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing CommandLineGetProgramRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_COMMAND_LINE_GET_PROGRAM_REQUEST_H_
