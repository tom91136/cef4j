...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_FOCUSABLE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_FOCUSABLE_REQUEST_H_
...
struct ViewIsFocusableRequest {
    static constexpr int32_t kMessageId = 1574817730;
    ...
    /** Reads a ViewIsFocusableRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewIsFocusableRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewIsFocusableRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewIsFocusableRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewIsFocusableRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_FOCUSABLE_REQUEST_H_
