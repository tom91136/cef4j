...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_OPEN_FILE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_OPEN_FILE_REQUEST_H_
...
struct ZipReaderOpenFileRequest {
    static constexpr int32_t kMessageId = 336850608;
    ...
    /** Reads a ZipReaderOpenFileRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ZipReaderOpenFileRequest decode(const std::uint8_t* src, std::size_t len) {
        ZipReaderOpenFileRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ZipReaderOpenFileRequest payload");
        };
        requireAvailable(4);
        ...
        if (pos != len)
            throw std::invalid_argument("trailing ZipReaderOpenFileRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_ZIP_READER_OPEN_FILE_REQUEST_H_
