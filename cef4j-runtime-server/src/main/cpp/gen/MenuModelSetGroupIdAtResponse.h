...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_SET_GROUP_ID_AT_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_SET_GROUP_ID_AT_RESPONSE_H_
...
struct MenuModelSetGroupIdAtResponse {
    ...
    /** Reads a MenuModelSetGroupIdAtResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MenuModelSetGroupIdAtResponse decode(const std::uint8_t* src, std::size_t len) {
        MenuModelSetGroupIdAtResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MenuModelSetGroupIdAtResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MenuModelSetGroupIdAtResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_SET_GROUP_ID_AT_RESPONSE_H_
