...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_CAN_GO_FORWARD_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_CAN_GO_FORWARD_REQUEST_H_
...
struct BrowserCanGoForwardRequest {
    static constexpr int32_t kMessageId = 733389362;
    ...
    /** Reads a BrowserCanGoForwardRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserCanGoForwardRequest decode(const std::uint8_t* src, std::size_t len) {
        BrowserCanGoForwardRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserCanGoForwardRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserCanGoForwardRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_CAN_GO_FORWARD_REQUEST_H_
