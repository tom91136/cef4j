...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_GET_RESPONSE_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_GET_RESPONSE_REQUEST_H_
...
struct UrlRequestGetResponseRequest {
    static constexpr int32_t kMessageId = 1754641586;
    ...
    /** Reads a UrlRequestGetResponseRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static UrlRequestGetResponseRequest decode(const std::uint8_t* src, std::size_t len) {
        UrlRequestGetResponseRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated UrlRequestGetResponseRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing UrlRequestGetResponseRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_URL_REQUEST_GET_RESPONSE_REQUEST_H_
