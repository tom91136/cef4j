...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SCHEME_REGISTRAR_ADD_CUSTOM_SCHEME_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SCHEME_REGISTRAR_ADD_CUSTOM_SCHEME_RESPONSE_H_
...
struct SchemeRegistrarAddCustomSchemeResponse {
    ...
    /** Reads a SchemeRegistrarAddCustomSchemeResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static SchemeRegistrarAddCustomSchemeResponse decode(const std::uint8_t* src, std::size_t len) {
        SchemeRegistrarAddCustomSchemeResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated SchemeRegistrarAddCustomSchemeResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing SchemeRegistrarAddCustomSchemeResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_SCHEME_REGISTRAR_ADD_CUSTOM_SCHEME_RESPONSE_H_
