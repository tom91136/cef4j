...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_SETTINGS_GET_COLOR_MODEL_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_SETTINGS_GET_COLOR_MODEL_RESPONSE_H_
...
struct PrintSettingsGetColorModelResponse {
    ...
    /** Reads a PrintSettingsGetColorModelResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static PrintSettingsGetColorModelResponse decode(const std::uint8_t* src, std::size_t len) {
        PrintSettingsGetColorModelResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated PrintSettingsGetColorModelResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing PrintSettingsGetColorModelResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PRINT_SETTINGS_GET_COLOR_MODEL_RESPONSE_H_
