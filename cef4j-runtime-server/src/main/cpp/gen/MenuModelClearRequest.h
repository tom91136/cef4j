...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_CLEAR_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_CLEAR_REQUEST_H_
...
struct MenuModelClearRequest {
    static constexpr int32_t kMessageId = 1537170853;
    ...
    /** Reads a MenuModelClearRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static MenuModelClearRequest decode(const std::uint8_t* src, std::size_t len) {
        MenuModelClearRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated MenuModelClearRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing MenuModelClearRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_MENU_MODEL_CLEAR_REQUEST_H_
