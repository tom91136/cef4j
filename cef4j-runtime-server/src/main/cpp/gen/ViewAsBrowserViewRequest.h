...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_BROWSER_VIEW_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_BROWSER_VIEW_REQUEST_H_
...
struct ViewAsBrowserViewRequest {
    static constexpr int32_t kMessageId = 930280078;
    ...
    /** Reads a ViewAsBrowserViewRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewAsBrowserViewRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewAsBrowserViewRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewAsBrowserViewRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewAsBrowserViewRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_AS_BROWSER_VIEW_REQUEST_H_
