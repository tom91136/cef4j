...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CONTEXT_MENU_PARAMS_GET_TYPE_FLAGS_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CONTEXT_MENU_PARAMS_GET_TYPE_FLAGS_RESPONSE_H_
...
struct ContextMenuParamsGetTypeFlagsResponse {
    ...
    /** Reads a ContextMenuParamsGetTypeFlagsResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ContextMenuParamsGetTypeFlagsResponse decode(const std::uint8_t* src, std::size_t len) {
        ContextMenuParamsGetTypeFlagsResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ContextMenuParamsGetTypeFlagsResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ContextMenuParamsGetTypeFlagsResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_CONTEXT_MENU_PARAMS_GET_TYPE_FLAGS_RESPONSE_H_
