...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_ATTACHED_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_ATTACHED_REQUEST_H_
...
struct ViewIsAttachedRequest {
    static constexpr int32_t kMessageId = 1134763394;
    ...
    /** Reads a ViewIsAttachedRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ViewIsAttachedRequest decode(const std::uint8_t* src, std::size_t len) {
        ViewIsAttachedRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ViewIsAttachedRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ViewIsAttachedRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_VIEW_IS_ATTACHED_REQUEST_H_
