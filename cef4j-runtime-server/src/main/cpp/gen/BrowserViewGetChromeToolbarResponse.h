...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_VIEW_GET_CHROME_TOOLBAR_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_VIEW_GET_CHROME_TOOLBAR_RESPONSE_H_
...
struct BrowserViewGetChromeToolbarResponse {
    ...
    /** Reads a BrowserViewGetChromeToolbarResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static BrowserViewGetChromeToolbarResponse decode(const std::uint8_t* src, std::size_t len) {
        BrowserViewGetChromeToolbarResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated BrowserViewGetChromeToolbarResponse payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing BrowserViewGetChromeToolbarResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_BROWSER_VIEW_GET_CHROME_TOOLBAR_RESPONSE_H_
