...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_COPY_REQUEST_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_COPY_REQUEST_H_
...
struct ProcessMessageCopyRequest {
    static constexpr int32_t kMessageId = 1804279971;
    ...
    /** Reads a ProcessMessageCopyRequest from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static ProcessMessageCopyRequest decode(const std::uint8_t* src, std::size_t len) {
        ProcessMessageCopyRequest out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated ProcessMessageCopyRequest payload");
        };
        requireAvailable(4);
        ...
        pos += 4;
        if (pos != len)
            throw std::invalid_argument("trailing ProcessMessageCopyRequest payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_PROCESS_MESSAGE_COPY_REQUEST_H_
