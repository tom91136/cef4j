...
#ifndef NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_READ_RESPONSE_H_
#define NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_READ_RESPONSE_H_
...
struct StreamReaderReadResponse {
    ...
    /** Reads a StreamReaderReadResponse from a payload starting at {@code src}; {@code len} bounds the buffer. */
    static StreamReaderReadResponse decode(const std::uint8_t* src, std::size_t len) {
        StreamReaderReadResponse out;
        std::size_t pos = 0;
        [[maybe_unused]] auto requireAvailable = [&](std::size_t count) {
            if (pos > len || count > len - pos)
                throw std::invalid_argument("truncated StreamReaderReadResponse payload");
        };
        requireAvailable(8);
        ...
        pos += 8;
        if (pos != len)
            throw std::invalid_argument("trailing StreamReaderReadResponse payload");
        return out;
    }
};
...
#endif // NET_KUROBAKO_CEF4J_IPC_PROTOCOL_GEN_STREAM_READER_READ_RESPONSE_H_
