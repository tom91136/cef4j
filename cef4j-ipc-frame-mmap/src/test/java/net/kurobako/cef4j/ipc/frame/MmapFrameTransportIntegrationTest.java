...
/**
 * Validates the {@link MmapFrameTransport} end-to-end against a real helper. Spawns a helper, drives a navigation,
 * binds a frame transport to the browser, and checks that the consumer is invoked with a properly-sized BGRA pixel
 * buffer + populated dirty rect — i.e. the same wire validation as {@code OsrPaintIntegrationTest} but routed through
 * the public {@link FrameTransport} API instead of decoding {@code OsrPaintEvent} by hand.
 */
@Timeout(60)
class MmapFrameTransportIntegrationTest {
    ...
    private static HelperProcess spawnHelperWithEnv() throws IOException {
        return IpcBrowserBackend.launchHelper(helperBinary, cefResources, Duration.ofSeconds(30));
    }
    ...
    void onFrameDeliversBgraPixelsAndDirtyRect() throws Exception {
        try (HelperProcess helper = spawnHelperWithEnv();
            ...
            // Bind the frame transport eagerly — the helper's first paint can fire before
            ...
            // MmapFrameTransport — UI marshalling is the consumer's job, not the transport's, otherwise non-FX
            ...
            try (FrameTransport ft = MmapFrameTransport.bindAll(session)) {
    ...
    void onFrameOnlyDeliversForBoundBrowser() throws Exception {
        try (HelperProcess helper = spawnHelperWithEnv();
            ...
            try (FrameTransport ft = MmapFrameTransport.bind(session, wrongHandle)) {
