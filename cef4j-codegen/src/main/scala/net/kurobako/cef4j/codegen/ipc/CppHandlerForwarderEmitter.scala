...
/** Emits one C++ class per CEF HandlerStruct that the helper can hand back from `cef_client_t::get_X_handler`. Each
  * struct subclasses the corresponding `cef_<name>_t`, sets every callback to a lambda that encodes the matching AST
  * event and sends it via the helper's `IpcServer`.
  *
  ...
  *
  * Output is a single `HandlerForwarders.h` that the helper includes once. The helper is responsible for instantiating
  * and exposing whichever forwarders it cares about (typically via `CefClient::get_X_handler`).
  *
...
object CppHandlerForwarderEmitter {
  ...
  /** Per-handler class name: `cef_load_handler_t` → `LoadHandlerForwarder`. The CEF "Cef" prefix on the JVM-side
    * interface is dropped here for brevity since these are internal helper types.
    */
  ...
  private def renderCallbackInit(m: HandlerMethod, cefStruct: String, facadeStructs: Set[String]): String = {
    ...
    val fnTypeRef = s"gendisp::fn_args<decltype($cefStruct::${m.cefMethodName})>"
    ...
    m.returnType match {
      ...
      case Some(FieldType.Bool) =>
        ...
        val fnRef   = s"decltype($cefStruct::${m.cefMethodName})"
